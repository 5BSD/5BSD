...
ATF_TC_BODY(iso_event_mask, tc)
{
	...
	/*
	 * No ISO features -> no feature-gated ISO event bits.
	 *
	 * CIS Established is NOT among them: §7.7.65.25 says the event "is
	 * generated by the Controller in the Central and Peripheral", so
	 * gating it on the central-role feature lost it on a peripheral-only
	 * ISO controller -- which accepts a CIS request and then never learns
	 * the stream came up.  §7.8.1 makes setting a bit for an event the
	 * controller does not support free ("the Controller shall ignore
	 * those bits"), so it is set unconditionally.
	 */
	...
	/* Either CIS role -> CIS Established (subevent 0x19, §7.7.65.25). */
