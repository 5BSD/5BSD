...
manifest_body()
{
	...
	cp "${objdir}/blued" "${unit}/bin/blued"
	chmod 0555 "${bundle}" "${bundle}/Units" "${unit}" "${unit}/bin" \
	    "${unit}/Config" "${unit}/bin/blued"
	...
	atf_check -s exit:0 -o match:'org.5bsd.blued' \
	    grep 'activation.*ipc.*org.5bsd.blued' "${unit}/Unit.ucl"
...
manifest_cleanup()
{
	chmod -R u+w "${PWD}/Blued.cap" 2>/dev/null || true
	rm -rf "${PWD}/Blued.cap"
}
...
package_layout_head()
{
	atf_set "descr" "pkgbase installs every Blued artifact inside Blued.cap"
}
package_layout_body()
{
	makefile="@SRCTOP@/usr.sbin/bluetooth/blued/Makefile"

	for token in '/Capabilities/System/Blued.cap' \
