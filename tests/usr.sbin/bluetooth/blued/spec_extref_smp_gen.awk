...
END {
	...
	print " *"
	print " * WARNING - NO CORE 6.3 SOURCE EXISTS FOR THE SIGNED-WRITE VECTORS."
	print " * LE data signing has been REMOVED from the Core specification as of"
	print " * 6.x: Core_Specification_6_3.txt contains zero occurrences of"
	print " * \"Signed Write\", \"CSRK\", \"Signing Information\" or \"SignKey\"."
	print " * ATT opcode 0xD2 (Signed Write Command) and SMP command code 0x0A"
	print " * (Signing Information) are now both marked \"Previously used\", and"
	print " * the LE key distribution SignKey bit ([SPEC] Vol 3, Part H,"
	print " * Section 3.6.1, Figure 3.11) is likewise \"Previously used\"."
	print " * \"Previously used\" is defined in [SPEC] Vol 1, Part E, Section 2.4.2."
	print " *"
	print " * The signature construction therefore has no in-tree spec citation."
	print " * Its only external sources are:"
	...
	print " *       Vol 3, Part C, Section 10.4.1 - text that is ABSENT from"
	print " *       Core 6.3 because the feature was removed."
	print " */"
	for (i = 1; i <= nbz; i++) {
		c = bzorder[i]
