...
/* ================================================================
 * LE Event Mask bits that must be set UNCONDITIONALLY (§7.8.1)
 *
 * §7.8.1: "The Controller shall ignore those bits which are reserved for
 * future use or represent events which it does not support.  If the Host sets
 * any of these bits to 1, the Controller shall act as if they were set to 0."
 * Unmasking an event the controller cannot generate is therefore free, while
 * a gate keyed on the wrong feature loses the event silently.  These four
 * bits were either gated on the wrong feature or never set at all.
 * ================================================================ */
ATF_TC_WITHOUT_HEAD(le_event_mask_unconditional_bits);
ATF_TC_BODY(le_event_mask_unconditional_bits, tc)
{
	uint64_t none;

	/* A controller advertising NO optional LE feature at all. */
	none = hci_le_default_event_mask(0);

	/*
	 * Bit 5, Remote Connection Parameter Request.  Vol 6 Part B §5.1.7.2:
	 * masked, the Link Layer rejects the peer's procedure on air with
	 * Unsupported Remote Feature (0x1A).
	 */
	ATF_CHECK_MSG((none & (UINT64_C(1) <<
	    BT_EXTREF_LE_EVTMASK_REMOTE_CONN_PARAM_REQ)) != 0,
	    "bit 5 masked: peers get LL_REJECT_EXT_IND 0x%02x",
	    BT_EXTREF_LE_EVTMASK_BIT5_MASKED_LL_REJECT_ERROR);

	/* Bit 10, Directed Advertising Report: a bonded peer's reconnection. */
	ATF_CHECK_MSG((none & (UINT64_C(1) <<
	    BT_EXTREF_LE_EVTMASK_DIRECTED_ADV_REPORT)) != 0,
	    "bit 10 masked: directed advertisements are invisible");

	/* Bit 30, Request Peer SCA Complete: the daemon issues the command. */
	ATF_CHECK_MSG((none & (UINT64_C(1) <<
	    BT_EXTREF_LE_EVTMASK_REQUEST_PEER_SCA_COMPLETE)) != 0,
	    "bit 30 masked: LE Request Peer SCA cannot complete");

	/*
	 * Bit 24, CIS Established.  §7.7.65.25: "It is generated by the
	 * Controller in the Central and Peripheral", so gating it on the
	 * central-role feature loses it on a peripheral-only ISO controller.
	 */
	ATF_CHECK_MSG((none & (UINT64_C(1) <<
	    BT_EXTREF_LE_EVTMASK_CIS_ESTABLISHED)) != 0,
	    "bit 24 masked: an accepted CIS never reports establishment");

	/* A peripheral-only ISO controller: both CIS events reach the host. */
	{
		uint64_t periph = hci_le_default_event_mask(LE_FEAT_CIS_PERIPH);

		ATF_CHECK((periph & (UINT64_C(1) <<
		    BT_EXTREF_LE_EVTMASK_CIS_ESTABLISHED)) != 0);
		ATF_CHECK((periph & (UINT64_C(1) <<
		    BT_EXTREF_LE_EVTMASK_CIS_REQUEST)) != 0);
	}

	/* No bit outside the 5.2 + Subrating scope is set. */
	ATF_CHECK_EQ_MSG(0, none >> (BT_EXTREF_LE_EVTMASK_MAX_BIT_IN_SCOPE + 1),
	    "a bit beyond the in-scope range was set");
}
...
ATF_TP_ADD_TCS(tp)
{
	...
	ATF_TP_ADD_TC(tp, le_event_mask_unconditional_bits);
