 ...
 *
 * Tests for cap_rt_coalition — capability-based resource group management.
 *
 * Requires:
 *   kldload cap_rt
...
#define	COALITION_TEST_JAIL_NAME	"cap_rt_coalition_jail_member_test"
...
coalition_recv_event(int fd, struct coalition_event_msg *ev)
{
	...
	return (ioctl(fd, CAP_RT_RECVMSG, &ra));
}
...
ATF_TC_BODY(stat_empty, tc)
{
	...
	ATF_CHECK_EQ(sr.cap_rt_count, 0);
...
ATF_TC(cap_rt_terminate_ioctl);
ATF_TC_HEAD(cap_rt_terminate_ioctl, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_TERMINATE ioctl on coalition fd triggers co_revoke");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_coalition");
}
ATF_TC_BODY(cap_rt_terminate_ioctl, tc)
{
	int fd;
...
ATF_TC(enlist_cap_rt_member);
ATF_TC_HEAD(enlist_cap_rt_member, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Enlist a cap_rt instance as member, verify STAT");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
}
ATF_TC_BODY(enlist_cap_rt_member, tc)
{
	struct coalition_stat_reply sr;
	int cfd, member_fd;
	int32_t status;
	...
	member_fd = cap_rt_connect("test_keystore");
	...
	ATF_CHECK_EQ(sr.member_count, 1);
	ATF_CHECK_EQ(sr.cap_rt_count, 1);

	close(cfd);
	close(member_fd);
}

ATF_TC(terminate_revokes_cap_rt);
ATF_TC_HEAD(terminate_revokes_cap_rt, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Terminating coalition revokes cap_rt members (ECONNRESET)");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
}
ATF_TC_BODY(terminate_revokes_cap_rt, tc)
{
	int cfd, member_fd;
	int32_t status;
	...
	/*
	 * The cap_rt member should now be revoked.
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Enlist socket and cap_rt, verify mixed STAT");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
}
...
ATF_TC_BODY(enlist_nested_coalition, tc)
{
	...
	parent = cap_rt_connect("coalition");
	ATF_REQUIRE(parent >= 0);

	child = cap_rt_connect("coalition");
	ATF_REQUIRE(child >= 0);
...
ATF_TC_BODY(nested_depth_tracking, tc)
{
	...
	level0 = cap_rt_connect("coalition");
	ATF_REQUIRE(level0 >= 0);
	level1 = cap_rt_connect("coalition");
	ATF_REQUIRE(level1 >= 0);
	level2 = cap_rt_connect("coalition");
...
ATF_TC_BODY(nested_depth_limit, tc)
{
	...
	for (i = 0; i < (int)nitems(fds); i++) {
		fds[i] = cap_rt_connect("coalition");
...
ATF_TC(watchdog_fires_revokes_cap_rt);
ATF_TC_HEAD(watchdog_fires_revokes_cap_rt, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Watchdog expiry revokes cap_rt members");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
	atf_tc_set_md_var(tc, "timeout", "10");
}
ATF_TC_BODY(watchdog_fires_revokes_cap_rt, tc)
{
	struct coalition_set_watchdog_req wr;
	struct coalition_reply rpl;
	struct cap_rt_sendmsg_args sa;
...
ATF_TC_BODY(large_timeouts_do_not_fire, tc)
{
	...
	sa.payload = payload;
	sa.payload_len = sizeof(payload);
	ATF_REQUIRE_MSG(ioctl(member_fd, CAP_RT_SENDMSG, &sa) == 0,
	    "cap_rt member revoked unexpectedly: %s", strerror(errno));
...
ATF_TC(set_leader_cap_rt);
ATF_TC_HEAD(set_leader_cap_rt, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Cap_rt leader death (revoke) terminates coalition members");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
}
ATF_TC_BODY(set_leader_cap_rt, tc)
{
	struct coalition_req_hdr hdr;
	struct coalition_stat_reply sr;
	struct coalition_reply rpl;
	...
	leader_fd = cap_rt_connect("test_keystore");
	ATF_REQUIRE(leader_fd >= 0);

	other_fd = cap_rt_connect("test_keystore");
	...
	/* Kill the leader — CAP_RT_TERMINATE revokes the instance */
	ATF_REQUIRE(ioctl(leader_fd, CAP_RT_TERMINATE, NULL) == 0);
	...
	sa.payload_len = sizeof(payload);
	ATF_CHECK_ERRNO(EPIPE,
	    ioctl(other_fd, CAP_RT_SENDMSG, &sa) == -1);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_GETINFO returns coalition service metadata");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_coalition");
}
...
ATF_TC(graceful_terminate_revokes_cap_rt);
ATF_TC_HEAD(graceful_terminate_revokes_cap_rt, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Graceful terminate revokes cap_rt members");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
}
ATF_TC_BODY(graceful_terminate_revokes_cap_rt, tc)
{
	struct coalition_graceful_req gr;
	struct coalition_reply rpl;
	struct cap_rt_sendmsg_args sa;
...
ATF_TC(deadline_fires_revokes_cap_rt);
ATF_TC_HEAD(deadline_fires_revokes_cap_rt, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Deadline expiry revokes cap_rt members");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
	atf_tc_set_md_var(tc, "timeout", "10");
}
ATF_TC_BODY(deadline_fires_revokes_cap_rt, tc)
{
	struct coalition_set_deadline_req dr;
	struct coalition_reply rpl;
	struct cap_rt_sendmsg_args sa;
...
ATF_TC_BODY(multiple_coalitions, tc)
{
	...
	fd1 = cap_rt_connect("coalition");
	ATF_REQUIRE(fd1 >= 0);
	fd2 = cap_rt_connect("coalition");
	ATF_REQUIRE(fd2 >= 0);
	fd3 = cap_rt_connect("coalition");
	ATF_REQUIRE(fd3 >= 0);
...
/* ================================================================
 * Cap_rt descriptor type tests
 * ================================================================ */

ATF_TC(cap_rt_member_type_tracking);
ATF_TC_HEAD(cap_rt_member_type_tracking, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Cap_rt members are tracked as DTYPE_CAP_RT, not generic");
	atf_tc_set_md_var(tc, "require.kmods",
	    "cap_rt cap_rt_coalition cap_rt_test_keystore");
}
ATF_TC_BODY(cap_rt_member_type_tracking, tc)
{
	struct coalition_stat_reply sr;
	int cfd, ks_fd, sv[2];
	int32_t status;
	...
	/* STAT must distinguish cap_rt from sockets */
	ATF_REQUIRE(coalition_stat(cfd, &sr) == 0);
	ATF_CHECK_EQ(sr.member_count, 2);
	ATF_CHECK_EQ(sr.cap_rt_count, 1);
	ATF_CHECK_EQ(sr.other_count, 1);
...
ATF_TC(cap_rt_member_multiple_services);
ATF_TC_HEAD(cap_rt_member_multiple_services, tc)
{
	atf_tc_set_md_var(tc, "descr",
...
ATF_TC_BODY(cap_rt_member_multiple_services, tc)
{
	...
	ATF_REQUIRE(coalition_stat(cfd, &sr) == 0);
	ATF_CHECK_EQ(sr.cap_rt_count, 2);
	ATF_CHECK_EQ(sr.member_count, 2);
...
ATF_TC_HEAD(coalition_is_cap_rt_type, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Coalition fd is a cap_rt descriptor (GETINFO works)");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_coalition");
}
ATF_TC_BODY(coalition_is_cap_rt_type, tc)
{
	struct cap_rt_info_args info;
	struct stat sb;
	int fd;
	...
	/* GETINFO confirms it's a cap_rt instance */
	memset(&info, 0, sizeof(info));
	ATF_REQUIRE(ioctl(fd, CAP_RT_GETINFO, &info) == 0);
	ATF_CHECK_STREQ(info.name, "coalition");
	ATF_CHECK(info.features & CAP_RT_INFO_F_CALL);
...
ATF_TC(cap_rt_revoke_send_on_coalition);
ATF_TC_HEAD(cap_rt_revoke_send_on_coalition, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_REVOKE_CALL on coalition fd strips call ability");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_coalition");
}
ATF_TC_BODY(cap_rt_revoke_send_on_coalition, tc)
{
	struct coalition_req_hdr hdr;
	struct coalition_reply rpl;
	int fd;
...
ATF_TP_ADD_TCS(tp)
{
	...
	ATF_TP_ADD_TC(tp, cap_rt_terminate_ioctl);
	...
	/* Cap_rt members */
	ATF_TP_ADD_TC(tp, enlist_cap_rt_member);
	ATF_TP_ADD_TC(tp, terminate_revokes_cap_rt);
	...
	ATF_TP_ADD_TC(tp, watchdog_fires_revokes_cap_rt);
	...
	ATF_TP_ADD_TC(tp, set_leader_cap_rt);
	...
	ATF_TP_ADD_TC(tp, graceful_terminate_revokes_cap_rt);

	/* Deadline */
	ATF_TP_ADD_TC(tp, deadline_fires);
	ATF_TP_ADD_TC(tp, deadline_fires_revokes_cap_rt);
	...
	ATF_TP_ADD_TC(tp, cap_rt_member_multiple_services);
	ATF_TP_ADD_TC(tp, terminate_revokes_multiple_services);
	ATF_TP_ADD_TC(tp, coalition_is_cap_rt_type);
	ATF_TP_ADD_TC(tp, cap_rt_revoke_send_on_coalition);
