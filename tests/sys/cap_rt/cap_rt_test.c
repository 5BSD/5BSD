 ...
 *
 * Tests for cap_rt — generic capability interface for kernel modules.
 *
 * Requires:
 *   kldload cap_rt
...
static int
cap_rt_send(int fd, const void *payload, uint32_t len, uint64_t token)
{
...
/* Helper: receive a message via CAP_RT_RECVMSG (blocks) */
static int
cap_rt_recv(int fd, void *buf, uint32_t *lenp, uint64_t *tokenp)
{
...
static void
cap_rt_fetch_trailer(int fd, struct cap_rt_cred_trailer *trailer)
{
...
static void
cap_rt_store(int fd, uint32_t keyid, const void *data, size_t datalen)
{
	...
	ATF_REQUIRE(cap_rt_send(fd, buf, sizeof(*req) + datalen, 0) == 0);
	rlen = sizeof(reply);
...
{
	atf_tc_set_md_var(tc, "descr", "Open and close /dev/cap_rt");
}
...
{
	int fd = cap_rt_connect("test_keystore");
	ATF_REQUIRE(fd >= 0);
	close(fd);
}
...
ATF_TC_BODY(connect_noent, tc)
{
	...
	ATF_CHECK_ERRNO(ENOENT, ioctl(ctl, CAP_RT_CONNECT, &ca) == -1);
	close(ctl);
}
...
ATF_TC_BODY(connect_empty_name, tc)
{
	...
	memset(&ca, 0, sizeof(ca));
	ATF_CHECK_ERRNO(EINVAL, ioctl(ctl, CAP_RT_CONNECT, &ca) == -1);
	close(ctl);
}
...
ATF_TC_HEAD(fcntl_cloexec_flag, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "F_GETFD/F_SETFD work on cap_rt instance fds");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "FD_CLOEXEC closes a cap_rt instance fd across exec");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(cloexec_on_exec, tc)
{
	...
	ATF_REQUIRE(pid >= 0);
	if (pid == 0) {
		snprintf(exec_path, sizeof(exec_path), "%s/cap_rt_exec_helper",
...
{
	atf_tc_set_md_var(tc, "descr",
	    "fcntl(O_NONBLOCK) enables nonblocking CAP_RT_RECVMSG");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(fcntl_nonblock_recvmsg, tc)
{
	...
	ATF_CHECK_ERRNO(EAGAIN, ioctl(fd, CAP_RT_RECVMSG, &ra) == -1);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "cap_ioctls_limit restricts CAP_RT instance ioctls");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(capsicum_ioctl_limit, tc)
{
	...
	sa.payload = "x";
	sa.payload_len = 1;
	ATF_CHECK_ERRNO(ENOTCAPABLE, ioctl(fd, CAP_RT_SENDMSG, &sa) == -1);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "cap_fcntls_limit restricts fcntl operations on cap_rt fds");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_EVENT gates kqueue registration on cap_rt instance fds");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Existing cap_rt instances keep working after cap_enter");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(capmode_existing_instance, tc)
{
	...
	if (pid == 0) {
		...
		if (cap_enter() != 0)
			_exit(4);
		memset(&info, 0, sizeof(info));
		if (ioctl(fd, CAP_RT_GETINFO, &info) != 0)
			_exit(5);
		_exit(0);
	}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_SENDMSG enqueues async, CAP_RT_RECVMSG gets reply");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(write_read, tc)
{
	...
	ATF_REQUIRE(cap_rt_send(fd, buf, sizeof(req) + 5, 0) == 0);
...
ATF_TC_BODY(concurrent_instances, tc)
{
	...
	fd1 = cap_rt_connect("test_keystore");
	fd2 = cap_rt_connect("test_keystore");
	ATF_REQUIRE(fd1 >= 0);
	ATF_REQUIRE(fd2 >= 0);
	ATF_CHECK(fd1 != fd2);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_SENDMSG with payload_len=0 sends a zero-length message");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(zero_length_write_noop, tc)
{
	...
	ATF_REQUIRE(cap_rt_send(fd, NULL, 0, 0) == 0);
	...
	ra.payload = buf;
	ra.payload_len = sizeof(buf);
	ATF_CHECK(ioctl(fd, CAP_RT_RECVMSG, &ra) == 0);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "O_NONBLOCK CAP_RT_RECVMSG with no pending messages returns EAGAIN");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(nonblock_read_eagain, tc)
{
	...
	rlen = sizeof(buf);
	ATF_CHECK_ERRNO(EAGAIN, cap_rt_recv(fd, buf, &rlen, NULL) == -1);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_RECVMSG dequeues reply with payload and reply token");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Sending > CAP_RT_MAX_FDS fds fails EINVAL");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(fd_too_many, tc)
{
	struct cap_rt_sendmsg_args sa;
	int fds[CAP_RT_MAX_FDS + 1];
	int fd, i;

	fd = cap_rt_connect("test_keystore");
	ATF_REQUIRE(fd >= 0);

	for (i = 0; i <= CAP_RT_MAX_FDS; i++)
		fds[i] = STDOUT_FILENO;
	...
	sa.payload_len = 1;
	sa.fds = fds;
	sa.nfds = CAP_RT_MAX_FDS + 1;
	ATF_CHECK_ERRNO(EINVAL, ioctl(fd, CAP_RT_SENDMSG, &sa) == -1);
...
ATF_TC_BODY(queue_ordering, tc)
{
	...
	for (i = 0; i < 5; i++) {
		...
		ra.payload = buf;
		ra.payload_len = sizeof(buf);
		ATF_REQUIRE_MSG(ioctl(fd, CAP_RT_RECVMSG, &ra) == 0,
...
ATF_TC_BODY(capability_delegation, tc)
{
	...
	if (pid == 0) {
		...
		sa.payload = &req;
		sa.payload_len = sizeof(req);
		if (ioctl(fd, CAP_RT_SENDMSG, &sa) != 0)
			_exit(1);
		memset(&ra, 0, sizeof(ra));
		ra.payload = &reply;
...
ATF_TC_BODY(scm_rights_passing, tc)
{
	...
	if (pid == 0) {
...

		{
			...
			sa.payload = &req;
			sa.payload_len = sizeof(req);
			if (ioctl(received_fd, CAP_RT_SENDMSG, &sa) != 0)
				_exit(3);
			memset(&ra, 0, sizeof(ra));
			ra.payload = &reply;
			ra.payload_len = sizeof(reply);
			if (ioctl(received_fd, CAP_RT_RECVMSG, &ra) != 0)
				_exit(4);
		}
		close(received_fd);
		close(sv[1]);
...
ATF_TC_BODY(multiproc_store_fetch, tc)
{
	...
	if (pid == 0) {
		...
		struct ks_reply reply;
		char buf[128];
		int cfd = cap_rt_connect("test_keystore");
		if (cfd < 0)
			_exit(10);
		...
		sa.payload = &req;
		sa.payload_len = sizeof(req);
		if (ioctl(cfd, CAP_RT_SENDMSG, &sa) != 0)
			_exit(1);
		...
		ra.payload = buf;
		ra.payload_len = sizeof(buf);
		if (ioctl(cfd, CAP_RT_RECVMSG, &ra) != 0)
			_exit(2);
...
ATF_TC_BODY(close_during_pending, tc)
{
	...
	for (i = 0; i < 20; i++) {
		req.op = KS_OP_FETCH;
		req.keyid = (uint32_t)i;
		(void)cap_rt_send(fd, &req, sizeof(req), 0);
	}
...
ATF_TC_BODY(rapid_fire, tc)
{
	...
	for (i = 0; i < 100; i++) {
		rlen = sizeof(buf);
		ATF_REQUIRE_MSG(
		    cap_rt_recv(fd, buf, &rlen, NULL) == 0,
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_SENDMSG with payload > CAP_RT_MAX_MSG returns EMSGSIZE");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(write_oversized, tc)
{
	...
	big = calloc(1, CAP_RT_MAX_MSG + 1);
	ATF_REQUIRE(big != NULL);
	memset(&sa, 0, sizeof(sa));
	sa.payload = big;
	sa.payload_len = CAP_RT_MAX_MSG + 1;
	ATF_CHECK_ERRNO(EMSGSIZE, ioctl(fd, CAP_RT_SENDMSG, &sa) == -1);
	free(big);
	close(fd);
}
...
ATF_TC_BODY(sendmsg_bad_fd, tc)
{
	...
	sa.fds = &badfd;
	sa.nfds = 1;
	ATF_CHECK_ERRNO(EBADF, ioctl(fd, CAP_RT_SENDMSG, &sa) == -1);
	close(fd);
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_RECVMSG with buffer smaller than message returns EMSGSIZE");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(read_buffer_too_small, tc)
{
	...
	cap_rt_store(fd, 100, "big_value", 9);
	...
	ATF_CHECK_ERRNO(EMSGSIZE, ioctl(fd, CAP_RT_RECVMSG, &ra) == -1);
...
ATF_TC_BODY(recvmsg_buffer_too_small, tc)
{
	...
	fd = cap_rt_connect("test_keystore");
	ATF_REQUIRE(fd >= 0);
	cap_rt_store(fd, 200, "somedata", 8);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Unknown ioctl on /dev/cap_rt returns ENOTTY");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CONNECT on a instance fd returns ENOTTY");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(connect_on_instance_fd, tc)
{
	...
	ATF_CHECK_ERRNO(ENOTTY, ioctl(fd, CAP_RT_CONNECT, &ca) == -1);
	close(fd);
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_SENDMSG after peer process closes shared fd still works");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_GETINFO returns service identity and basic metadata");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(getinfo, tc)
{
	...
	{
		struct cap_rt_info_args info2;
		int fd2 = cap_rt_connect("test_keystore");
		ATF_REQUIRE(fd2 >= 0);
		ATF_REQUIRE(ioctl(fd2, CAP_RT_GETINFO, &info2) == 0);
...
ATF_TC_BODY(pair_bidirectional, tc)
{
	...
	ATF_REQUIRE(cap_rt_send(fd_b, "world", 5, 0) == 0);

	memset(buf, 0, sizeof(buf));
	rlen = sizeof(buf);
	ATF_REQUIRE(cap_rt_recv(fd_a, buf, &rlen, NULL) == 0);
	ATF_REQUIRE(rlen == 5);
...
ATF_TC_BODY(pair_forwards_metadata, tc)
{
	struct cap_rt_recvmsg_args ra;
	struct cap_rt_info_args info;
	struct cap_rt_cred_trailer trailer;
	char payload[] = "meta";
	char buf[64];
	int fd_a, fd_b, ks_fd;
	...
	ks_fd = cap_rt_connect("test_kernelstore");
	ATF_REQUIRE(ks_fd >= 0);
	cap_rt_fetch_trailer(ks_fd, &trailer);
	ATF_REQUIRE(trailer.nonce != 0);

	ATF_REQUIRE(cap_rt_send(fd_a, payload, sizeof(payload) - 1,
	...
	ra.payload = buf;
	ra.payload_len = sizeof(buf);
	ATF_REQUIRE(ioctl(fd_b, CAP_RT_RECVMSG, &ra) == 0);
...
ATF_TC_BODY(pair_forwards_fds_and_metadata, tc)
{
	...
	ATF_REQUIRE(ioctl(fd_a, CAP_RT_SENDMSG, &sa) == 0);
...
ATF_TC_BODY(pair_close_one_end, tc)
{
	...
	for (i = 0; i < 100; i++) {
		if (cap_rt_send(fd_b, "x", 1, 0) == -1 &&
		    (errno == EPIPE || errno == ECONNRESET))
			break;
		usleep(10000);
	}
	ATF_CHECK(cap_rt_send(fd_b, "x", 1, 0) == -1);
	ATF_CHECK(errno == EPIPE || errno == ECONNRESET);

	{
		uint32_t rlen = sizeof(buf);
		ATF_CHECK(cap_rt_recv(fd_b, buf, &rlen, NULL) == -1);
...
ATF_TC_BODY(pair_multiproc, tc)
{
	...
	if (pid == 0) {
		...
		sa.payload = "child";
		sa.payload_len = 5;
		if (ioctl(fd_b, CAP_RT_SENDMSG, &sa) != 0)
			_exit(1);
		close(fd_b);
		_exit(0);
	}
...
ATF_TC_BODY(pair_crossproc_forwards_metadata, tc)
{
	struct cap_rt_recvmsg_args ra;
	struct cap_rt_info_args info;
	struct cap_rt_cred_trailer expected;
	char buf[64];
	...
	memset(&info, 0, sizeof(info));
	ATF_REQUIRE(ioctl(fd_b, CAP_RT_GETINFO, &info) == 0);
	...
	if (pid == 0) {
		struct cap_rt_cred_trailer child_trailer;
		...
		cap_rt_fetch_trailer(ks_fd, &child_trailer);
		if (write(sv[1], &child_trailer, sizeof(child_trailer)) !=
		    sizeof(child_trailer))
			_exit(12);
		if (cap_rt_send(fd_b, "childmeta", 9, 0xaabbccddULL) != 0)
			_exit(13);
		close(ks_fd);
		close(fd_b);
		close(sv[1]);
	...
	ra.payload = buf;
	ra.payload_len = sizeof(buf);
	ATF_REQUIRE(ioctl(fd_a, CAP_RT_RECVMSG, &ra) == 0);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_GETINFO returns real limits and feature bits");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(getinfo_limits, tc)
{
	...
	memset(&info, 0, sizeof(info));
	ATF_REQUIRE(ioctl(fd, CAP_RT_GETINFO, &info) == 0);
	ATF_CHECK_EQ(info.msg_limit, CAP_RT_MAX_MSG);
	ATF_CHECK_EQ(info.queue_depth, CAP_RT_DEFAULT_QUEUE_DEPTH);
	ATF_CHECK_EQ(info.tx_limit, CAP_RT_DEFAULT_TX_LIMIT);
	ATF_CHECK_EQ(info.max_fds, CAP_RT_MAX_FDS);
	...
	ATF_CHECK((info.features & CAP_RT_INFO_F_KQUEUE) != 0);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CALL on async-only service returns EOPNOTSUPP");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(call_no_handler, tc)
{
	...
	ATF_CHECK_ERRNO(EOPNOTSUPP, ioctl(fd, CAP_RT_CALL, &ca) == -1);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CALL with nonzero flags returns EINVAL on sync service");
...
ATF_TC_BODY(call_flags_nonzero, tc)
{
	...
	memset(&ca, 0, sizeof(ca));
	ca.flags = 1;
	ATF_CHECK_ERRNO(EINVAL, ioctl(fd, CAP_RT_CALL, &ca) == -1);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_TERMINATE kills the instance for all holders");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(terminate_instance, tc)
{
	...
	/* Send revoke token. */
	ATF_REQUIRE(ioctl(fd, CAP_RT_TERMINATE, NULL) == 0);
	...
	ra.payload = buf;
	ra.payload_len = sizeof(buf);
	ATF_CHECK_ERRNO(ECONNRESET, ioctl(fd, CAP_RT_RECVMSG, &ra) == -1);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_TERMINATE works on sync-only service");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_kernelstore");
}
ATF_TC_BODY(terminate_sync_service, tc)
{
	...
	ATF_CHECK_ERRNO(ECONNRESET, ioctl(fd, CAP_RT_CALL, &ca) == -1);
...
ATF_TC_BODY(normal_ops_no_terminate, tc)
{
	...
	ATF_REQUIRE(cap_rt_send(fd, &req, sizeof(req), 0x1234) == 0);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_SENDMSG on sync-only service returns EOPNOTSUPP");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_kernelstore");
}
ATF_TC_BODY(sendmsg_on_sync_service, tc)
{
	...
	op = 1; /* any op */
	ATF_CHECK_ERRNO(EOPNOTSUPP, cap_rt_send(fd, &op, sizeof(op), 0) == -1);
...
ATF_TC_BODY(pair_stress, tc)
{
	...
	for (i = 0; i < 100; i++) {
		...
		ATF_REQUIRE(cap_rt_send(fd_a, &val, sizeof(val), 0) == 0);
...
ATF_TC_BODY(keystore_max_keys, tc)
{
	...
	for (i = 0; i < 50; i++)
		cap_rt_store(fd, 90000 + (uint32_t)i, "val", 3);
...
ATF_TC_BODY(keystore_overwrite, tc)
{
	...
	/* Store "old". */
	cap_rt_store(fd, 80000, "old", 3);

	/* Overwrite with "new_value". */
	cap_rt_store(fd, 80000, "new_value", 9);
...
ATF_TC_BODY(keystore_uid_isolation, tc)
{
	...
	if (pid == 0) {
		...
		req.op = KS_OP_FETCH;
		req.keyid = keyid;
		if (cap_rt_send(fd, &req, sizeof(req), 0) != 0)
			_exit(1);
		rlen = sizeof(buf);
		if (cap_rt_recv(fd, buf, &rlen, NULL) != 0)
			_exit(2);
		memcpy(&reply, buf, sizeof(reply));
...
{
	struct cap_rt_info_args info1, info2, info3;
	int fd1, fd2, fd3;

	fd1 = cap_rt_connect("test_keystore");
	fd2 = cap_rt_connect("test_keystore");
	fd3 = cap_rt_connect("test_keystore");
	ATF_REQUIRE(fd1 >= 0);
	ATF_REQUIRE(fd2 >= 0);
	ATF_REQUIRE(fd3 >= 0);
	...
	ATF_REQUIRE(ioctl(fd1, CAP_RT_GETINFO, &info1) == 0);
	ATF_REQUIRE(ioctl(fd2, CAP_RT_GETINFO, &info2) == 0);
	ATF_REQUIRE(ioctl(fd3, CAP_RT_GETINFO, &info3) == 0);
...
ATF_TC_BODY(fdclose_dup_survives, tc)
{
	...
	ATF_REQUIRE(cap_rt_send(fd_a2, "alive", 5, 0) == 0);
	{
		char buf[64];
		uint32_t rlen = sizeof(buf);
		ATF_REQUIRE(cap_rt_recv(fd_b, buf, &rlen, NULL) == 0);
...
ATF_TC_BODY(revoke_send_recv_both, tc)
{
	...
	memset(&info, 0, sizeof(info));
	ATF_CHECK(ioctl(fd, CAP_RT_GETINFO, &info) == 0);
	ATF_CHECK_STREQ(info.name, "test_keystore");
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_TERMINATE works even after CAP_RT_REVOKE_SEND");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CALL with reply_nfds=0 works normally");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_kernelstore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CALL with reply_fds buffer but handler returns 0 fds");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_kernelstore");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CALL with reply_nfds > CAP_RT_MAX_FDS returns EINVAL");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_kernelstore");
}
ATF_TC_BODY(call_reply_nfds_too_many, tc)
{
	...
	ca.req = &kr;
	ca.req_len = sizeof(kr);
	ca.reply_nfds = CAP_RT_MAX_FDS + 1;
	ATF_CHECK_ERRNO(EINVAL, ioctl(fd, CAP_RT_CALL, &ca) == -1);
...
ATF_TC_BODY(lock_trampoline, tc)
{
	...
	if (pid == 0) {
		...
		/* Verify it still works. */
		{
			struct cap_rt_info_args info;
			memset(&info, 0, sizeof(info));
			if (ioctl(received_fd, CAP_RT_GETINFO, &info) != 0)
				_exit(4);
		}
...
run_shield_helper(const char *op, pid_t target)
{
	...
	if (helper < 0)
		return (-1);
	if (helper == 0) {
		execl(path, "cap_rt_shield_helper", op, pidstr, NULL);
		_exit(2);
	}
...
ATF_TC_BODY(cap_pro_shield_on_token_fails, tc)
{
	...
	ca.req = &req;
	ca.req_len = sizeof(req);
	ATF_CHECK_ERRNO(EINVAL, ioctl(token_fd, CAP_RT_CALL, &ca) == -1);
	close(token_fd);
	close(fd);
}
...
ATF_TC_BODY(cap_pro_authorize_grants_access, tc)
{
	 ...
	 * the auth table.  But CAP_RT fds are CLOEXEC...
	 *
	...
	ca.reply_fds = reply_fds;
	ca.reply_nfds = 1;
	ATF_REQUIRE(ioctl(shield_fd, CAP_RT_CALL, &ca) == 0);
	token_fd = reply_fds[0];
	...
	ca.req = &req;
	ca.req_len = sizeof(req);
	ATF_CHECK(ioctl(token_fd, CAP_RT_CALL, &ca) == 0);
...
ATF_TC_BODY(cap_pro_token_close_revokes, tc)
{
	...
	ca.req = &req;
	ca.req_len = sizeof(req);
	ATF_REQUIRE(ioctl(token_fd, CAP_RT_CALL, &ca) == 0);
...
ATF_TC_BODY(cap_pro_delegated_access, tc)
{
	...
	if (helper == 0) {
		execl(path, "cap_rt_shield_helper",
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Send exactly CAP_RT_MAX_MSG bytes, verify round trip");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(max_payload_size, tc)
{
	...
	big = calloc(1, CAP_RT_MAX_MSG);
	ATF_REQUIRE(big != NULL);
	req = (struct ks_request *)big;
	req->op = KS_OP_STORE;
	req->keyid = 55555;
	memset(big + sizeof(*req), 'A', CAP_RT_MAX_MSG - sizeof(*req));
	ATF_REQUIRE(cap_rt_send(fd, big, CAP_RT_MAX_MSG, 0) == 0);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Send CAP_RT_MAX_MSG+1 bytes returns EMSGSIZE");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
...
ATF_TC_BODY(pair_send_after_peer_close, tc)
{
	...
	ATF_CHECK(cap_rt_send(fd_b, "dead", 4, 0) == -1);
	ATF_CHECK(errno == EPIPE || errno == ECONNRESET);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_TERMINATE twice is harmless");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(terminate_twice, tc)
{
	...
	ATF_CHECK(ioctl(fd, CAP_RT_TERMINATE, NULL) == 0);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CONNECT with nonzero reserved fields returns EINVAL");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "CAP_RT_CALL with req_len > max returns EMSGSIZE");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_kernelstore");
}
ATF_TC_BODY(call_req_oversized, tc)
{
	...
	memset(&ca, 0, sizeof(ca));
	ca.req = big;
	ca.req_len = CAP_RT_MAX_MSG + 1;
	ca.reply = reply;
	ca.reply_len = sizeof(reply);
	ATF_CHECK_ERRNO(EMSGSIZE, ioctl(fd, CAP_RT_CALL, &ca) == -1);
	...
	for (i = 0; i < 4; i++) {
		...
		if (pids[i] == 0) {
			...
			for (j = 0; j < 10; j++) {
				...
				if (cap_rt_send(fd_a, &val, sizeof(val), 0) != 0)
					_exit(1);
			}
			close(fd_a);
			_exit(0);
...
ATF_TC_BODY(pair_nonblock_recv_empty, tc)
{
	...
	ra.payload = buf;
	ra.payload_len = sizeof(buf);
	ATF_CHECK_ERRNO(EAGAIN, ioctl(fd_b, CAP_RT_RECVMSG, &ra) == -1);
...
ATF_TC_BODY(recvmsg_flags_nonzero, tc)
{
	...
	ra.payload_len = sizeof(buf);
	ra.flags = 1;
	ATF_CHECK_ERRNO(EINVAL, ioctl(fd, CAP_RT_RECVMSG, &ra) == -1);
...
ATF_TC_BODY(revoke_send_on_sync, tc)
{
	...
	sa.payload = "x";
	sa.payload_len = 1;
	ATF_CHECK(ioctl(fd, CAP_RT_SENDMSG, &sa) == -1);
...
ATF_TC_BODY(revoke_call_on_async, tc)
{
	...
	ca.req = "x";
	ca.req_len = 1;
	ATF_CHECK(ioctl(fd, CAP_RT_CALL, &ca) == -1);
	ATF_CHECK(errno == EACCES || errno == EOPNOTSUPP);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "cap_ioctls_limit can block CAP_RT_TERMINATE");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_keystore");
}
ATF_TC_BODY(capsicum_gates_terminate, tc)
{
	...
	ATF_CHECK_ERRNO(ENOTCAPABLE, ioctl(fd, CAP_RT_TERMINATE, NULL) == -1);
...
ATF_TC_BODY(nonce_inherits_fork, tc)
{
	...
	if (pid == 0) {
		struct cap_rt_call_args child_ca;
		...
		if (ioctl(fd, CAP_RT_CALL, &child_ca) != 0)
			_exit(10);
...
ATF_TC_BODY(kstore_mint_shared, tc)
{
	...
	owner_fd = cap_rt_connect("test_kernelstore");
...
ATF_TC_BODY(kstore_close_destroys, tc)
{
	...
	fd1 = cap_rt_connect("test_kernelstore");
	ATF_REQUIRE(fd1 >= 0);
...
ATF_TC_BODY(kstore_separate_stores, tc)
{
	...
	fd1 = cap_rt_connect("test_kernelstore");
	fd2 = cap_rt_connect("test_kernelstore");
	ATF_REQUIRE(fd1 >= 0);
	ATF_REQUIRE(fd2 >= 0);
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Owner can revoke member via CAP_RT_TERMINATE");
	atf_tc_set_md_var(tc, "require.kmods", "cap_rt cap_rt_test_kernelstore");
}
ATF_TC_BODY(kstore_revoke_member, tc)
{
	...
	ATF_REQUIRE(ioctl(member_fd, CAP_RT_TERMINATE) == 0);
