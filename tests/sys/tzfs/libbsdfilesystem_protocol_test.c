 ...
 *
 * Host-independent adversarial protocol tests for libbsdfilesystem.
 */
...
client(enum scenario sc, int fd)
{
	...
	if (sc == SC_SHORT || sc == SC_PING_FD) {
		...
		rc = bsdfilesystem_request(fd, &req, &grant);
...
{
	struct bsdfilesystem_request request;
	struct bsdfilesystem_reply reply;
	pid_t pid;
