...
{
	struct bsdfilesystem_config config;

	bsdfilesystem_config_defaults(&config);
	...
	ATF_REQUIRE_EQ(0, bsdfilesystem_config_load(&config, "valid.ucl"));
...
ATF_TC_BODY(invalid_overlays_are_transactional, tc)
{
	...
	struct bsdfilesystem_config before, config;
	size_t i;
	...
	for (i = 0; i < nitems(invalid); i++) {
		...
		errno = 0;
		ATF_CHECK_MSG(bsdfilesystem_config_load(&config, "invalid.ucl") == -1,
