 ...
 *
 * Integration tests for bsdfilesystem(8): start the real daemon on a scratch pool and
 * drive it through libbsdfilesystem, covering bare dataset requests, the anonymous
...
begin_test_session(int chan)
{
	...
	ATF_REQUIRE_EQ(0, bsdfilesystem_begin_session(chan,
...
ATF_TC_BODY(bare_lease, tc)
{
	...
	req.lifetime = BSDFILESYSTEM_LEASE;
	ATF_REQUIRE_EQ(0, bsdfilesystem_request(chan, &req, &g));
	ATF_CHECK(strstr(g.dataset, "/ephemeral/lease-") != NULL);
	ATF_CHECK((fcntl(g.handle_fd, F_GETFD) & FD_CLOEXEC) != 0);

	dir = bsdfilesystem_mount_dir(g.handle_fd, 0);
	ATF_REQUIRE(dir != -1);
	...
	ATF_REQUIRE_EQ(0, bsdfilesystem_release(chan, "t1"));
	/* Release is idempotent. */
	ATF_CHECK_EQ(0, bsdfilesystem_release(chan, "t1"));
	(void)close(chan);
}
...
ATF_TC_BODY(rights_attenuation, tc)
{
	...
	(void)close(g.handle_fd);
	(void)bsdfilesystem_release(chan, "r1");
	(void)close(chan);
}
...
ATF_TC_BODY(persistent_and_badname, tc)
{
	...
	req.lifetime = BSDFILESYSTEM_PERSISTENT;
	ATF_REQUIRE_EQ(0, bsdfilesystem_request(chan, &req, &g));
	(void)close(g.handle_fd);
	...
	req.rights = ZH_PROPS_READ;
	req.lifetime = BSDFILESYSTEM_LEASE;
	ATF_CHECK_EQ(-1, bsdfilesystem_request(chan, &req, &g));
...
static int
raw_request(int chan, const void *request, size_t len, struct bsdfilesystem_reply *rp)
{
	ssize_t n;
...
ATF_TC_BODY(protocol_malformed, tc)
{
	...
	ATF_REQUIRE(chan >= 0);
	begin_test_session(chan);
	memset(&rq, 0, sizeof(rq)); rq.op = BSDFILESYSTEM_OP_PING;
	ATF_REQUIRE_EQ(0, raw_request(chan, &rq, sizeof(rq) - 1, &rp));
	ATF_CHECK_EQ(EPROTO, rp.status);
	ATF_REQUIRE_EQ(0, bsdfilesystem_ping(chan));

	memset(&rq, 0, sizeof(rq)); rq.op = BSDFILESYSTEM_OP_REQUEST;
	rq.rights = ZH_PROPS_READ; rq.lifetime = BSDFILESYSTEM_LEASE;
	...
	memset(&rq, 0, sizeof(rq)); rq.op = BSDFILESYSTEM_OP_PING; rq.rights = 1;
	ATF_REQUIRE_EQ(0, raw_request(chan, &rq, sizeof(rq), &rp));
	ATF_CHECK_EQ(EINVAL, rp.status);
...
ATF_TC_BODY(idle_client_isolated, tc)
{
	...
	tzt_require(); tzt_pool_create(tc); tzt_daemon_start();
	idle = bsdfilesystem_connect();
	active = bsdfilesystem_connect();
...
ATF_TC_BODY(lifetimes_and_session_recovery, tc)
{
	...
	struct bsdfilesystem_req req;
	struct bsdfilesystem_grant persistent, cache, boot, lease, resumed;
	char lease_dataset[BSDFILESYSTEM_DATASET_MAX], boot_dataset[BSDFILESYSTEM_DATASET_MAX];
	int chan;
	...
	req.rights = ZH_PROPS_READ;
	req.lifetime = BSDFILESYSTEM_LEASE;
	ATF_CHECK_ERRNO(ENXIO, bsdfilesystem_request(chan, &req, &resumed) == -1);
	ATF_CHECK_ERRNO(EINVAL, bsdfilesystem_begin_session(chan, "short") == -1);
	ATF_CHECK_ERRNO(EINVAL, bsdfilesystem_begin_session(chan,
	    "gggggggggggggggggggggggggggggggg") == -1);
	ATF_REQUIRE_EQ(0, bsdfilesystem_begin_session(chan, session_a));
...
#define REQUEST_ONE(key, life, result) do { \
	...
	req.rights = ZH_PROPS_READ; \
	req.lifetime = (life); \
	ATF_REQUIRE_EQ(0, bsdfilesystem_request(chan, &req, &(result))); \
	close((result).handle_fd); \
} while (0)
	REQUEST_ONE("p-life", BSDFILESYSTEM_PERSISTENT, persistent);
	REQUEST_ONE("c-life", BSDFILESYSTEM_CACHE, cache);
	REQUEST_ONE("b-life", BSDFILESYSTEM_BOOT, boot);
	REQUEST_ONE("l-life", BSDFILESYSTEM_LEASE, lease);
#undef REQUEST_ONE
	...
	req.rights = ZH_PROPS_READ;
	req.lifetime = BSDFILESYSTEM_LEASE;
	ATF_REQUIRE_EQ(0, bsdfilesystem_request(chan, &req, &resumed));
	 ...
	 */
	{
		int chan2 = bsdfilesystem_connect();

		ATF_REQUIRE(chan2 >= 0);
		ATF_REQUIRE_EQ(0, bsdfilesystem_begin_session(chan2, session_b));
		ATF_CHECK_EQ(0, tzt_systemf("zfs list %s >/dev/null 2>&1",
		    lease_dataset));
		close(chan2);
	}
...
ATF_TC_BODY(session_reconcile_refuses_snapshot_loss, tc)
{
	...
	req.lifetime = BSDFILESYSTEM_LEASE;
	ATF_REQUIRE_EQ(0, bsdfilesystem_request(chan, &req, &grant));
	ATF_REQUIRE_EQ(0, tzfs_snapshot(grant.handle_fd, "keep"));
	close(grant.handle_fd);
	ATF_CHECK(bsdfilesystem_begin_session(chan,
