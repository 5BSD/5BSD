...
vsock_head()
{
	atf_set "descr" "Linux AF_VSOCK sockets: loopback stream/dgram, SOL_VSOCK options, GET_LOCAL_CID, address validation"
...
vsock_body()
{
	atf_check -s exit:0 -o empty -e empty clang --target=x86_64-linux-gnu \
	    ...
	    -O2 -Wall -Wextra -Werror -o vsock \
	    "$(atf_get_srcdir)/linux_vsock.c"
	./vsock 2>stderr.txt; rc=$?
	cat stderr.txt >&2
	[ $rc -eq 0 ] || atf_fail "exit status $rc"
}
atf_init_test_cases()
{
	atf_add_test_case vsock
}
