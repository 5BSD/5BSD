...
atf_test_case break_mseal
break_mseal_head()
{
	atf_set "descr" "Linux break_mseal adversarial suite"
...
break_mseal_body()
{
	atf_check -s exit:0 -o empty -e empty clang --target=x86_64-linux-gnu \
	    ...
	    -O2 -Wall -Wextra -Werror -o break_mseal \
	    "$(atf_get_srcdir)/linux_break_mseal.c"
	./break_mseal 2>stderr.txt; rc=$?
	cat stderr.txt >&2
	[ $rc -eq 0 ] || atf_fail "exit status $rc"
}
atf_init_test_cases()
{
	atf_add_test_case break_mseal
}
