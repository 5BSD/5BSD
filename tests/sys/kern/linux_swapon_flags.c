/* SPDX-License-Identifier: BSD-2-Clause */
/* Freestanding Linux amd64 swapon flag-validation and lifecycle gate. */
static const char linux_device[] = "/dev/vdb";
static const char freebsd_device[] = "/dev/vtbd1";
static const char missing[] = "/no-such-swap-device";
static long
call(long n, long a, long b, long c)
{
	long ret;
	__asm__ volatile("syscall" : "=a"(ret) : "a"(n), "D"(a),
	    "S"(b), "d"(c) : "rcx", "r11", "memory");
	return (ret);
}
static int
test(void)
{
	const char *device;

	device = call(21, (long)linux_device, 0, 0) == 0 ?
	    linux_device : freebsd_device;
	if (call(21, (long)device, 0, 0) != 0)
		return (1);
	/* Unknown bits take precedence over path lookup and privilege. */
	if (call(167, (long)missing, 0x80000, 0) != -22 ||
	    call(167, 0, 0x80000000, 0) != -22 ||
	    call(167, (long)device, 0x80001, 0) != -22)
		return (2);
	if (call(167, (long)missing, 0, 0) != -2 ||
	    call(167, 0, 0, 0) != -14)
		return (3);
	if (call(167, (long)device, 0, 0) != 0)
		return (4);
	if (call(167, (long)device, 0, 0) != -16)
		return (5);
	if (call(168, (long)device, 0, 0) != 0)
		return (6);
	if (call(117, 1000, 1000, 1000) != 0)
		return (7);
	if (call(167, (long)device, 0x80000, 0) != -22 ||
	    call(167, (long)device, 0, 0) != -1 ||
	    call(167, (long)missing, 0, 0) != -1)
		return (8);
	return (0);
}
__attribute__((force_align_arg_pointer)) void
_start(void)
{
	call(60, test(), 0, 0);
	__builtin_unreachable();
}
