...
1. **Device and transport harnesses (VM-free).** Run
   `../vsock_device_harness/run.sh`.  These tests include the real bhyve device
   sources and exercise malformed descriptors, invalid queue directions,
   short buffers, reset/failure cleanup, interrupt suppression, feature
   negotiation, and boundary sizes under ASan and UBSan.
