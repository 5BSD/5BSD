...
atf_test_case cloneflags
cloneflags_head()
{
	atf_set "descr" "Linux clone/clone3 namespace-flag rejection and wait4/waitid __WALL/__WCLONE"
...
cloneflags_body()
{
	atf_check -s exit:0 -o empty -e empty clang --target=x86_64-linux-gnu \
	    ...
	    -O2 -Wall -Wextra -Werror -o cloneflags \
	    "$(atf_get_srcdir)/linux_cloneflags.c"
	./cloneflags 2>stderr.txt; rc=$?
	cat stderr.txt >&2
	[ $rc -eq 0 ] || atf_fail "exit status $rc"
}
atf_init_test_cases()
{
	atf_add_test_case cloneflags
}
