...
#define PTRACE_SYSCALL 101
#endif
#define PTRACE_STR_(x) #x
#define PTRACE_STR(x) PTRACE_STR_(x)
__asm__(".text\n.globl _start\n_start:\n"
    "mov $" PTRACE_STR(PTRACE_SYSCALL) ",%eax\nmov $-1,%edi\nsyscall\nint3\nud2\n");
#else
#include <sys/types.h>
...
main(int argc, char **argv)
{
	...
	if (argc != 2)
		errx(1, "usage: ptrace_capmode /absolute/path/to/linux_probe");
	...
	printf("PTRACE_CAPMODE result=%ld rc=%d\n", (long)regs.r_rax, rc);
out:
	if (reaped)
		return (rc);
