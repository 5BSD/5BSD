...
run(const char *path, int mode)
{
	...
	if (!p) {
		...
		if (mode == 1) cap_rights_clear(&rights, CAP_GETPEERNAME);
		if (mode == 2) cap_rights_clear(&rights, CAP_GETSOCKOPT);
		if (cap_rights_limit(sock, &rights))
			_exit(91);
		int e = getsockopt(sock, SOL_SOCKET, SO_TYPE, &type, &len);
		if (mode == 2 ? (e != -1 || errno != ENOTCAPABLE) :
				(e || type != SOCK_STREAM))
			_exit(92);
	...
	if (got != p || !WIFSTOPPED(st) || WSTOPSIG(st) != SIGTRAP ||
		...
		goto out;
	rc = mode == 0 ? ((long)regs.r_rax != 0 || regs.r_r12 != 1) :
			 ((long)regs.r_rax != -1);
	printf("PEER_CAPS mode=%d result=%ld rc=%d\n", mode, (long)regs.r_rax,
	    rc);
out:
	close(pair[1]);
	if (!reaped) {
