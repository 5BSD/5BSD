 ...
 *
 * Wire protocol for bsdnamespace(8) — the namespace (jail) broker.
 *
 * bsdnamespace owns jail construction (jail_set(2)), taking it out of PID 1.  It is a
 ...
 * fd; the *credential stored in that descriptor* (root, from bsdnamespace) is what
 ...
 * (BSDNAMESPACE_F_EPHEMERAL) bsdnamespace instead retains a *separate* owning descriptor
 ...
 * its own process — so bsdnamespace needs no per-caller token: it scopes each jail by
 ...
 * bsdnamespace — like bsdextension — is a root, non-capability-mode privileged provider.
 */

#ifndef BSDNAMESPACE_PROTO_H
#define BSDNAMESPACE_PROTO_H

#include <sys/param.h>		/* PATH_MAX */

#include <stdint.h>

#define	BSDNAMESPACE_SERVICE_NAME	"system.Namespace"

#define	BSDNAMESPACE_OP_ENTER_JAIL	1	/* create/reuse the caller's jail */
#define	BSDNAMESPACE_OP_DESTROY_JAIL	2	/* remove the caller's jail */
#define	BSDNAMESPACE_OP_LIST_JAILS	3	/* report the caller's jail */

/*
 * Request flags.
 *
 * BSDNAMESPACE_F_EPHEMERAL — the jail's lifetime is bound to the consumer.  bsdnamespace
 ...
 * bsdnamespace cannot acquire that owning descriptor it fails the request (and removes
 ...
 *
 * BSDNAMESPACE_F_VNET — the jail gets its own virtual network stack (vnet=new), a
 ...
 */
#define	BSDNAMESPACE_F_EPHEMERAL	0x1u
#define	BSDNAMESPACE_F_VNET		0x2u

/*
 * Enter-jail request.  The jail is named and scoped by the caller's channel
 * label (bsdnamespace derives it), so the request carries only the jail's shape.
 *
 * ip4_addr/ip6_addr are the jail's addresses on the host stack (empty = none of
 * that family).  With BSDNAMESPACE_F_VNET the jail has its own stack instead; an
 * address may still be supplied to seed that stack.
 */
struct bsdnamespace_request {
	uint32_t	op;			/* BSDNAMESPACE_OP_ENTER_JAIL */
	uint32_t	flags;			/* BSDNAMESPACE_F_* */
 ...
 * the owning descriptor bsdnamespace's per-client worker retains (see above).
 *
 * bsdnamespace_reply is also the reply for BSDNAMESPACE_OP_DESTROY_JAIL, which carries no
 * SCM fd (status only).
 */
struct bsdnamespace_reply {
	int32_t		status;			/* 0, or errno */
	uint32_t	_reserved;
};

/*
 * Lifecycle-control request, for BSDNAMESPACE_OP_DESTROY_JAIL and
 * BSDNAMESPACE_OP_LIST_JAILS.  Both ops act on the ONE jail that the caller's
 * unforgeable channel label scopes (bsdnamespace derives its name), so the request
 ...
 */
struct bsdnamespace_control_request {
	uint32_t	op;			/* BSDNAMESPACE_OP_DESTROY/LIST */
	uint32_t	_reserved;
};
 ...
 *
 * `flags` carries the same BSDNAMESPACE_F_* bits an ENTER would use to (re)create this
 * jail, so a consumer that LISTs after a restart can reconstruct a matching
 * request.  BSDNAMESPACE_F_VNET is set when the jail has its own virtual network stack
 * (vnet=new); BSDNAMESPACE_F_EPHEMERAL is set when this jail's lifetime is anchored by
 * bsdnamespace's per-client worker serving THIS channel (an owning descriptor is held),
 ...
 * unchanged, so it remains distinct from bsdnamespace_reply for length dispatch.)
 */
struct bsdnamespace_list_reply {
	int32_t		status;			/* 0, or errno */
	int32_t		present;		/* 1 = jail exists, 0 = none */
	int32_t		jid;			/* jail id when present */
	uint32_t	flags;			/* BSDNAMESPACE_F_* describing the jail */
