provider shmring {
	probe create(uint64_t capacity, uint32_t shape, uint32_t mode,
	    uint32_t max_record, int result);
	probe open(uint32_t role, uint64_t capacity, uint32_t shape,
	    uint32_t mode, int result);
