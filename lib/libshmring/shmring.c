    ...
    struct shmring_fds *consumer)
{
	...
	if (producer == NULL || consumer == NULL ||
	    ...
	    capacity))) {
		errno = EINVAL;
		SHMRING_PROBE_CREATE(capacity, mode, max_record, EINVAL);
		return (-1);
	}
	...
	for (i = 0; i < SHMRING_NFDS; i++)
		close(originals[i]);
	SHMRING_PROBE_CREATE(capacity, mode, max_record, 0);
	return (0);
...
fail:
	...
	SHMRING_PROBE_CREATE(capacity, mode, max_record, error);
	errno = error;
	}
	return (-1);
}
    ...
    uint32_t role)
{
	...
	*ringp = NULL;
	...
	ring->role = role;
	*ringp = ring;
	SHMRING_PROBE_OPEN(role, ring->capacity, ring->config->mode, 0);
	return (0);
...
fail:
	...
	SHMRING_PROBE_OPEN(role, ring->capacity,
	    probe_mode, error);
	shmring_close(ring);
	errno = error;
	}
...
shmring_readable(const struct shmring *ring)
{
	...
	if (ring == NULL || ring_usage(ring, &used) == -1)
		return (0);
	return ((size_t)used);
}
...
shmring_writable(const struct shmring *ring)
{
	...
	if (ring == NULL || ring_usage(ring, &used) == -1)
		return (0);
	return (ring->capacity - (size_t)used);
}
...
shmring_write(struct shmring *ring, const void *buf, size_t len)
{
	...
	if (ring == NULL || ring->role != SHMRING_ROLE_PRODUCER ||
	    ring->config->mode != SHMRING_MODE_STREAM ||
...
shmring_read(struct shmring *ring, void *buf, size_t len)
{
	...
	if (ring == NULL || ring->role != SHMRING_ROLE_CONSUMER ||
	    ring->config->mode != SHMRING_MODE_STREAM ||
	    (buf == NULL && len != 0) || len > SSIZE_MAX) {
		errno = EINVAL;
		return (-1);
