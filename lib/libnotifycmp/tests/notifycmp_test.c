...
ATF_TC_BODY(abi, tc)
{
	...
	ATF_CHECK_EQ(offsetof(struct notifycmp_event, data), 72);
...
ATF_TC_BODY(topics, tc)
{
	...
	ATF_CHECK_EQ(notifycmp_validate_topic("org.5bsd.test", 13), 0);
...
ATF_TC_BODY(events, tc)
{
	...
	*msg = message(NOTIFYCMP_OP_NEXT);
	...
	event->lost_count = 9;
	ATF_CHECK_EQ(notifycmp_validate_message(msg,
	    sizeof(*msg) + sizeof(*event), NOTIFYCMP_MESSAGE_REPLY), 0);
	event->lost_count = 0;
	ATF_CHECK_EQ(notifycmp_validate_message(msg,
	    sizeof(*msg) + sizeof(*event), NOTIFYCMP_MESSAGE_REPLY), -1);
}
...
ATF_TC_BODY(timeout_saturation, tc)
{
	...
	ATF_CHECK_EQ(notifycmp_rpc_timeout(0), 1000);
	ATF_CHECK_EQ(notifycmp_rpc_timeout(1), 1001);
	ATF_CHECK_EQ(notifycmp_rpc_timeout(UINT32_MAX - 1001),
	    UINT32_MAX - 1);
	ATF_CHECK_EQ(notifycmp_rpc_timeout(UINT32_MAX - 1000),
	    UINT32_MAX - 1);
	ATF_CHECK_EQ(notifycmp_rpc_timeout(UINT32_MAX - 1),
	    UINT32_MAX - 1);
	ATF_CHECK_EQ(notifycmp_rpc_timeout(NOTIFYCMP_TIMEOUT_INFINITE),
...
ATF_TC_BODY(api_arguments, tc)
{
	...
	ATF_CHECK_ERRNO(EINVAL, notifycmp_client_open(NULL) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_subscribe(NULL, "system.ready") == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_unsubscribe(NULL, "system.ready") == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_publish(NULL, "system.ready", NULL, 0) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_state_set(NULL, "system.ready", 1) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_state_get(NULL, "system.ready", &state) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_next(NULL, &event, sizeof(event), 0) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_timer_add(NULL, 1, 1, 0) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_timer_cancel(NULL, 1) == -1);
	ATF_CHECK_ERRNO(EINVAL, notifycmp_stats(NULL, &stats) == -1);
	notifycmp_client_close(NULL);
}
...
ATF_TC_BODY(reply_validation, tc)
{
	...
	memset(&storage, 0, sizeof(storage));
	msg = (void *)storage.bytes;
	*msg = message(NOTIFYCMP_OP_HELLO);
	hello = (void *)(msg + 1);
	...
	ATF_CHECK_EQ(0, notifycmp_validate_message(msg, length,
	    NOTIFYCMP_MESSAGE_REPLY));
	hello->router_epoch = 0;
	ATF_CHECK_EQ(-1, notifycmp_validate_message(msg, length,
	    NOTIFYCMP_MESSAGE_REPLY));
	hello->router_epoch = 1;
	hello->features &= ~NOTIFYCMP_FEATURE_STATE;
	ATF_CHECK_EQ(-1, notifycmp_validate_message(msg, length,
	    NOTIFYCMP_MESSAGE_REPLY));

	memset(&storage, 0, sizeof(storage));
	msg = (void *)storage.bytes;
	*msg = message(NOTIFYCMP_OP_STATE_GET);
	state = (void *)(msg + 1);
	state->router_epoch = 1;
	state->generation = 1;
	ATF_CHECK_EQ(0, notifycmp_validate_message(msg,
	    sizeof(*msg) + sizeof(*state), NOTIFYCMP_MESSAGE_REPLY));
	ATF_CHECK_EQ(-1, notifycmp_validate_message(msg,
	    sizeof(*msg) + sizeof(*state) - 1, NOTIFYCMP_MESSAGE_REPLY));
	msg->status = -ENOENT;
	ATF_CHECK_EQ(0, notifycmp_validate_message(msg, sizeof(*msg),
	    NOTIFYCMP_MESSAGE_REPLY));
	ATF_CHECK_EQ(-1, notifycmp_validate_message(msg,
	    sizeof(*msg) + sizeof(*state), NOTIFYCMP_MESSAGE_REPLY));
}
...
ATF_TC_BODY(event_classes, tc)
{
	...
	*msg = message(NOTIFYCMP_OP_NEXT);
	...
	ATF_CHECK_EQ(0, notifycmp_validate_message(msg, base,
	    NOTIFYCMP_MESSAGE_REPLY));
	event->flags = NOTIFYCMP_EVENT_F_GAP;
	ATF_CHECK_EQ(-1, notifycmp_validate_message(msg, base,
	    NOTIFYCMP_MESSAGE_REPLY));
	...
	event->publisher_length = 3;
	memcpy(event->data, "svc", 3);
	ATF_CHECK_EQ(0, notifycmp_validate_message(msg, base + 3,
	    NOTIFYCMP_MESSAGE_REPLY));
	...
	ATF_CHECK_EQ(0, notifycmp_validate_message(msg, base + 8,
	    NOTIFYCMP_MESSAGE_REPLY));
	event->generation = 0;
	ATF_CHECK_EQ(-1, notifycmp_validate_message(msg, base + 8,
	    NOTIFYCMP_MESSAGE_REPLY));
}
