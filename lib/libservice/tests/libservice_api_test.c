...
ATF_TC_WITHOUT_HEAD(installation_query_errors);
ATF_TC_BODY(installation_query_errors, tc)
{
	enum service_installation_state state = SERVICE_INSTALLATION_REMOVED;
	uint8_t generation[16] = { 1 };
	uint8_t zero[16] = { 0 };

	ATF_CHECK_ERRNO(EINVAL, service_installation_query(NULL, generation, &state) == -1);
	ATF_CHECK_EQ(SERVICE_INSTALLATION_UNKNOWN, state);
	ATF_CHECK_ERRNO(EINVAL, service_installation_query("org.test/app", zero, &state) == -1);
	ATF_CHECK_EQ(SERVICE_INSTALLATION_UNKNOWN, state);
	ATF_CHECK_EQ(-1, service_installation_query("org.test/app", generation, &state));
	ATF_CHECK_EQ(SERVICE_INSTALLATION_UNKNOWN, state);
}

struct query_capture {
	unsigned mode;
	bool got;
	char authority_path[64];
	uint8_t generation[16];
};

static void
query_capture_request(struct channel *channel __unused,
    struct channel_message *message, void *argument)
{
	struct query_capture *capture = argument;
	struct svc_installation_query_reply reply = { 0 };
	const struct svc_installation_query_req *req;
	size_t length = sizeof(reply);

	ATF_REQUIRE_EQ(sizeof(*req), channel_message_length(message));
	ATF_REQUIRE_EQ(0, channel_message_fd_count(message));
	req = channel_message_data(message);
	ATF_CHECK_EQ(SVC_OP_INSTALLATION_QUERY, req->op);
	ATF_CHECK_EQ(0, req->flags);
	ATF_CHECK_STREQ("org.test/app", req->label);
	ATF_CHECK_EQ(0, memcmp(capture->generation, req->generation, sizeof(capture->generation)));
	if (capture->authority_path[0] != '\0') {
		ATF_REQUIRE_EQ(0, svc_installation_query_reply(capture->authority_path, message));
		capture->got = true;
		channel_message_free(message);
		return;
	}
	if (capture->mode <= SERVICE_INSTALLATION_REMOVED)
		reply.state = capture->mode;
	else if (capture->mode == 5) {
		reply.status = EIO;
		length = sizeof(struct svc_reply);
	} else if (capture->mode == 6)
		length = sizeof(struct svc_reply); /* Truncated success must fail. */
	else
		reply.state = 99; /* Unknown protocol state must fail. */
	ATF_REQUIRE_EQ(0, channel_send_reply(message, &(struct channel_outgoing)
	    CHANNEL_OUTGOING_INITIALIZER(&reply, length)));
	capture->got = true;
	channel_message_free(message);
}

ATF_TC(installation_query_wire);
ATF_TC_HEAD(installation_query_wire, tc)
{
	atf_tc_set_md_var(tc, "require.user", "root");
...
static void
run_query_modes(unsigned first, unsigned limit)
{
	for (unsigned mode = first; mode < limit; mode++) {
		struct channel_options options = CHANNEL_OPTIONS_INITIALIZER(CHANNEL_ROLE_PROVIDER);
		struct query_capture capture = { .mode = mode };
		struct sl_db held_writer;
		struct channel *peer;
		int pair[2], status, events, wants_write;
		pid_t child;

		capture.generation[0] = 1;
		if (mode >= 8) {
			struct sl_db db;
			const char *op1 = "11111111111111111111111111111111";
			const char *op2 = "22222222222222222222222222222222";
			const char *op3 = "33333333333333333333333333333333";
			char state_path[128];

			snprintf(capture.authority_path, sizeof(capture.authority_path), "authority-%u", mode);
			ATF_REQUIRE_EQ(0, mkdir(capture.authority_path, 0700));
			ATF_REQUIRE_EQ(0, sl_open(capture.authority_path, &db));
			ATF_REQUIRE_EQ(0, sl_install_begin(&db, "org.test/app", "pkg.test", op1));
			memcpy(capture.generation, sl_owner(&db, "org.test/app")->generation, 16);
			if (mode != 10)
				ATF_REQUIRE_EQ(0, sl_install_finish(&db, "org.test/app", op1, false));
			if (mode == 11 || mode == 12) {
				ATF_REQUIRE_EQ(0, sl_remove_begin(&db, "org.test/app", "pkg.test", op2));
				if (mode == 12) {
					ATF_REQUIRE_EQ(0, sl_remove_finish(&db, "org.test/app", op2, false));
					ATF_REQUIRE_EQ(0, sl_install_begin(&db, "org.test/app", "pkg.test", op3));
					ATF_REQUIRE_EQ(0, sl_install_finish(&db, "org.test/app", op3, false));
				}
			}
			ATF_REQUIRE_EQ(0, sl_commit(&db));
			sl_close(&db);
			if (mode == 8)
				capture.generation[0] ^= 0x80; /* Unknown exact ID, known label. */
			snprintf(state_path, sizeof(state_path), "%s/state", capture.authority_path);
			if (mode == 13)
				ATF_REQUIRE_EQ(0, truncate(state_path, 3));
			if (mode == 14)
				ATF_REQUIRE_EQ(0, unlink(state_path));
			if (mode == 15)
				ATF_REQUIRE_EQ(0, sl_open(capture.authority_path, &held_writer));
		}
		capability_channel_pair(&pair[0], &pair[1]);
		child = fork();
		ATF_REQUIRE(child >= 0);
		if (child == 0) {
			struct service_bootstrap bootstrap;
			...
			if (setenv(SERVICE_BOOTSTRAP_ENV, "5", 1) == -1 ||
				...
				_exit(2);
			result = service_installation_query("org.test/app", capture.generation, &state);
			if (mode >= 8) {
				if (mode <= 12)
					_exit(result == 0 && (unsigned)state == mode - 8 ? 0 : 5);
				_exit(result == -1 && state == SERVICE_INSTALLATION_UNKNOWN &&
				    errno == (mode == 13 ? EINVAL : mode == 14 ? EIO : EWOULDBLOCK) ? 0 : 6);
			}
			if (mode <= SERVICE_INSTALLATION_REMOVED)
				_exit(result == 0 && (unsigned)state == mode ? 0 : 3);
			_exit(result == -1 && state == SERVICE_INSTALLATION_UNKNOWN &&
			    errno == (mode == 5 ? EIO : EPROTO) ? 0 : 4);
		}
		close(pair[0]);
		ATF_REQUIRE_EQ(0, channel_create(pair[1], &options, &peer));
		ATF_REQUIRE_EQ(0, channel_set_request_handler(peer, query_capture_request, &capture));
		for (;;) {
			wants_write = channel_wants_write(peer);
			ATF_REQUIRE(wants_write >= 0);
			if (capture.got && wants_write == 0)
				break;
			events = channel_wait(peer, wants_write, 5000);
			ATF_REQUIRE(events > 0);
			if (events & CHANNEL_WAIT_WRITE)
				ATF_REQUIRE_EQ(0, channel_flush(peer));
			if ((events & CHANNEL_WAIT_READ) && channel_dispatch(peer) == -1) {
				ATF_REQUIRE(capture.got);
				break;
			}
		}
		ATF_REQUIRE_EQ(child, waitpid(child, &status, 0));
		ATF_CHECK_MSG(WIFEXITED(status) && WEXITSTATUS(status) == 0,
		    "query mode %u child status %#x", mode, status);
		channel_destroy(peer);
		if (mode == 15)
			sl_close(&held_writer);
	}
}

ATF_TC_BODY(installation_query_wire, tc)
{
	run_query_modes(0, 8);
}

ATF_TC(installation_authority_channel);
ATF_TC_HEAD(installation_authority_channel, tc)
{
	atf_tc_set_md_var(tc, "require.user", "root");
	atf_tc_set_md_var(tc, "require.kmods", "mac_capability mac_capability_channel");
	atf_tc_set_md_var(tc, "timeout", "30");
}
ATF_TC_BODY(installation_authority_channel, tc)
{
	/* Actual manager handler and durable store, with no cleanup consumer. */
	run_query_modes(8, 16);
}

ATF_TP_ADD_TCS(tp)
{
	ATF_TP_ADD_TC(tp, installation_authority_channel);
	ATF_TP_ADD_TC(tp, installation_query_wire);
	ATF_TP_ADD_TC(tp, installation_query_errors);
