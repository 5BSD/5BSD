 ...
 * session).
 *
 * The getty path is the one hop the environment cannot cross.  oracle-init
 * (PID 1) spawns getty from /etc/ttys with a hand-built minimal environment
 * ({TERM,NULL}), so SERVICE_LOOKUP_ENV does not survive oracle-init -> getty ->
 ...
 * oracle-init dup2()s the channel there (clearing FD_CLOEXEC) just before
