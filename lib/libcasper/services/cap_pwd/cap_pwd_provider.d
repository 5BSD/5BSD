...
/*
 * USDT provider for the Casper cap_pwd service.  Each probe exposes a
 * capability-mediated decision the service made on behalf of a sandboxed
 * process: which command was permitted (allow), which command/field/user
...
provider cap_pwd {
	...
	/* A channel narrowed the set of allowed users (by name). */
	probe limit__users(const char *user);
