 ...
 *
 * Wire-contract tests for timecmp_validate_message -- the shared guard the
...
ATF_TC_BODY(bare_header_is_valid, tc)
{
	...
	ATF_CHECK_EQ(0, timecmp_validate_message(&m, sizeof(m),
...
{
	uint8_t buf[TIMECMP_MAX_MESSAGE];
	struct timecmp_msg *m = (void *)buf;

	init(m, TIMECMP_OP_SET);
	ATF_CHECK_EQ(0, timecmp_validate_message(m,
...
{
	uint8_t buf[TIMECMP_MAX_MESSAGE + 8];
	struct timecmp_msg *m = (void *)buf;

	init(m, TIMECMP_OP_GET);
	/* shorter than a header */
	ATF_CHECK_EQ(-1, timecmp_validate_message(m, sizeof(*m) - 1,
	    TIMECMP_MESSAGE_REQUEST));
	/* header + a partial body */
	ATF_CHECK_EQ(-1, timecmp_validate_message(m, sizeof(*m) + 4,
	    TIMECMP_MESSAGE_REQUEST));
	/* header + more than one body */
	ATF_CHECK_EQ(-1, timecmp_validate_message(m,
	    sizeof(*m) + 2 * sizeof(struct timecmp_time),
	    TIMECMP_MESSAGE_REQUEST));
}
