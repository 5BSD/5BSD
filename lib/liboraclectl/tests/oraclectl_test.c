...
{
	int sv[2];
	struct oraclectl_status status;
	char summary[8];

	(void)tc;
	ATF_REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
	ATF_REQUIRE_EQ(close(sv[1]), 0);
	ATF_REQUIRE_EQ(oraclectl_status(sv[0], &status, summary,
...
ATF_TC_BODY(oversized_summary_rejected, tc)
{
	...
	(void)tc;
	ATF_REQUIRE_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
	pid = spawn_reply_server(sv[1], 0, ORACLECTL_SUMMARY_MAX + 1, NULL);
	ATF_REQUIRE_EQ(close(sv[1]), 0);
	ATF_REQUIRE_EQ(oraclectl_status(sv[0], &status, summary,
...
ATF_TC_BODY(path_too_long, tc)
{
	...
	errno = 0;
	ATF_REQUIRE_EQ(oraclectl_open(path), -1);
