...
ATF_TC_WITHOUT_HEAD(retired_pool_owner_cannot_write_or_disrupt_peer);
ATF_TC_BODY(retired_pool_owner_cannot_write_or_disrupt_peer, tc)
{
	struct fixture fixture;
	...
	ATF_REQUIRE_EQ(0, logcmp_storage_session_activate(&pool));
	length = make_record(record, 11);
	ATF_REQUIRE_EQ(0, logcmp_storage_append_for(&pool, "install.old",
	    (const void *)record, length));
	ATF_REQUIRE_EQ(0, logcmp_storage_flush(&pool, LOGCMP_STORAGE_TIMEOUT_MS));
	ATF_REQUIRE_EQ(0, logcmp_storage_retire_owner(fixture.control_fd, "install.old"));
	/* A still-open old client queues another record; storage must discard it. */
	ATF_REQUIRE_EQ(0, logcmp_storage_append_for(&pool, "install.old",
	    (const void *)record, length));
	((struct logcmp_record *)(void *)record)->sequence = 22;
	ATF_REQUIRE_EQ(0, logcmp_storage_append_for(&pool, "install.new",
	    (const void *)record, length));
	ATF_REQUIRE_EQ(0, logcmp_storage_flush(&pool, LOGCMP_STORAGE_TIMEOUT_MS));
	ATF_REQUIRE_EQ(0, logcmp_storage_retire_owner(fixture.control_fd, "install.old"));
	memset(&cursor, 0, sizeof(cursor));
	ATF_CHECK_EQ(LOGCMP_STORE_QUERY_EOF, logcmp_storage_query_next_for(&pool,
	    "install.old", 0, &cursor, output, sizeof(output), &output_length,
	    LOGCMP_STORAGE_TIMEOUT_MS));
	memset(&cursor, 0, sizeof(cursor));
	ATF_REQUIRE_EQ(LOGCMP_STORE_QUERY_RECORD, logcmp_storage_query_next_for(&pool,
	    "install.new", 0, &cursor, output, sizeof(output), &output_length,
	    LOGCMP_STORAGE_TIMEOUT_MS));
	ATF_CHECK_EQ(22, ((struct logcmp_record *)(void *)output)->sequence);
	logcmp_storage_session_close(&pool);
	fixture_destroy(&fixture);
...
ATF_TP_ADD_TCS(tp)
{
	...
	ATF_TP_ADD_TC(tp, retired_pool_owner_cannot_write_or_disrupt_peer);
