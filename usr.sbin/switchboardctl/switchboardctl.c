 ...
 *
 * switchboardctl — command-line interface to switchboard(8).
 *
 * Each invocation opens a connection to switchboard's control socket,
...
/* Operator disable list; must match SWITCHBOARD_DISABLED_PATH in switchboard.h. */
 ...
 * /Capabilities/db/switchboard) so the atomic temp+rename can create the file
    ...
    char *summary, size_t sumlen, int *status_out)
{
	...
	if (rhdr->status > ELAST ||
	    rhdr->flags > SWITCHBOARD_CTL_SUMMARY_MAX ||
...
/*
 * Send a request over the capability control plane (system.switchboard) and return
 ...
 * reach switchboard's control plane.
 */
static int
 ...
 * and start verbs use (toward service(8) parity).  switchboard's stop is
 ...
 * without switchboard needing a dedicated restart op.  A unit already stopped is
...
cmd_enable_disable(const char *id, bool disable)
{
	...
	if (error != 0) {
		warnx("%s: recorded, but reload failed: %s -- run "
		    "'switchboardctl reload'", verb, strerror(error));
		return (1);
	}
	if (summary[0] != '\0')
		printf("%s", summary);
