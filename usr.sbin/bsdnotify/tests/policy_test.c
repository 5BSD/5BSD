...
ATF_TC_BODY(identity_database, tc)
{
	...
	ATF_REQUIRE_EQ(notifycmp_policy_db_parse(
	    "{clients={\"com.example/publisher\"={publish=[\"system.ready\"];};"
	    "\"com.example/subscriber\"={subscribe=[\"system.ready\"];"
	    "timers=true;};};}", &db), 0);
	policy = notifycmp_policy_db_lookup(&db, "com.example/publisher");
	ATF_REQUIRE(policy != NULL);
	ATF_CHECK(notifycmp_policy_can_publish(policy, "system.ready", 12));
	ATF_CHECK(!notifycmp_policy_can_subscribe(policy, "system.ready", 12));
	policy = notifycmp_policy_db_lookup(&db, "com.example/subscriber");
	ATF_REQUIRE(policy != NULL);
	ATF_CHECK(policy->timers);
	ATF_CHECK(notifycmp_policy_db_lookup(&db, "unknown") == NULL);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_policy_db_parse("{clients={};extra=true;}", &db) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_policy_db_parse("{clients={\"bad label\"={};};}",
	    &db) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_policy_db_parse("{clients={\"missing-separator\"={};};}",
	    &db) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_policy_db_parse("{clients={\"bad//label\"={};};}",
	    &db) == -1);
...
ATF_TC_BODY(load_rejects_unsafe_files, tc)
{
	...
	close(fd);
	ATF_REQUIRE_EQ(0, notifycmp_policy_db_load("bsdnotify.conf", &db));
	ATF_REQUIRE_EQ(0, chmod("bsdnotify.conf", 0666));
	ATF_CHECK_ERRNO(EPERM,
	    notifycmp_policy_db_load("bsdnotify.conf", &db) == -1);
	ATF_REQUIRE_EQ(0, chmod("bsdnotify.conf", 0600));
	ATF_REQUIRE_EQ(0, symlink("bsdnotify.conf", "policy-link.conf"));
	ATF_CHECK(notifycmp_policy_db_load("policy-link.conf", &db) == -1);
	ATF_CHECK(errno == ELOOP || errno == EMLINK);
	ATF_REQUIRE_EQ(0, mkdir("policy-directory", 0700));
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_policy_db_load("policy-directory", &db) == -1);
	fd = open("large.conf",
	    O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0600);
	ATF_REQUIRE(fd >= 0);
	ATF_REQUIRE_EQ(0, ftruncate(fd, NOTIFYCMP_POLICY_FILE_MAX + 1));
	close(fd);
	ATF_CHECK_ERRNO(EFBIG,
	    notifycmp_policy_db_load("large.conf", &db) == -1);
	...
	close(fd);
	ATF_CHECK_ERRNO(EINVAL,
	    notifycmp_policy_db_load("nul.conf", &db) == -1);
}
