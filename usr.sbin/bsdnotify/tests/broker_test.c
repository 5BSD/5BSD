...
ATF_TC_BODY(queue_pressure, tc)
{
	...
	ATF_REQUIRE(notifycmp_broker_next(subscriber, event,
	    sizeof(buffer)) > 0);
	ATF_CHECK_EQ(event->type, NOTIFYCMP_EVENT_GAP);
	ATF_CHECK_EQ(event->lost_count, 3);
	ATF_CHECK_EQ(event->router_epoch, notifycmp_broker_epoch(broker));
	ATF_REQUIRE(notifycmp_broker_next(subscriber, event,
	    sizeof(buffer)) > 0);
...
{
	struct notifycmp_broker_client *publisher, *slow, *healthy;
	struct notifycmp_broker *broker;
	uint8_t payload[NOTIFYCMP_MAX_PAYLOAD + 1];
	union event_buffer buffer;
	struct notifycmp_event *event;
	struct notifycmp_stats stats;
	ssize_t length;
	...
	for (i = 0; i < NOTIFYCMP_MAX_PAYLOAD; i++)
		payload[i] = (uint8_t)(i * 37U);
	broker = notifycmp_broker_create();
	event = (void *)buffer.bytes;
	ATF_REQUIRE(broker != NULL);
	publisher = notifycmp_broker_add(broker, "binary.publisher", 2);
	slow = notifycmp_broker_add(broker, "slow.subscriber", 1);
	healthy = notifycmp_broker_add(broker, "healthy.subscriber", 4);
	ATF_REQUIRE(publisher != NULL && slow != NULL && healthy != NULL);
	ATF_REQUIRE_EQ(0, notifycmp_broker_subscribe(broker, slow,
	    "binary.data", 11));
	ATF_REQUIRE_EQ(0, notifycmp_broker_subscribe(broker, healthy,
	    "binary.data", 11));

	ATF_REQUIRE_EQ(0, notifycmp_broker_publish(broker, publisher,
	    "binary.data", 11, payload, NOTIFYCMP_MAX_PAYLOAD));
	ATF_REQUIRE_EQ(0, notifycmp_broker_publish(broker, publisher,
	    "binary.data", 11, "second", 6));
	notifycmp_broker_stats(slow, &stats);
	ATF_CHECK_EQ(1, stats.delivered);
	ATF_CHECK_EQ(1, stats.dropped);
	notifycmp_broker_stats(healthy, &stats);
	...
	length = notifycmp_broker_next(healthy, event, sizeof(buffer));
	ATF_REQUIRE(length > 0);
	ATF_CHECK_EQ(NOTIFYCMP_MAX_PAYLOAD, event->payload_length);
	ATF_CHECK_EQ(0, memcmp(event_payload(event), payload,
	    NOTIFYCMP_MAX_PAYLOAD));
	ATF_REQUIRE(notifycmp_broker_next(healthy, event, sizeof(buffer)) > 0);
	...
	ATF_REQUIRE(notifycmp_broker_next(slow, event, sizeof(buffer)) > 0);
	ATF_CHECK_EQ(NOTIFYCMP_EVENT_GAP, event->type);
	ATF_CHECK_EQ(1, event->lost_count);
	length = notifycmp_broker_next(slow, event, sizeof(buffer));
	ATF_REQUIRE(length > 0);
	ATF_CHECK_EQ(NOTIFYCMP_MAX_PAYLOAD, event->payload_length);
	ATF_CHECK_EQ(0, memcmp(event_payload(event), payload,
	    NOTIFYCMP_MAX_PAYLOAD));

	ATF_CHECK_ERRNO(EINVAL, notifycmp_broker_publish(broker, publisher,
...
ATF_TC_BODY(state_and_generation, tc)
{
	...
	broker = notifycmp_broker_create();
	ATF_REQUIRE(broker != NULL);
	ATF_REQUIRE(notifycmp_broker_epoch(broker) != 0);
	publisher = notifycmp_broker_add(broker, "publisher", 4);
	subscriber = notifycmp_broker_add(broker, "subscriber", 4);
	ATF_REQUIRE(publisher != NULL && subscriber != NULL);
	ATF_REQUIRE_EQ(notifycmp_broker_subscribe(broker, subscriber,
	    "system.state", 12), 0);
	ATF_REQUIRE_EQ(notifycmp_broker_state_set(broker, publisher,
	    "system.state", 12, 41, &state), 0);
	ATF_CHECK_EQ(state.router_epoch, notifycmp_broker_epoch(broker));
	ATF_CHECK_EQ(state.generation, 1);
	ATF_CHECK_EQ(state.state, 41);
	ATF_REQUIRE_EQ(notifycmp_broker_state_set(broker, publisher,
	    "system.state", 12, 42, &state), 0);
	ATF_CHECK_EQ(state.generation, 2);
	ATF_REQUIRE_EQ(notifycmp_broker_state_get(broker, "system.state", 12,
	    &state), 0);
	ATF_CHECK_EQ(state.generation, 2);
	...
	ATF_REQUIRE(notifycmp_broker_next(subscriber, event,
	    sizeof(buffer)) > 0);
	ATF_CHECK_EQ(event->type, NOTIFYCMP_EVENT_STATE);
	ATF_CHECK_EQ(event->generation, 1);
	ATF_CHECK_EQ(event->state, 41);
	ATF_CHECK_ERRNO(ENOENT, notifycmp_broker_state_get(broker, "missing",
	    7, &state) == -1);
	notifycmp_broker_destroy(broker);
}
...
ATF_TC_BODY(state_bound, tc)
{
	...
	broker = notifycmp_broker_create();
	ATF_REQUIRE(broker != NULL);
	publisher = notifycmp_broker_add(broker, "publisher", 1);
	ATF_REQUIRE(publisher != NULL);
	for (i = 0; i < NOTIFYCMP_MAX_STATES; i++) {
		snprintf(topic, sizeof(topic), "state.t%u", i);
		ATF_REQUIRE_EQ(notifycmp_broker_state_set(broker, publisher,
		    topic, strlen(topic), i, &state), 0);
	}
	ATF_CHECK_ERRNO(ENOSPC, notifycmp_broker_state_set(broker, publisher,
