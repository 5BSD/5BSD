...
#
# Tests for oracled's PID 1 lifecycle control operations
...
SOCK=/var/run/oracled.sock

# Skip when the live daemon is PID 1 (oracle-init): sending an accepted
# lifecycle op there would reboot the test machine.
require_daemon_not_pid1()
{
	if [ ! -S "$SOCK" ]; then
		atf_skip "oracled control socket not present"
	fi
	if [ "$(ps -o comm= -p 1 2>/dev/null)" = "oracle-init" ]; then
		atf_skip "oracled is PID 1; lifecycle ops would reboot the host"
	fi
}
...
lifecycle_rejects_payload_body()
{
	...
	# so this is safe even against a PID 1 oracle-init.  EINVAL = 22 (16).
	reply=$(send_op 4 4)
	[ "$reply" = "16" ] ||
