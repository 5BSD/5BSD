 ...
 *
 * MAC_CAPABILITY claim primitives for oracled.
 *
...
int
mac_capability_claim_jail(const struct oracled_jail_claim *jc)
{
...
int
mac_capability_release_jail(const struct oracled_jail_claim *jc)
{
	struct fi_jail_request req;
	struct fi_reply reply;
	char jailbuf[ORACLED_JAIL_DESC_MAX];
...
isolate_resources(void)
{
	...
	/* Always claim /dev/mac_capability — oracled owns this device. */
