...
fail_at(const char *stage)
{
	...
	requested = getenv("CAPSULECTL_FAIL");
...
service_session_close(struct service_session *session)
{
	if (session == &fake_session && session->fd >= 0) {
		...
		session->fd = -1;
		if (getenv("CAPSULECTL_TRACE_CLOSE") != NULL)
    ...
    const struct service_call_options *options)
{
	...
	mode = getenv("CAPSULECTL_EXPECT_OP");
	if (mode != NULL) {
		errno = 0;
	...
	summary = "capsule-ready\n";
	summary_len = strlen(summary);
	mode = getenv("CAPSULECTL_REPLY");
