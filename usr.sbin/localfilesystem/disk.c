...
scan_directory(int fd, uint64_t maximum_bytes, uint64_t maximum_objects,
    uint32_t maximum_file_bytes, uint64_t *total_bytes,
    uint64_t *total_objects)
{
	...
	while ((entry = readdir(directory)) != NULL) {
		...
		if (S_ISREG(status.st_mode)) {
			...
			if (status.st_size < 0 ||
			    (uint64_t)status.st_size > maximum_file_bytes) {
				errno = EFBIG;
				goto fail;
			}
			...
			*total_bytes += (uint64_t)status.st_size;
			...
			if (scan_directory(child, maximum_bytes, maximum_objects,
			    maximum_file_bytes, total_bytes, total_objects) == -1) {
				error = errno;
				close(child);
				errno = error;
				goto fail;
...
static int
validate_status(const struct disk_store *store, const struct stat *status,
    uint32_t expected_type)
{

	if ((expected_type == FILESYSTEMCMP_TYPE_DIRECTORY &&
	    !S_ISDIR(status->st_mode)) ||
	    (expected_type == FILESYSTEMCMP_TYPE_REGULAR &&
	    !S_ISREG(status->st_mode)) ||
	    (expected_type == 0 && !S_ISREG(status->st_mode) &&
	    !S_ISDIR(status->st_mode))) {
		errno = EFTYPE;
		return (-1);
	}
	if (S_ISREG(status->st_mode)) {
		if (status->st_size < 0 ||
		    (uint64_t)status->st_size > store->max_file_bytes) {
			errno = EFBIG;
			return (-1);
		}
		if (!store->readonly && status->st_nlink != 1) {
			errno = EMLINK;
			return (-1);
		}
	}
    ...
    struct stat *status)
{
	...
	if (validate_status(store, status, 0) == -1) {
		int error;
    ...
    uint32_t max_objects, uint32_t max_file_bytes, struct disk_store **storep)
{
	...
	/* Use an independent open file description so releasing the store also
	 * releases its lease even when the bootstrap root remains open. */
	fd = openat(rootfd, ".", O_RDONLY | O_DIRECTORY | O_CLOEXEC);
	if (fd == -1)
		return (-1);
	/*
	 * Quota reconstruction and mutation are intentionally single-writer.
	 * Keep the advisory lease on the root description for the lifetime of
	 * the store.  This makes the label-scoped persistence invariant hold
	 * even if a future serviced configuration accidentally overlaps two
	 * component instances, or an administrator starts one by hand.
	 */
	if (!readonly && flock(fd, LOCK_EX | LOCK_NB) == -1) {
		int error;

		error = errno == EWOULDBLOCK ? EBUSY : errno;
		close(fd);
		errno = error;
		return (-1);
	}
	...
	if (!readonly && scan_directory(fd, max_bytes, max_objects,
	    max_file_bytes, &store->bytes, &store->nobjects) == -1) {
		int error;
    ...
    struct filesystemcmp_handle *handle)
{
	...
	if (fstat(fd, &opened) == -1 ||
	    validate_status(store, &opened, type) == -1) {
		error = errno;
		close(fd);
		if (created) {
			unlink_flags = type == FILESYSTEMCMP_TYPE_DIRECTORY ?
    ...
    uint64_t offset, const void *buffer, size_t length)
{
	...
	if (before.st_nlink != 1) {
		errno = before.st_nlink == 0 ? ESTALE : EMLINK;
		return (-1);
	}
	if (before.st_size < 0 ||
	    (uint64_t)before.st_size > store->max_file_bytes) {
		errno = EFBIG;
		return (-1);
	}
	end = offset + length;
    ...
    const void *new_name, size_t new_length)
{
	...
	if (fstatat(old_dir->fd, old_component, &old_status,
	    AT_SYMLINK_NOFOLLOW) == -1)
		return (-1);
	if (!S_ISREG(old_status.st_mode) && !S_ISDIR(old_status.st_mode)) {
		errno = EFTYPE;
		return (-1);
	}
	if (S_ISREG(old_status.st_mode) && old_status.st_nlink != 1) {
		errno = EMLINK;
		return (-1);
	}
	if (S_ISREG(old_status.st_mode) && (old_status.st_size < 0 ||
	    (uint64_t)old_status.st_size > store->max_file_bytes)) {
		errno = EFBIG;
		return (-1);
	}
