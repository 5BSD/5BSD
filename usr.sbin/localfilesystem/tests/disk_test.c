...
limit_persistent_root(int fd)
{
	...
	cap_rights_init(&rights, CAP_READ, CAP_WRITE, CAP_PREAD, CAP_PWRITE,
	    CAP_SEEK, CAP_FCNTL, CAP_LOOKUP, CAP_FSTAT, CAP_FSTATAT, CAP_FLOCK,
...
{
	atf_tc_set_md_var(tc, "descr",
	    "restart and live hard links cannot alias writable authority");
}

ATF_TC(oversized_file_restart_security);
ATF_TC_HEAD(oversized_file_restart_security, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "restart reconstruction enforces the per-file size ceiling");
}
...
ATF_TC(exclusive_persistent_lease);
ATF_TC_HEAD(exclusive_persistent_lease, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "only one writable store may reconstruct and mutate a persistent namespace");
}
ATF_TC_BODY(exclusive_persistent_lease, tc)
{
	struct disk_store *first, *second;
	char template[] = "/tmp/filesystemcmp.lease.XXXXXX";
	char *path;
	int firstfd, secondfd;

	path = mkdtemp(template);
	ATF_REQUIRE(path != NULL);
	firstfd = open(path, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
	ATF_REQUIRE(firstfd >= 0);
	secondfd = open(path, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
	ATF_REQUIRE(secondfd >= 0);
	ATF_REQUIRE_EQ(0, disk_store_create(firstfd, false, 4096, 8, 1024,
	    &first));
	errno = 0;
	ATF_CHECK_EQ(-1, disk_store_create(secondfd, false, 4096, 8, 1024,
	    &second));
	ATF_CHECK_EQ(EBUSY, errno);
	disk_store_destroy(first);
	ATF_REQUIRE_EQ(0, disk_store_create(secondfd, false, 4096, 8, 1024,
	    &second));
	disk_store_destroy(second);
	close(secondfd);
	close(firstfd);
...
ATF_TC_BODY(hardlink_quota_security, tc)
{
	...
	errno = 0;
	ATF_CHECK_EQ(-1, disk_store_create(rootfd, false, 16, 8, 16, &store));
	...
	ATF_REQUIRE_EQ(0, disk_root(store, &root));
	ATF_REQUIRE_EQ(0, disk_lookup(store, root, "first", 5, &object));
	ATF_REQUIRE_EQ(0, linkat(rootfd, "first", rootfd, "second", 0));
	errno = 0;
	ATF_CHECK_EQ(-1, disk_write(store, object, 0, "x", 1));
	ATF_CHECK_EQ(EMLINK, errno);
	errno = 0;
	ATF_CHECK_EQ(-1, disk_open(store, object,
	    FILESYSTEMCMP_OPEN_TRUNCATE));
	ATF_CHECK_EQ(EMLINK, errno);
	errno = 0;
	ATF_CHECK_EQ(-1, disk_create(store, root, "first", 5, 0, 0600,
	    &object));
	ATF_CHECK_EQ(EMLINK, errno);
	errno = 0;
	ATF_CHECK_EQ(-1, disk_rename(store, root, "first", 5, root,
	    "renamed", 7));
	ATF_CHECK_EQ(EMLINK, errno);
	errno = 0;
	ATF_CHECK_EQ(-1, disk_unlink(store, root, "second", 6));
	ATF_CHECK_EQ(EMLINK, errno);
	fd = openat(rootfd, "second", O_RDONLY | O_CLOEXEC);
	ATF_REQUIRE(fd >= 0);
	memset(data, 0, sizeof(data));
	ATF_REQUIRE_EQ(4, read(fd, data, sizeof(data)));
	ATF_CHECK_EQ(0, memcmp(data, "data", 4));
	close(fd);
	disk_store_destroy(store);
	ATF_REQUIRE_EQ(0, unlinkat(rootfd, "second", 0));
...
ATF_TC_BODY(oversized_file_restart_security, tc)
{
	struct disk_store *store;
	char template[] = "/tmp/filesystemcmp.filescan.XXXXXX";
	char *path;
	int fd, rootfd;
	...
	rootfd = open(path, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
	ATF_REQUIRE(rootfd >= 0);
	fd = openat(rootfd, "oversized", O_WRONLY | O_CREAT | O_EXCL, 0600);
	ATF_REQUIRE(fd >= 0);
	ATF_REQUIRE_EQ(9, write(fd, "123456789", 9));
	close(fd);
	errno = 0;
	ATF_CHECK_EQ(-1, disk_store_create(rootfd, false, 16, 8, 8, &store));
	ATF_CHECK_EQ(EFBIG, errno);
	ATF_REQUIRE_EQ(0, unlinkat(rootfd, "oversized", 0));
	close(rootfd);
	ATF_REQUIRE_EQ(0, rmdir(path));
}
