 ...
 *
 * Claim validation helpers.  Used by capsule_proto.c (runtime minting)
 * to check whether a requested resource is covered by Capsule's
 * claimed set.
 *
...
static inline bool
claim_net_covered(const struct capsule_config *cfg,
    const struct ort_net_claim *req)
{
	unsigned i;
...
static inline bool
claim_system_covered(const struct capsule_config *cfg, const char *gate_name)
{
	int bit;
