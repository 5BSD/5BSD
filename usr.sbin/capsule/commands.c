 ...
 */
void
config_apply_claims(const struct capsule_config *newcfg)
{
...
cmd_shutdown(uid_t euid, struct ctl_reply *reply)
{
	...
	/*
	 * CTL_OP_SHUTDOWN stops the capsule daemon.  When capsule is
	...
	if (getpid() == 1) {
		reply->status = EPERM;
		syslog(LOG_WARNING,
		    "control: shutdown rejected: capsule is PID 1");
		CAPSULE_PROBE_CTL_DENY(CTL_OP_SHUTDOWN, euid);
		return (0);
	}
	if (euid != 0) {
		reply->status = EPERM;
 ...
 * only when capsule is PID 1; an ordinary daemon has no authority to
...
cmd_lifecycle(uid_t euid, uint32_t op, struct ctl_reply *reply)
{
	...
	if (getpid() != 1) {
		...
		CAPSULE_PROBE_CTL_DENY(op, euid);
		return (0);
	}
	if (euid != 0) {
		reply->status = EPERM;
    ...
    char *summary, size_t sumlen)
{
	...
	if (euid != 0) {
		...
		CAPSULE_PROBE_CTL_DENY(CTL_OP_RELOAD, euid);
		return;
	}

	syslog(LOG_INFO, "control: reload uid %u", euid);
	CAPSULE_PROBE_RELOAD();
