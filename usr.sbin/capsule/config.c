 ...
 *
 * UCL configuration file parser for capsule.
 *
 * Reads /etc/capsule.conf (or a path given with -f) and populates
 * struct capsule_config.  Missing or empty file is not an error —
...
void
config_init_defaults(struct capsule_config *cfg)
{

	memset(cfg, 0, sizeof(*cfg));
	strlcpy(cfg->pidfile, CAPSULE_DEFAULT_PIDFILE,
	    sizeof(cfg->pidfile));
	strlcpy(cfg->control_socket, CAPSULE_CTL_SOCK,
	    sizeof(cfg->control_socket));
	cfg->control_socket_mode = CAPSULE_DEFAULT_CTLMODE;

	strlcpy(cfg->service_manager, CAPSULE_DEFAULT_SVC_MANAGER,
	    sizeof(cfg->service_manager));

	/* rc(8) administers Capsule through its root-only control socket. */
	cfg->integrity_flags = CP_SF_PTRACE | CP_SF_WAIT | CP_SF_SCHED |
	    CP_SF_KTRACE | CAPSULE_REQUIRED_INTEGRITY_FLAGS;
}
...
static void
cfg_mode(const ucl_object_t *root, struct capsule_config *cfg)
{
	const ucl_object_t *o;
	long val;
	char *endp;
	...
	if (ucl_object_type(o) == UCL_STRING) {
		...
		if (endp == ms || *endp != '\0' || errno == ERANGE ||
		    val < 0 || val > 07777) {
			fprintf(stderr, "capsule: invalid "
		 ...
		 */
		fprintf(stderr, "capsule: control_socket_mode must be "
	...
	if ((val & 07) != 0)
		fprintf(stderr, "capsule: warning: control_socket_mode "
...
static void
cfg_integrity(const ucl_object_t *root, struct capsule_config *cfg)
{
	...
	for (i = 0; i < nitems(integrity_cfg_map); i++) {
		...
		if (o != NULL && ucl_object_type(o) == UCL_BOOLEAN) {
			if (ucl_object_toboolean(o))
			    ...
			    CAPSULE_REQUIRED_INTEGRITY_FLAGS) != 0) {
				fprintf(stderr, "capsule: integrity.%s=false "
...
static void
cfg_claims(const ucl_object_t *root, struct capsule_config *cfg)
{
	...
	if (arr != NULL && ucl_object_type(arr) == UCL_ARRAY) {
		...
		while ((elem = ucl_object_iterate(arr, &it, true))
			...
			if (ucl_object_type(elem) != UCL_OBJECT)
				continue;
			if (cfg->nclaim_net >= CAPSULE_MAX_NET_CLAIMS) {
				fprintf(stderr, "capsule: too many "
				    "network claims (max %d)\n",
				    CAPSULE_MAX_NET_CLAIMS);
				break;
			}
	...
	if (arr != NULL && ucl_object_type(arr) == UCL_ARRAY) {
		...
		while ((elem = ucl_object_iterate(arr, &it, true))
			...
			if (!found)
				fprintf(stderr, "capsule: unknown "
				    "system gate: %s\n", s);
		}
	}
...
int
config_load(struct capsule_config *cfg, const char *path)
{
	...
	if (!S_ISREG(csb.st_mode)) {
		fprintf(stderr, "capsule: %s: not a regular file\n", path);
		return (-1);
	}
	if (csb.st_size > CONFIG_MAX_UCL_SIZE) {
		fprintf(stderr, "capsule: %s: file too large (%jd bytes)\n",
	...
	if (parser == NULL) {
		fprintf(stderr, "capsule: ucl_parser_new failed\n");
		return (-1);
	}
	...
	cfg->integrity_flags |= CAPSULE_REQUIRED_INTEGRITY_FLAGS;
	cfg_claims(root, cfg);
...
void
config_log(const struct capsule_config *cfg)
{
