 ...
 *
 * Control socket for capsule.
 *
 * Provides a unix domain socket at /var/run/capsule.sock for
...
conn_destroy(struct ctl_conn *c)
{
	...
	nconns--;
	CAPSULE_PROBE_CONN_COUNT(nconns);
	free(c);
}
...
ctl_conn_event(struct kevent *kev)
{
	...
	if (c->state == CTL_CONN_DONE) {
		...
		CAPSULE_PROBE_CTL_CMD_DONE(c->req.op, c->euid,
		    c->reply.status, dur);
		action = c->action;
		/*
