...
ATF_TC_BODY(service_request_validation, tc)
{
	static const char *const valid[] = { "mount", "node", "accounting",
	    "identity" };
	struct capsule_service_req req;
	const struct capsule_service_req *validated;
	unsigned i;
	int error;

	for (i = 0; i < nitems(valid); i++) {
		memset(&req, 0, sizeof(req));
		req.op = CAPSULE_OP_DELEGATE_SERVICE;
