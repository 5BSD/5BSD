...
#
# Build a clean source-tree VM image and verify that it boots with Capsule as
...
boot_test="$src/usr.sbin/capsule/tests/run_capsule_vm_boot_test.sh"
...
workdir=$(mktemp -d /tmp/capsule-pid1-vm.XXXXXX) || fail "mktemp failed"
case "$workdir" in
/tmp/capsule-pid1-vm.*) ;;
...
echo "Building clean pkgbase Capsule PID 1 VM image in $workdir"
