 ...
 *
 * Dynamic claim/release handlers for the Capsule channel protocol.
 *
...
auto_claim_net(const struct ort_net_claim *nc, int *errp)
{
	...
	if (od.cfg.nclaim_net >= CAPSULE_MAX_NET_CLAIMS) {
		*errp = ENOSPC;
		return (-1);
	}
	...
	syslog(LOG_INFO, "capsule_proto: auto-claimed net %u-%u/%d",
	    nc->port_min, nc->port_max, nc->protocol);
	CAPSULE_PROBE_DYN_CLAIM_NET(nc->port_min, nc->port_max,
	    nc->protocol, 0);
	return (0);
}
...
auto_claim_vsock(const struct ort_vsock_claim *vc, int *errp)
{
	...
	if (od.cfg.nclaim_vsock >= CAPSULE_MAX_VSOCK_CLAIMS) {
		*errp = ENOSPC;
		return (-1);
	}
	...
	CAPSULE_PROBE_DYN_CLAIM_VSOCK(vc->cid, vc->port_min, vc->port_max, 0);
	return (0);
}
...
auto_claim_system(uint32_t gates, int *errp)
{
	...
	if (new_bits != 0) {
		syslog(LOG_INFO,
		    "capsule_proto: auto-claimed system gates 0x%x",
		    new_bits);
		CAPSULE_PROBE_DYN_CLAIM_SYSTEM(gates, 0);
	}
	return (0);
}
...
release_auto_claim_net(const struct ort_net_claim *nc)
{
	...
	CAPSULE_PROBE_DYN_RELEASE_NET(nc->port_min, nc->port_max,
...
release_auto_claim_vsock(const struct ort_vsock_claim *vc)
{
	...
	CAPSULE_PROBE_DYN_RELEASE_VSOCK(vc->cid, vc->port_min, vc->port_max,
	    new_refcount, 0);
}
...
handle_release_net(const void *payload, uint32_t len, uint64_t reply_token)
{
	...
	if (od.cfg.claim_net_refcount[idx] == 0) {
		syslog(LOG_WARNING,
		    "capsule_proto: release_net refcount already 0");
		CAPSULE_PROBE_DYN_RELEASE_NET(nc.port_min, nc.port_max,
	...
	od.cfg.claim_net_refcount[idx]--;
	{
		...
		if (new_refcount == 0) {
			mac_capability_release_net(&od.cfg.claim_net[idx]);
			syslog(LOG_INFO,
			    "capsule_proto: released dynamic net claim %u-%u/%d",
			    nc.port_min, nc.port_max, nc.protocol);
			remove_net_claim((unsigned)idx);
		}
...
handle_release_system(const void *payload, uint32_t len, uint64_t reply_token)
{
	 ...
	 * Capsule owns on a dedicated connection, refcounted separately from
	...
	if ((gates & SYS_GATE_SYSCTL) != 0 &&
		...
		if (gates == 0) {
			CAPSULE_PROBE_DYN_RELEASE_SYSTEM(req->gates,
			    SYS_GATE_SYSCTL, 0);
			proto_reply(0, reply_token, NULL, 0);
			return;
		}
	...
	if (gates & od.cfg.claim_system_policy) {
		syslog(LOG_NOTICE,
		    "capsule_proto: release_system denied (manifest): 0x%x",
		    req->gates);
		CAPSULE_PROBE_DYN_RELEASE_SYSTEM(req->gates, 0, EPERM);
		proto_reply(EPERM, reply_token, NULL, 0);
		return;
	}

	if ((gates & od.cfg.claim_system_service) != gates) {
		CAPSULE_PROBE_DYN_RELEASE_SYSTEM(req->gates, 0, ENOENT);
		proto_reply(ENOENT, reply_token, NULL, 0);
		return;
	}
...
sweep_dynamic_claims(void)
{
	...
	for (i = od.cfg.nclaim_net; i > 0; i--) {
		if (od.cfg.claim_net_source[i - 1] == CLAIM_SOURCE_SERVICE) {
			...
			syslog(LOG_INFO,
			    "capsule_proto: sweep released net claim");
