...
#include "probes.h"
#include "capsule_proto_claims.h"
...
proto_reply(int status, uint64_t reply_token, int *fds, int nfds)
{
	...
	if (status == 0 && nfds > 0) {
		for (i = 0; i < nfds; i++) {
			if (cap_xfer_limit(fds[i], CAP_XFER_ONCE) == -1) {
				syslog(LOG_ERR,
				    "capsule_proto: confine reply fd: %m");
				status = EIO;
				fds = NULL;
				nfds = 0;
				break;
	...
	if (ioctl(proto_channel_fd, MAC_CAPABILITY_SENDMSG, &sa) == -1) {
		syslog(LOG_WARNING, "capsule_proto: reply: %m");
		return (-1);
	}
	dispatch_status = status;
	CAPSULE_PROBE_IPC_REPLY(dispatch_op, status);
	return (0);
}

/*
 * Validate that a path is within Capsule's claimed set.
 */
...
handle_mint_net(const void *payload, uint32_t len, uint64_t reply_token)
{
	...
	if (auto_claim_net(&nc, &err) != 0 &&
	    !net_is_claimed(&nc)) {
		CAPSULE_PROBE_MINT_NET(nc.port_min, nc.port_max,
		    nc.protocol, err);
		proto_reply(err, reply_token, NULL, 0);
		return;
	}
	...
	if (token_fd == -1) {
		...
		CAPSULE_PROBE_MINT_NET(nc.port_min, nc.port_max, nc.protocol,
		    EIO);
		proto_reply(EIO, reply_token, NULL, 0);
		return;
	}

	CAPSULE_PROBE_MINT_NET(nc.port_min, nc.port_max, nc.protocol, 0);
...
handle_mint_vsock(const void *payload, uint32_t len, uint64_t reply_token)
{
	...
	if (!validate_vsock_req(payload, len, &vc, &err)) {
		CAPSULE_PROBE_MINT_VSOCK(0, 0, 0, err);
		proto_reply(err, reply_token, NULL, 0);
		return;
	}
	if (auto_claim_vsock(&vc, &err) != 0) {
		CAPSULE_PROBE_MINT_VSOCK(vc.cid, vc.port_min, vc.port_max, err);
		proto_reply(err, reply_token, NULL, 0);
		return;
	}
	...
	CAPSULE_PROBE_MINT_VSOCK(vc.cid, vc.port_min, vc.port_max, 0);
	proto_reply(0, reply_token, &token_fd, 1);
	close(token_fd);
}
...
handle_mint_system(const void *payload, uint32_t len, uint64_t reply_token)
{
	 ...
	 * token; Capsule relays the bytes into a scoped kernel claim under
	...
	if (oidset_len > 0) {
		...
		if (mac_capability_claim_system_sysctl(oidset, oidset_len) != 0) {
			CAPSULE_PROBE_MINT_SYSTEM(req->gates, EIO);
			proto_reply(EIO, reply_token, NULL, 0);
			return;
		}
		...
		CAPSULE_PROBE_MINT_SYSTEM(req->gates, 0);
		proto_reply(0, reply_token, &token_fd, 1);
		close(token_fd);
		return;
	}
	...
	if (auto_claim_system(req->gates, &err) != 0) {
		CAPSULE_PROBE_MINT_SYSTEM(req->gates, err);
		proto_reply(err, reply_token, NULL, 0);
		return;
	}
...
handle_create_channel(uint64_t reply_token)
{
	...
	if (mac_capability_create_channel(&fds[0], &fds[1]) == -1) {
		CAPSULE_PROBE_CHANNEL_CREATE(EIO);
		proto_reply(EIO, reply_token, NULL, 0);
		return;
	}

	CAPSULE_PROBE_CHANNEL_CREATE(0);
...
handle_create_coalition(uint64_t reply_token)
{
	...
	if (cfd == -1) {
		CAPSULE_PROBE_COALITION_CREATE(EIO);
		proto_reply(EIO, reply_token, NULL, 0);
		return;
	}

	CAPSULE_PROBE_COALITION_CREATE(0);
...
{
	const struct capsule_lifecycle_req *req;
	int error;
	...
	if (error == 0)
		syslog(LOG_NOTICE,
		    "capsule_proto: capability lifecycle op %u accepted",
		    req->lifecycle_op);
...
handle_set_ambient_lookup(uint32_t len, int fd, uint64_t reply_token)
{
	...
	if (len != sizeof(struct capsule_req_hdr) || fd < 0) {
		if (fd >= 0)
			(void)close(fd);
		proto_reply(EINVAL, reply_token, NULL, 0);
		return;
	...
	if (capsule_set_ambient_lookup(fd) == -1) {
		...
		syslog(LOG_NOTICE,
		    "capsule_proto: ambient lookup install failed: %m");
		proto_reply(errno, reply_token, NULL, 0);
		return;
	}
	syslog(LOG_INFO, "capsule_proto: ambient lookup channel installed for "
	    "interactive logins");
	proto_reply(0, reply_token, NULL, 0);
}
...
{
	const struct capsule_service_req *req;
	int error, fd;

	if (!validate_service_req(payload, len, &req, &error)) {
		CAPSULE_PROBE_SERVICE_DELEGATE("", error);
		proto_reply(error, reply_token, NULL, 0);
		return;
	}
	...
	if (fd == -1) {
		error = errno;
		CAPSULE_PROBE_SERVICE_DELEGATE(req->name, error);
		proto_reply(error, reply_token, NULL, 0);
		return;
	}
	CAPSULE_PROBE_SERVICE_DELEGATE(req->name, 0);
	proto_reply(0, reply_token, &fd, 1);
	close(fd);
}

/* Claim/release handlers and sweep are in capsule_proto_claims.c. */
...
proto_dispatch_one(void)
{
	...
	union {
		struct capsule_net_req net;
		struct capsule_vsock_req vsock;
		struct capsule_system_req system;
		struct capsule_service_req service;
		struct capsule_req_hdr hdr;
		/*
		 ...
		 */
		uint8_t		raw[CAPSULE_MINT_SYSTEM_REQ_MAX];
	} buf;
	uint32_t op;
	int recv_fd = -1;
	 ...
	 * CAPSULE_OP_SET_AMBIENT_LOOKUP (§21) sends one; any stray fd on another
	...
	if (ioctl(proto_channel_fd, MAC_CAPABILITY_RECVMSG, &ra) == -1) {
		if (errno == EAGAIN)
			return (0);
		syslog(LOG_WARNING, "capsule_proto: recvmsg: %m");
		return (-1);
	}

	if (ra.payload_len < sizeof(uint32_t)) {
		syslog(LOG_WARNING, "capsule_proto: short message (%u bytes)",
		    ra.payload_len);
		if (recv_fd >= 0)
			(void)close(recv_fd);
		return (0);
	...
	CAPSULE_PROBE_IPC_DISPATCH_DONE(op, dispatch_status, dur);
	}
