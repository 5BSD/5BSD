 ...
 *
 * MAC_CAPABILITY core lifecycle for capsule.
 *
...
/*
 * Freeze a Capsule-only capability into this process.  The mechanism lives
 * in libcapability (capability_confine_fd); capsule keeps the named log.
 */
int
mac_capability_confine_capsule_fd(int fd, const char *name)
{
...
/*
 * Connect to a named mac_capability service on capsule's device fd.
 */
int
 ...
 * All subsystems are required — capsule must not start without
...
mac_capability_setup(void)
{
	...
	/*
	 * None of Capsule's root authority, claims, or self-protection may be
	 ...
	 */
	if (mac_capability_confine_capsule_fd(mac_capability_fd,
	    "mac_capability control") == -1 ||
	    mac_capability_confine_capsule_fd(mac_capability_isolation_fd,
	    "isolation") == -1 ||
	    mac_capability_confine_capsule_fd(mac_capability_system_fd,
	    "system") == -1 ||
	    mac_capability_confine_capsule_fd(mac_capability_capprotect_fd,
	    "capprotect") == -1)
		goto fail;
