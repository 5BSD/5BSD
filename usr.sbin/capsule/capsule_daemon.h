...
#ifndef CAPSULE_DAEMON_H
#define CAPSULE_DAEMON_H
...
extern struct capsule_state od;
...
int	mac_capability_create_channel(int *capsule_end, int *child_end);
...
int	mac_capability_reload_claims(const struct capsule_config *newcfg);
...
/* capsule_proto.c — channel protocol handler */
void	capsule_proto_init(int channel_fd);
int	capsule_proto_dispatch(void);
void	capsule_proto_reset(void);
bool	capsule_proto_is_ready(void);
int	capsule_proto_fd(void);
