 ...
 *
 * capsule — Capsule capability-plane root supervisor.
 *
 * Startup lifecycle:
...
usage(void)
{
	...
	fprintf(stderr,
	    "usage: capsule [-dT] [-b bootstrap] [-f conffile] "
	    "[-p pidfile]\n");
	exit(1);
}
...
main(int argc, char *argv[])
{
	...
	conffile = CAPSULE_DEFAULT_CONFFILE;
	...
	openlog("capsule", LOG_PID | (od.foreground ? LOG_PERROR : 0),
	    LOG_DAEMON);
