...
- The path is resolved using the calling process's credentials
  (capsule runs as root, so all paths are accessible)
- Path resolution is subject to symlink races, but claiming is
  idempotent — re-claiming the same vnode is a no-op
- The path string comes from capsule's config, not from untrusted
  input
...
For `capsule.conf` claims, the same syntax applies:
...
2. Add `mac_capability_claim_path_str()` to capsule's `mac_capability_claims.c` that
