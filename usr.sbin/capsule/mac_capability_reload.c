 ...
 *
 * MAC_CAPABILITY reload orchestration for capsule.
 *
 ...
 */
int
mac_capability_reload_claims(const struct capsule_config *newcfg)
{
	const struct capsule_config *oldcfg;
	unsigned i;
	unsigned nacquire, nrelease;
	int acquired, released, failed;
	bool net_ok[CAPSULE_MAX_NET_CLAIMS];
	...
	if (newcfg->claim_system != oldcfg->claim_system &&
		...
		nrelease++;
	CAPSULE_PROBE_RELOAD_CLAIMS_START(nacquire, nrelease);
	 ...
	 */
	{
		 ...
		 */
		eff = __DECONST(struct capsule_config *, newcfg);
...
#undef CARRY_FORWARD
	...
	CAPSULE_PROBE_RELOAD_CLAIMS_DONE(acquired, released, failed);
	return (failed > 0 ? -1 : 0);
}
