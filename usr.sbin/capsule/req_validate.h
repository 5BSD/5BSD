 ...
 *
 * Shared request validation helpers.  Used by capsule_proto.c
 * (mint handlers) and capsule_proto_claims.c (claim/release
...
validate_service_req(const void *payload, uint32_t len,
    const struct capsule_service_req **reqp, int *errp)
{
...
/*
 * Validate an capsule_net_req and populate an ort_net_claim.
...
{
	const struct capsule_net_req *req;
...
{
	const struct capsule_vsock_req *req;
