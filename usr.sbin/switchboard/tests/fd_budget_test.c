...
ATF_TC_HEAD(raise_limit, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "switchboard raises its soft descriptor limit before reserving slots");
}
ATF_TC_BODY(raise_limit, tc)
{
	struct switchboard_fd_budget_stats stats;
	...
	ATF_REQUIRE_EQ(0, switchboard_fd_budget_raise_limit());
	...
	switchboard_fd_budget_get_stats(&stats);
...
ATF_TC_BODY(reserve_and_admission, tc)
{
	...
	lower_limit();
	ATF_REQUIRE_EQ(0, switchboard_fd_budget_init());
	switchboard_fd_budget_get_stats(&stats);
	ATF_CHECK_EQ(SWITCHBOARD_FD_EMERGENCY_RESERVE, stats.reserve_count);
	ATF_REQUIRE_EQ(0, switchboard_fd_budget_check(4, "unit-test"));
	n = fill_descriptors(fillers, nitems(fillers));
	ATF_CHECK_ERRNO(EMFILE,
	    switchboard_fd_budget_check(1, "exhausted") == -1);
	switchboard_fd_budget_get_stats(&stats);
	...
	close(fillers[--n]);
	close(fillers[--n]);
	ATF_CHECK_EQ(0, switchboard_fd_budget_check(3, "recovered"));
	while (n > 0)
		close(fillers[--n]);
...
ATF_TC_BODY(control_shedding, tc)
{
	...
	snprintf(path, sizeof(path), "/tmp/switchboard-fd.%jd.sock",
	    (intmax_t)getpid());
	...
	n = fill_descriptors(fillers, nitems(fillers));
	ATF_CHECK_ERRNO(EMFILE,
	    switchboard_fd_budget_check(1, "control connection") == -1);
	switchboard_fd_budget_shed_control(listener);
	switchboard_fd_budget_get_stats(&stats);
