...
#
# Full-stack naming contracts for switchboard.  Managed programs come from the
...
finish_naming_stack()
{
	stop_stack || atf_fail "Capsule stack did not stop cleanly"
}

atf_test_case service_ready_protocol cleanup
service_ready_protocol_head()
{
	atf_set "descr" "service READY transitions its switchboard state to running"
...
naming_register_and_lookup_body()
{
	...
	prepare_paths
	install_naming_fixture org.test.switchboard-ls-provider \
	    'activation { ipc = ["org.test.ls-provider"]; }
arguments = ["provider", "provider-registered.result", "provider.result"];'
	install_naming_fixture org.test.switchboard-ls-client \
	    'arguments = ["client", "client.result"];'
	sed -i '' -e 's/ipc = \[[^]]*\]; //' -e 's/arguments = \["compat-ready", "[^"]*"\];/arguments = ["compat-ready"];/' \
	    "${APPS_DIR}/org.test.switchboard-ls-client.cap/Units/switchboard-ls-client.unit/Unit.ucl"
	start_stack
	...
	atf_check -s exit:0 \
	    -o match:'client_label=org.test.switchboard-ls-client/[^ ]* message=world confined=yes$' \
	    cat provider.result
	switchboardctl status > naming-status.result
	atf_check -s exit:0 -o match:'org.test.switchboard-ls-provider.*conns=1' \
	    cat naming-status.result
	atf_check -s exit:0 -o not-match:'org.test.switchboard-ls-client/switchboard-ls-client running.*conns=[1-9]' \
	    cat naming-status.result
	finish_naming_stack
}
...
arguments = ["provider", "provider-registered.result", "provider.result"];'
	...
	start_stack
	wait_naming_result provider.result
	provider_pid=$(switchboardctl status |
	    sed -n 's/.*org.test.switchboard-ls-provider\/switchboard-ls-provider running  *pid \([0-9][0-9]*\).*/\1/p')
	...
	if ! wait_for_log 'auto-unregistered.*org.test.ls-provider'; then
		capd_dump_diagnostics
		atf_fail "switchboard did not auto-unregister the exited provider"
	fi
	finish_naming_stack
}
