...
static bool
has(char list[][SWITCHBOARD_LABEL_MAX], unsigned n, const char *name)
{
	unsigned i;
...
{
	/* More provides than SWITCHBOARD_MAX_PROVIDES must not overflow. */
	static const char hdr[] =
	    "# PROVIDE: a b c d e f g h i j k l\n";
	struct rc_unit_meta m;

	ATF_CHECK_EQ(0, rc_parse_header(hdr, &m));
	ATF_CHECK(m.nprovides <= SWITCHBOARD_MAX_PROVIDES);
}
