 ...
 *
 * The switchboard control plane is the "system.switchboard" capability discovery name
 ...
 * system.switchboard capability channel and print "status=<n>" from the reply.
    ...
    const void *payload, size_t paylen)
{
	...
	char reqbuf[sizeof(struct sctl_request) + SWITCHBOARD_CTL_MAX_PAYLOAD + 16];
	char rplbuf[sizeof(struct sctl_reply) + SWITCHBOARD_CTL_SUMMARY_MAX];
	int fd;

	if (paylen > SWITCHBOARD_CTL_MAX_PAYLOAD + 16)
 ...
 * maximum (and does not match the bytes actually sent).  switchboard must reject
