 ...
 * is NOT adopted, while switchboard natively adopts a small, explicit allow-list
 * of rc.d services as supervised SVC_KIND_RC units.  switchboard first probes
...
#ifndef SWITCHBOARD_RC_ADOPT_H
#define SWITCHBOARD_RC_ADOPT_H
 ...
 * prefix permits switchboard to start it regardless of the rc.conf rcvar.
 */
...
/*
 * The subcommand switchboard uses to stop an adopted RC unit.  "onestop" (like
 ...
 * it is NOT the process switchboard holds a descriptor for.
 */
 ...
 * "/etc/rc.d", overridable with SWITCHBOARD_RCD_DIR for tests) that is not already
