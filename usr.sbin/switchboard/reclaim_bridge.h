 ...
 * It confers NO new authority (root can already drive `switchboardctl reclaim` over
 * the ambient ADMIN control plane); it is root-gated by getpeereid(2) and does
 * reclaim and nothing else.  See the block comment at SWITCHBOARD_RECLAIM_SOCK in
 * switchboard_ctl.h for the full rationale, and reclaim_bridge.c for the listener.
 *
 ...
 */
#ifndef SWITCHBOARD_RECLAIM_BRIDGE_H
#define SWITCHBOARD_RECLAIM_BRIDGE_H
...
#include "switchboard_ctl.h"		/* struct switchboard_reclaim_req, *_reply */
 ...
 */
static inline bool
reclaim_req_valid(const struct switchboard_reclaim_req *req)
{
	const void *nul;

	if (req->version != SWITCHBOARD_RECLAIM_VERSION)
		return (false);
...
/* Best-effort listener lifecycle (switchboard.c wires these into the event loop). */
...
#endif /* SWITCHBOARD_RECLAIM_BRIDGE_H */
