 ...
 *
 * Timer, path, and socket activation sources for switchboard (Phase 5 timer/path,
 ...
 *
 * A socket source is the manager-OWNED listener: switchboard binds and listen(2)s
 ...
 * by logical name and, because switchboard keeps its own copy open across the
...
activation_create_demand(struct svc_runtime *svc, const char *source, int kq)
{
	...
	if (svc->state != SVC_STATE_STOPPED && svc->state != SVC_STATE_DONE) {
		...
		SWITCHBOARD_PROBE_ON_DEMAND_COALESCE(svc->manifest.label);
		return;
	}
	...
	SWITCHBOARD_PROBE_ON_DEMAND_LAUNCH(svc->manifest.label, source);
	switchboard_audit(AUE_SWITCHBOARD_ONDEMAND, getuid(), 0,
 ...
 * switchboard binds and listen(2)s directly — deliberately NOT through the network
 * broker, which is itself an on-demand service and cannot be required to mint
 * switchboard's own boot listeners without a chicken-and-egg hazard.  All socket
...
listener_create(const struct svc_activation_socket *s)
{
	 ...
	 * EADDRINUSE; unlink it first.  This is safe because switchboard owns the
 ...
 * there is work to drain.  If the directory already holds work when switchboard
 ...
 * arm_path(), it never aborts switchboard and never blocks the other listeners or
...
arm_sockets(struct svc_runtime *svc, int kq)
{
	...
	for (i = 0; i < svc->manifest.nactivation_sockets; i++) {
		...
		SWITCHBOARD_PROBE_TIMEOUT_ARM(svc->manifest.label, "socket", 0);
 ...
 */
#define	CALENDAR_STATE_DIR	"/var/db/switchboard/calendar"
...
arm_calendar(struct svc_runtime *svc, int kq)
{
	...
	SWITCHBOARD_PROBE_TIMEOUT_ARM(svc->manifest.label, "calendar", delay);
...
activation_source_arm(struct svc_runtime *svc, int kq)
{
	...
	if (svc->manifest.timer_interval_sec > 0 &&
		...
		if (kevent(kq, &kev, 1, NULL, 0, NULL) == -1)
			...
			svc->activation_timer_ident = ident;
			SWITCHBOARD_PROBE_TIMEOUT_ARM(svc->manifest.label, "timer",
