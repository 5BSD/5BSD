 ...
 *
 * Capsule channel protocol client.
 *
 * Sends requests to capsule over the inherited mac_capability channel
...
/* Timeout for Capsule replies (milliseconds). */
#define	CAPSULE_RPC_TIMEOUT_MS	SWITCHBOARD_RPC_TIMEOUT_MS
#define	CAPSULE_DRAIN_REPOLL_MS	10	/* short re-poll for stragglers */
 ...
 * the status from capsule (0 = success, errno on failure).  Returns -1 on
 * communication error (sets errno).
 */
static int
capsule_rpc_fds(int channel_fd, const void *req, uint32_t reqlen,
    ...
    int *reply_fds, int max_reply_fds, int *nfds_out)
{
	...
	if (ioctl(channel_fd, MAC_CAPABILITY_SENDMSG, &sa) == -1) {
		syslog(LOG_WARNING, "capsule_rpc: sendmsg: %m");
		return (-1);
	}
	 ...
	 * to avoid hanging the event loop if capsule drops a reply.
	 *
...
retry:
		for (;;) {
			rv = poll(&pfd, 1, CAPSULE_RPC_TIMEOUT_MS);
			if (rv == -1) {
				if (errno == EINTR)
					continue;
				syslog(LOG_WARNING,
			...
			if (rv == 0) {
				syslog(LOG_ERR,
				    "capsule_rpc: timeout waiting for reply");
				errno = ETIMEDOUT;
				return (-1);
			}
			if (pfd.revents & (POLLERR | POLLHUP)) {
				syslog(LOG_ERR,
				    "capsule_rpc: channel closed");
				errno = ECONNRESET;
				return (-1);
			}
			...
			if (errno == EAGAIN)
				continue;
			syslog(LOG_WARNING,
			    "capsule_rpc: recvmsg: %m");
			return (-1);
		}

		if (ra.reply_token != token) {
			syslog(LOG_WARNING,
			    "capsule_rpc: draining stale reply "
			    "(got %ju, expected %ju)",
			...
			syslog(LOG_ERR,
			    "capsule_rpc: too many stale replies, giving up");
			errno = EPROTO;
			return (-1);
		}
	}
	...
	if (ra.payload_len != sizeof(rpl) ||
		...
		syslog(LOG_ERR, "capsule_rpc: malformed reply (%u bytes, %u fds)",
 ...
 */
static int
capsule_rpc(int channel_fd, const void *req, uint32_t reqlen,
    int *reply_fds, int max_reply_fds, int *nfds_out)
{

	return (capsule_rpc_fds(channel_fd, req, reqlen, NULL, 0,
	    reply_fds, max_reply_fds, nfds_out));
}

/*
 * Fill an capsule_system_req from a gates bitmask.
 */
static void
fill_system_req(struct capsule_system_req *req, uint32_t op, uint32_t gates)
{
 ...
 * Maps capsule error status to errno; returns 0 on success, -1 on error.
 */
static int
 ...
 * Maps capsule error status to errno; returns the fd on success, -1 on error.
 */
static int
check_status_fd(int status, int fd)
{
	...
	if (status == 0)
		return (fd);
	/*
	 * Error path.  capsule should not attach an fd alongside a non-zero
	 * status, but close one defensively rather than leak it.  status < 0
	 * is a communication error (errno already set by capsule_rpc); status
	 * > 0 is an capsule errno.
	 */
	if (fd >= 0)
		close(fd);
	if (status > 0)
...
int
capsule_mint_system(int channel_fd, uint32_t gates)
{
	struct capsule_system_req req;
	int token_fd, status;

	fill_system_req(&req, CAPSULE_OP_MINT_SYSTEM, gates);
	status = capsule_rpc(channel_fd, &req, sizeof(req), &token_fd, 1,
	    NULL);
 ...
 * A NULL/zero payload is exactly capsule_mint_system() (coarse), unchanged.
 * Returns the token fd on success, -1 (errno set) on failure.
 */
int
capsule_mint_system_scoped(int channel_fd, uint32_t gates,
    const void *oid_payload, size_t payload_len)
{
	...
	if (payload_len == 0 || oid_payload == NULL)
		return (capsule_mint_system(channel_fd, gates));

	if (payload_len > CAPSULE_MINT_SYSTEM_PAYLOAD_MAX) {
		errno = EINVAL;
		return (-1);
	}

	{
		uint8_t buf[CAPSULE_MINT_SYSTEM_REQ_MAX];

		fill_system_req(&req, CAPSULE_OP_MINT_SYSTEM, gates);
		memcpy(buf, &req, sizeof(req));
		memcpy(buf + sizeof(req), oid_payload, payload_len);
		status = capsule_rpc(channel_fd, buf,
...
int
capsule_create_channel(int channel_fd, int *our_end, int *child_end)
{
	struct capsule_req_hdr req;
	int fds[2];
	int status;

	memset(&req, 0, sizeof(req));
	req.op = CAPSULE_OP_CREATE_CHANNEL;

	status = capsule_rpc(channel_fd, &req, sizeof(req), fds, 2, NULL);
...
int
capsule_create_coalition(int channel_fd)
{
	struct capsule_req_hdr req;
	int cfd;
	int status;
	...
	memset(&req, 0, sizeof(req));
	req.op = CAPSULE_OP_CREATE_COALITION;

	status = capsule_rpc(channel_fd, &req, sizeof(req), &cfd, 1, NULL);
	return (check_status_fd(status, cfd));
}

int
capsule_send_ready(int channel_fd)
{
	struct capsule_req_hdr req;
	int status;

	memset(&req, 0, sizeof(req));
	req.op = CAPSULE_OP_READY;

	status = capsule_rpc(channel_fd, &req, sizeof(req), NULL, 0, NULL);
	if (status < 0)
		return (-1);
	return (status);
}
...
/*
 * Relay a system lifecycle transition to capsule (docs/lifecycle-capability-
 * design.md, P4b): switchboard authorized the request over its ADMIN-gated
 * system.lifecycle capability and forwards the opcode to the spine, which is
 * PID 1 and applies it.  Returns the Capsule's status (0 = accepted).
 */
int
capsule_lifecycle(int channel_fd, uint32_t lifecycle_op)
{
	struct capsule_lifecycle_req req;
	int status;

	memset(&req, 0, sizeof(req));
	req.op = CAPSULE_OP_LIFECYCLE;
...
/*
 * Relay a Capsule configuration reload to capsule (P4b): the reloadable half of
 * the capsule control surface, re-homed from the getpeereid socket onto the
 * Capsule channel.  Returns the Capsule's status (0 = ok).
 */
int
capsule_reload(int channel_fd)
{
	struct capsule_req_hdr req;
	int status;

	memset(&req, 0, sizeof(req));
	req.op = CAPSULE_OP_RELOAD;
 ...
 */
int
capsule_set_ambient_lookup(int channel_fd, int lookup_fd)
{
	struct capsule_req_hdr req;
	int status;
	...
	memset(&req, 0, sizeof(req));
	req.op = CAPSULE_OP_SET_AMBIENT_LOOKUP;

	status = capsule_rpc_fds(channel_fd, &req, sizeof(req), &lookup_fd, 1,
	    NULL, 0, NULL);
 ...
 */
static uint64_t
capsule_release_send(int channel_fd, const void *req, uint32_t reqlen)
{
	struct mac_capability_sendmsg_args sa;
	uint64_t token;
	...
	if (ioctl(channel_fd, MAC_CAPABILITY_SENDMSG, &sa) == -1) {
		syslog(LOG_WARNING, "capsule_release_send: sendmsg: %m");
		return (0);
	}
	return (token);
}
 ...
 * CAPSULE_RPC_TIMEOUT_MS, then reads as many replies as are
 * available.  Returns the number of replies successfully drained.
 */
static int
capsule_release_drain(int channel_fd, unsigned expected)
{
	struct mac_capability_recvmsg_args ra;
	struct capsule_reply rpl;
	struct pollfd pfd;
	...
	pfd.events = POLLIN;
	drained = 0;
	timeout = CAPSULE_RPC_TIMEOUT_MS;
	 ...
	 * timeout since Capsule is actively processing.
	 */
		...
		while (drained < expected) {
			...
			if (ioctl(channel_fd, MAC_CAPABILITY_RECVMSG, &ra) == -1) {
				if (errno == EAGAIN)
					break;	/* back to poll */
				syslog(LOG_WARNING,
				    "capsule_release_drain: recvmsg: %m");
				return ((int)drained);
			}
			drained++;
		}

		timeout = CAPSULE_DRAIN_REPOLL_MS;
	}
...
capsule_release_manifest(int channel_fd, const struct svc_manifest *m)
{
	...
	if (m->cap_system != 0) {
		...
		fill_system_req(&req, CAPSULE_OP_RELEASE_SYSTEM,
		    m->cap_system);
		if (capsule_release_send(channel_fd, &req, sizeof(req)) != 0)
			nsent++;
	}
	/*
	...
	if (nsent > 0)
		capsule_release_drain(channel_fd, nsent);
