 ...
 *
 * Service lifecycle supervisor for switchboard.
 *
...
schedule_restart(struct svc_runtime *svc, int kq)
{
	...
	SWITCHBOARD_PROBE_SVC_RESTART(svc->manifest.label, svc->restart_count);
	...
	if (kevent(kq, &kev, 1, NULL, 0, NULL) == -1)
		...
		SWITCHBOARD_PROBE_TIMEOUT_ARM(svc->manifest.label,
		    "restart", delay);
	}
}
...
svc_restart_now(struct svc_runtime *svc, int kq)
{
	...
	if (++svc->restart_count >= svc->manifest.max_failures) {
		...
		SWITCHBOARD_PROBE_SVC_DISABLED(svc->manifest.label,
		    svc->restart_count);
...
cancel_stop_timer(struct svc_runtime *svc)
{
	...
	EV_SET(&kev, svc->stop_timer_ident, EVFILT_TIMER, EV_DELETE,
	    0, 0, NULL);
	(void)kevent(switchboard_kq, &kev, 1, NULL, 0, NULL);
 ...
 * timer): honour a pending remove (shutdown / switchboardctl remove) and a pending
 * reload (swap the manifest and re-exec).  A switchboardctl restart is driven
...
supervisor_rc_post_stop(struct svc_runtime *svc)
{
	...
	if (svc->remove_pending) {
		...
		svc_remove(idx);
		svc_reregister_kevents(switchboard_kq);
		return;
	}
	if (svc->reload_pending) {
		...
		svc->restart_count = 0;
		if (svc_exec(svc, switchboard_kq) == -1)
 ...
 * daemonizes and reparents to init, so switchboard tracks only the command's
...
supervisor_command_exited(struct svc_runtime *svc, int exit_status)
{
	...
	if (svc->rc_checking) {
		...
		svc->state = SVC_STATE_STOPPED;
		if (svc_exec_rc_start(svc, switchboard_kq) == -1)
...
supervisor_handle_procdesc(struct kevent *kev)
{
	...
	if ((kev->fflags & NOTE_CAPMODE) != 0 &&
		...
		if (in_capmode == 1) {
			...
			SWITCHBOARD_PROBE_SVC_CAPMODE(svc->manifest.label,
			...
			SWITCHBOARD_PROBE_SVC_CAPMODE(svc->manifest.label,
			    svc->pid, -error);
			switchboard_audit(AUE_SWITCHBOARD_SVC_EXEC, getuid(), error,
			    "svc=%s pid=%jd phase=capmode-ready",
			    svc->manifest.label, (intmax_t)svc->pid);
		}
	}
	...
	if (kev->fflags & NOTE_EXIT) {
		...
		SWITCHBOARD_PROBE_SVC_EXIT(svc->manifest.label,
		    svc->pid, exit_status);
		...
		if (svc->idle_stop_pending) {
			int saved_listen[SWITCHBOARD_MAX_ACTIVATION_SOCKETS];
		...
		if (uptime_sec < RESTART_MIN_UPTIME_SEC) {
			...
			syslog(LOG_INFO, "service %s: restarting "
			    ...
			    svc->restart_count);
			svc_restart_now(svc, switchboard_kq);
		}
	}
}
...
supervisor_handle_timer(struct kevent *kev)
{
	...
	if (svc->stop_kill_pending && kev->ident == svc->stop_timer_ident) {
		svc->stop_kill_pending = false;
		svc->stop_timer_ident = 0;
		SWITCHBOARD_PROBE_TIMEOUT_FIRE(svc->manifest.label, "stop-kill");
		/*
		 * RC-unit backstop: the "onestop" command did not finish in
		 * time.  There is no meaningful process to SIGKILL — switchboard's
	...
	if (svc->restart_pending && kev->ident == svc->timer_ident) {
		svc->restart_pending = false;
		svc->timer_ident = 0;
		SWITCHBOARD_PROBE_TIMEOUT_FIRE(svc->manifest.label, "restart");
		if (sd.shutting_down) {
	...
	if (svc->idle_timer_ident != 0 && kev->ident == svc->idle_timer_ident) {
		svc->idle_timer_ident = 0;
		SWITCHBOARD_PROBE_TIMEOUT_FIRE(svc->manifest.label, "idle");
		/*
 ...
 * daemon daemonizes and reparents to init, so the process descriptor switchboard
...
svc_graceful_stop(struct svc_runtime *svc, int kq)
{
	...
	if (svc->protocol_ready && svc->control_channel != NULL) {
		...
		if (svc_channel_send_event(svc, &message, sizeof(message), NULL, 0,
			...
			SWITCHBOARD_PROBE_QUIESCE_REQUEST(svc->manifest.label,
			    message.reason, message.deadline_ms);
			return;
		}
...
svc_quiesce_complete(struct svc_runtime *svc, int status, int kq)
{
	...
	if (svc == NULL || svc->state != SVC_STATE_STOPPING ||
		...
		return;
	svc->quiesce_pending = false;
	SWITCHBOARD_PROBE_QUIESCE_COMPLETE(svc->manifest.label, status);
	if (status != 0)
