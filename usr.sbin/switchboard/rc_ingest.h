 ...
 * switchboard can run existing rc services (SVC_KIND_RC) in its own
...
#ifndef SWITCHBOARD_RC_INGEST_H
#define SWITCHBOARD_RC_INGEST_H

#include <stdbool.h>

#include "switchboard_manifest.h"	/* SWITCHBOARD_LABEL_MAX, SWITCHBOARD_MAX_PROVIDES */
 ...
 * BEFORE (must start before), and the KEYWORD flags switchboard cares about.
 */
struct rc_unit_meta {
	char		provides[SWITCHBOARD_MAX_PROVIDES][SWITCHBOARD_LABEL_MAX];
	unsigned	nprovides;
	char		req[RC_MAX_DEPS][SWITCHBOARD_LABEL_MAX];	/* REQUIRE */
	unsigned	nreq;
	char		before[RC_MAX_DEPS][SWITCHBOARD_LABEL_MAX]; /* BEFORE */
	unsigned	nbefore;
...
struct rc_unit {
	char			name[SWITCHBOARD_LABEL_MAX];   /* rc.d service name */
