 ...
 *
 * switchboard — service manager daemon internals.
 */
 ...
 *
 * SVC_KIND_RC provides rc(8) compatibility — switchboard can run existing
 ...
 * do not support: switchboard and capsule are always built and run together.)
 */
enum svc_kind {
...
struct svc_runtime {
	 ...
	 * channel.  Zero-initialized to SVC_DOMAIN_SYSTEM: switchboard-launched
	...
	uint8_t		name_state[SWITCHBOARD_MAX_PROVIDES];
	/*
	 ...
	 */
	bool		name_sendable[SWITCHBOARD_MAX_PROVIDES];
	...
	/*
	 * Manager-owned socket activation listeners (Phase 4).  switchboard binds
	 ...
	 */
	int		activation_listen_fds[SWITCHBOARD_MAX_ACTIVATION_SOCKETS];
 ...
 * switchboard skips at scan time even though their bundles are installed.  This
 * is the persistent enablement toggle behind switchboardctl enable/disable and the
 ...
 */
#define	SWITCHBOARD_DISABLED_PATH			"/Capabilities/Config/switchboard/disabled"
...
extern struct switchboard_state sd;
extern int switchboard_kq;

/* switchboard.c — dispatch one ready switchboard_kq event (main loop + rc bootstrap). */
void	switchboard_dispatch_event(struct kevent *kev);
 ...
 * process switchboard forks for the rc world (and everything rc launches)
 * inherits it; svc_exec_command spares it from the child's closefrom(2).
 * Installed once in startup.c before /etc/rc runs.
 */
extern int switchboard_ambient_lookup_fd;

/* sctl.c — capability control plane (system.switchboard / system.lifecycle) */
 ...
 * connection (P3): switchboard services SWITCHBOARD_CONTROL_NAME itself, gating
...
/*
 * A switchboard-held lookup channel (defined in domain.c).  on_demand.c only ever
...
switchboard_hash_djb2(const char *s)
{
	unsigned long h;
...
svc_runtime_init_fds(struct svc_runtime *svc)
{
	...
	for (unsigned aidx = 0; aidx < SWITCHBOARD_MAX_ACTIVATION_SOCKETS; aidx++)
