 ...
 *
 * switchboard — service manager and naming registry.
 *
 * Started by capsule as its single child.  Inherits a mac_capability channel
 * on fd 3 for requesting activation tokens, channels, coalitions, named
 * capability-service descriptors, and kernel-module prerequisites from
 * Capsule.  Scans capability bundles, dependency-sorts, pdfork/execs
 * services, and manages their lifecycle (restart, shutdown).
 *
 * Startup sequence:
 *   1. Inherit channel fd from CAPSULE_CHANNEL_FD env (fd 3)
 *   2. Create kqueue, register channel + signals
 *   3. Scan bundle directories
 *   4. Dependency sort
 *   5. Run /etc/rc, then fork/exec services (requesting tokens from capsule)
 *   6. Send CAPSULE_OP_READY to capsule to confirm this boot converged
 *   7. Enter event loop
 */
 ...
 * bootstrap wait can drive the same dispatch on switchboard_kq (answering ambient
...
switchboard_dispatch_event(struct kevent *kev)
{
	if (kev->filter == EVFILT_SIGNAL) {
		...
		case SIGINT:
			...
			sd.running = false;
			supervisor_stop(switchboard_kq);
			break;
		case SIGHUP:
			syslog(LOG_INFO,
			    "received SIGHUP, reloading");
			supervisor_reload(switchboard_kq,
			    NULL, 0);
			break;
		case SIGCHLD:
			/* Reap strays. */
	...
	if (kev->filter == EVFILT_READ &&
	    (int)kev->ident == sd.capsule_channel_fd) {
		if (kev->flags & EV_EOF) {
			syslog(LOG_CRIT,
			    "Capsule exited, stopping all services");
			SWITCHBOARD_PROBE_CAPSULE_DISCONNECTED();
			/* Loss of the Capsule
			 * is a security-relevant integrity event. */
			switchboard_audit(AUE_SWITCHBOARD_CAPSULE,
			    getuid(), EIO,
			    "Capsule channel disconnected; "
	...
	if (kev->filter == EVFILT_VNODE) {
		activation_path_event(kev, switchboard_kq);
		return;
	}

	/* Mount (EVFILT_FS) activation sources. */
	if (kev->filter == EVFILT_FS) {
		activation_mount_event(kev, switchboard_kq);
		return;
	}
	...
	if (kev->filter == EVFILT_READ &&
		...
		activation_socket_event(kev, switchboard_kq);
		return;
	}
	...
	if ((kev->filter == EVFILT_READ ||
		...
		domain_channel_event(kev, switchboard_kq);
		return;
	}
...
event_loop(void)
{
	...
	while (sd.running) {
		n = kevent(switchboard_kq, NULL, 0, &event, 1, NULL);
		if (n == -1) {
			if (errno == EINTR)
				continue;
...
usage(void)
{
	...
	fprintf(stderr, "usage: switchboard\n");
	exit(1);
}

/*
 * Validate the pkgbase-installed default identity.  switchboard deliberately
...
validate_default_identity(void)
{
	...
	group = getgrnam(SWITCHBOARD_DEFAULT_GROUP);
	user = getpwnam(SWITCHBOARD_DEFAULT_USER);
	...
	syslog(LOG_INFO, "default service identity validated: %s:%s "
	    "uid=%ju gid=%ju",
	    SWITCHBOARD_DEFAULT_USER, SWITCHBOARD_DEFAULT_GROUP,
	    (uintmax_t)user->pw_uid, (uintmax_t)group->gr_gid);
	SWITCHBOARD_PROBE_IDENTITY_VALIDATE(SWITCHBOARD_DEFAULT_USER,
	    SWITCHBOARD_DEFAULT_GROUP, 0);
...
fail:
	SWITCHBOARD_PROBE_IDENTITY_VALIDATE(SWITCHBOARD_DEFAULT_USER,
	    SWITCHBOARD_DEFAULT_GROUP, errno != 0 ? errno : EIO);
	syslog(LOG_CRIT, "invalid pkgbase default service identity %s:%s: %m",
	    SWITCHBOARD_DEFAULT_USER, SWITCHBOARD_DEFAULT_GROUP);
	switchboard_audit(AUE_SWITCHBOARD_SVC_EXEC, getuid(),
	    errno != 0 ? errno : EIO,
...
main(int argc, char *argv[])
{
	...
	/* Present as "SwitchBoard" in ps/top, distinct from lowercase base daemons. */
	setproctitle("-SwitchBoard");

	memset(&sd, 0, sizeof(sd));
	sd.capsule_channel_fd = -1;
	sd.channel_svc_fd = -1;
	 ...
	 */
	openlog("switchboard", LOG_PID | LOG_NDELAY | LOG_PERROR | LOG_CONS,
	    LOG_DAEMON);
	...
	/* Inherit channel fd. */
	channel_fd_str = getenv("CAPSULE_CHANNEL_FD");
	if (channel_fd_str != NULL) {
		...
		if (errno != 0 || *endp != '\0' ||
		    val < 0 || val > INT_MAX) {
			syslog(LOG_ERR, "invalid CAPSULE_CHANNEL_FD: %s",
			    channel_fd_str);
			return (1);
		}
		sd.capsule_channel_fd = (int)val;
	} else {
		syslog(LOG_ERR, "CAPSULE_CHANNEL_FD not set");
		return (1);
	}

	/* Non-blocking so capsule_rpc can timeout instead of hanging. */
	(void)fcntl(sd.capsule_channel_fd, F_SETFL, O_NONBLOCK);
	...
	/* Inherit delegated service instance fds (optional). */
	{
		...
		s = getenv("SWITCHBOARD_CHANNEL_SVC_FD");
		if (s != NULL) {
			errno = 0;
		...
		s = getenv("SWITCHBOARD_COALITION_SVC_FD");
		if (s != NULL) {
			errno = 0;
			val = strtol(s, &endp, 10);
			if (errno != 0 || *endp != '\0' ||
		...
		s = getenv("SWITCHBOARD_CAPPROTECT_FD");
		if (s != NULL) {
			errno = 0;
			val = strtol(s, &endp, 10);
			if (errno != 0 || *endp != '\0' ||
		...
		s = getenv("SWITCHBOARD_IDENTITY_FD");
		if (s != NULL) {
			errno = 0;
			val = strtol(s, &endp, 10);
			if (errno != 0 || *endp != '\0' ||
	...
	if (s != NULL && s[0] != '\0')
		switchboard_bundle_dir_system = s;
	...
	if (switchboard_fd_budget_init() == -1) {
		syslog(LOG_CRIT,
	...
	switchboard_kq = kqueue();
	if (switchboard_kq == -1) {
		syslog(LOG_ERR, "kqueue: %m");
		return (1);
	}
	if (cap_xfer_limit(switchboard_kq, CAP_XFER_NONE) == -1 ||
	    cap_clofork_limit(switchboard_kq, CAP_CLOFORK_LOCKED) == -1 ||
	    cap_cloexec_limit(switchboard_kq, CAP_CLOEXEC_LOCKED) == -1) {
	...
	add_signal_event(switchboard_kq, SIGTERM);
	add_signal_event(switchboard_kq, SIGINT);
	add_signal_event(switchboard_kq, SIGHUP);
	add_signal_event(switchboard_kq, SIGCHLD);
	 ...
	 * SIGCONT.  Capsule retains control through the procdesc returned by
	 ...
	 * process visibility.  Visibility is not authority to modify switchboard.
	 */
	if (sd.capprotect_fd < 0) {
		 ...
		 * ambient SIGKILL by design.  Only capsule's environment
		 * allowlist can set this.
		 */
		if (getenv("SWITCHBOARD_TEST_SHIELD_NO_SIGKILL") != NULL)
	...
	if (startup_launch_system(switchboard_kq) != 0) {
	 ...
	 * admin login mints a system.switchboard / system.lifecycle channel via
	 * naming_lookup, and switchboard adopts the provider end with
	 ...
	 * Individual service failures do not block convergence; only switchboard
	 * never reaching this point (crash or wedge) triggers PID 1 recovery.
	 */
	if (capsule_send_ready(sd.capsule_channel_fd) != 0) {
		syslog(LOG_ERR, "failed to send convergence READY to capsule");
		return (1);
	}
...
shutdown:
	...
	if (!sd.shutting_down)
		supervisor_stop(switchboard_kq);
	SWITCHBOARD_PROBE_SHUTDOWN_START(sd.nservices);
	{
		...
		}
		{
			...
			SWITCHBOARD_PROBE_SHUTDOWN_DONE(dur);
		}
	}
	on_demand_teardown(switchboard_kq);
	...
	syslog(LOG_INFO, "switchboard exiting");
	close(switchboard_kq);
	return (0);
}
