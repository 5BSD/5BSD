 ...
 * provider endpoint switchboard minted for SWITCHBOARD_CONTROL_NAME (system.switchboard)
 * or SWITCHBOARD_LIFECYCLE_NAME (system.lifecycle).  Administrative authority is
...
{
	struct switchboard_fd_budget_stats fd_stats;
	size_t off;
	...
	if (sd.nservices > 0)
		BUF_APPEND(summary, sumlen, &off,
		    ...
		    ndone);
	BUF_APPEND(summary, sumlen, &off, "\n");
	switchboard_fd_budget_get_stats(&fd_stats);
	BUF_APPEND(summary, sumlen, &off,
...
conn_destroy(struct sctl_conn *c)
{
	...
	nconns--;
	SWITCHBOARD_PROBE_CONN_CLOSE(nconns);
	free(c);
}
	...
	case SCTL_OP_RELOAD:
		...
		if (sctl_op_requires_admin(op) && !is_admin) {
			...
			switchboard_audit(AUE_SWITCHBOARD_CTL, audit_uid, EPERM,
			    "reload denied");
		} else if (supervisor_reload(switchboard_kq, summary,
		    summary_cap) == -1) {
			reply->status = EIO;
			switchboard_audit(AUE_SWITCHBOARD_CTL, audit_uid, EIO,
			    "reload failed");
		} else {
			reply->status = 0;
...
sctl_cap_sync_events(struct sctl_conn *c)
{
	...
	(void)kevent(switchboard_kq, &change, 1, NULL, 0, NULL);
}
    ...
    void *arg)
{
	...
	char payload[SWITCHBOARD_CTL_MAX_PAYLOAD + 1];
	char out[sizeof(struct sctl_reply) + SWITCHBOARD_CTL_SUMMARY_MAX];
    ...
    struct channel_message *request, void *arg)
{
	...
	char summary[SWITCHBOARD_CTL_SUMMARY_MAX];
	char out[sizeof(struct ctl_reply) + SWITCHBOARD_CTL_SUMMARY_MAX];
	const void *data;
	size_t len;
	int status;
	...
	if (data == NULL || len != sizeof(*req)) {
		...
		if (req->version != CTL_VERSION || req->flags != 0 ||
			...
			case CTL_OP_STATUS:
				/*
				 * Synthesized from what switchboard already tracks:
    ...
    bool capsule_relay)
{
	...
	if (switchboard_fd_budget_check(1, "capability control connection") == -1) {
		error = errno;
		close(provider_fd);
		errno = error;
		return (-1);
	...
	SWITCHBOARD_PROBE_CONN_ACCEPT((uid_t)-1, nconns);
	syslog(LOG_INFO,
