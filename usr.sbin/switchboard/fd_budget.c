...
static int emergency_fds[SWITCHBOARD_FD_EMERGENCY_RESERVE];
static size_t nemergency;
static struct switchboard_fd_budget_stats budget_stats;
static bool budget_initialized;

/*
 * Raise switchboard to the kernel's per-process descriptor ceiling before any
 ...
 */
int
switchboard_fd_budget_raise_limit(void)
{
...
int
switchboard_fd_budget_init(void)
{
	struct rlimit limit;
	int error;
	size_t i;
	...
	SWITCHBOARD_PROBE_FD_RESERVE((uint64_t)limit.rlim_cur,
...
void
switchboard_fd_budget_fini(void)
{
	size_t i;
 ...
 * emergency reserve.  switchboard is single-threaded, so opening temporary
 * duplicates and releasing them immediately is an admission barrier for the
 * allocation sequence that follows.
 */
int
switchboard_fd_budget_check(size_t required, const char *purpose)
{
	int *probe_fds;
	int error;
	size_t i, opened;
	...
	if (opened == required)
		...
		SWITCHBOARD_PROBE_FD_PRESSURE(purpose, required,
		    budget_stats.admission_denied);
	}
	for (i = 0; i < opened; i++)
		close(probe_fds[i]);
 ...
 */
void
switchboard_fd_budget_shed_control(int listen_fd)
{
...
void
switchboard_fd_budget_get_stats(struct switchboard_fd_budget_stats *stats)
{
