 ...
 *
 * On-demand service launch for switchboard.
 *
...
struct pending_lookup {
	...
	char			 provider_label[SWITCHBOARD_LABEL_MAX];
	pid_t			 provider_pid;
	uint64_t		 provider_launch_id;
	char			 requester_label[SWITCHBOARD_LABEL_MAX];
	pid_t			 requester_pid;
	...
	if (target == NULL) {
		...
		if (target != NULL) {
			if (target->state == SVC_STATE_STOPPED) {
				...
				SWITCHBOARD_PROBE_ON_DEMAND_COALESCE(name);
			}
			goto have_target;
		}
		...
		SWITCHBOARD_PROBE_ON_DEMAND_LAUNCH(name, target->launched_by);
...
on_demand_check_ready(struct svc_runtime *svc, int kq)
{
	...
	for (i = 0; i < svc->manifest.nprovides; i++) {
		...
		SWITCHBOARD_PROBE_ENDPOINT_ACTIVATE(svc->manifest.label,
...
on_demand_name_ready(struct svc_runtime *svc, const char *name, int kq)
{
	...
	if (nwaiters != 0)
		SWITCHBOARD_PROBE_ON_DEMAND_READY(name, nwaiters);
	if (nwaiters != 0) {
		syslog(LOG_INFO,
    ...
    int kq)
{
	...
	if (failed != 0) {
		syslog(LOG_WARNING,
		    ...
		    failed == 1 ? "" : "s");
		SWITCHBOARD_PROBE_ON_DEMAND_FAIL(name, error, failed);
		switchboard_audit(AUE_SWITCHBOARD_ONDEMAND, getuid(), error,
...
on_demand_provider_failed(struct svc_runtime *svc, int error, int kq)
{
	...
	if (failed != 0) {
		SWITCHBOARD_PROBE_ON_DEMAND_FAIL(svc->manifest.label, error,
		    failed);
		switchboard_audit(AUE_SWITCHBOARD_ONDEMAND, getuid(), error,
...
on_demand_requester_gone(struct svc_runtime *svc, int kq)
{
	...
	if (canceled != 0) {
		...
		SWITCHBOARD_PROBE_ON_DEMAND_CANCEL(svc->manifest.label,
		    svc->pid, svc->launch_id, canceled);
		switchboard_audit(AUE_SWITCHBOARD_ONDEMAND, getuid(), ECANCELED,
...
on_demand_timeout(uintptr_t ident, int kq)
{
	...
	char expired_provider[SWITCHBOARD_LABEL_MAX];
	pid_t expired_pid;
	...
	while (*pp != NULL) {
		...
		SWITCHBOARD_PROBE_ON_DEMAND_TIMEOUT(pl->name);
