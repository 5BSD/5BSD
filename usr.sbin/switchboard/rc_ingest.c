...
static void
add_name(char list[][SWITCHBOARD_LABEL_MAX], unsigned *n, unsigned max,
    const char *tok)
{

	if (*n >= max)
		return;
	strlcpy(list[*n], tok, SWITCHBOARD_LABEL_MAX);
	(*n)++;
}

/* Whitespace-split s, appending each token to list (bounded by max). */
static void
tokenize(const char *s, char list[][SWITCHBOARD_LABEL_MAX], unsigned *n,
    unsigned max)
{
	const char *start;
	char tok[SWITCHBOARD_LABEL_MAX];
	size_t tl;
...
{
	char kws[RC_MAX_DEPS][SWITCHBOARD_LABEL_MAX];
	unsigned n = 0, i;
