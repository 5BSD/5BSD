    ...
    uint32_t op, int status, int *fds, size_t nfds, bool cap_xfer)
{
	...
	if (channel_send_reply(request,
		...
		syslog(LOG_WARNING, "service %s: channel reply: %m",
		    svc->manifest.label);
		SWITCHBOARD_PROBE_ERROR("svc_proto", "channel reply failed");
		/*
	...
	SWITCHBOARD_PROBE_IPC_REPLY(svc->manifest.label, op, reply.status);
	svc_channel_sync_events(svc, switchboard_kq);
	return (0);
}
    ...
    struct channel_message *request)
{
	...
	(void)svc_channel_reply(svc, request, SVC_OP_QUIESCE_RESULT, error,
	    NULL, 0);
	if (error == 0)
		svc_quiesce_complete(svc, req->status, switchboard_kq);
}
...
handle_idle(struct svc_runtime *svc, struct channel_message *request)
{
	...
	if (channel_message_length(request) != sizeof(*req))
		...
		if (req->seconds > 0)
			arm_idle_timer(svc, switchboard_kq);
		else
    ...
    struct channel_message *request)
{
	...
	if (switchboard_fd_budget_check(4, "private worker channel") == -1)
		error = errno;
	else if (mac_cap_create_channel(&endpoints[0], &endpoints[1]) == -1)
		error = errno != 0 ? errno : EIO;
	SWITCHBOARD_PROBE_WORKER_CHANNEL(svc->manifest.label, error);
...
handle_name_claim(struct svc_runtime *svc, struct channel_message *request)
{
	...
	SWITCHBOARD_PROBE_ENDPOINT_CLAIM(svc->manifest.label, req->name, error);
    ...
    struct channel_message *request)
{
	...
	error = on_demand_name_withdraw(svc, req->name, switchboard_kq);
	SWITCHBOARD_PROBE_ENDPOINT_WITHDRAW(svc->manifest.label, req->name,
	    error);
	switchboard_audit(AUE_SWITCHBOARD_ONDEMAND, getuid(), error,
