...
lookup_channel_close(struct svc_lookup_channel *lc)
{
	 ...
	 */
	on_demand_lookup_channel_gone(lc, switchboard_kq);
 ...
 * a wider one.  On success switchboard pushes an ACK on the ADOPTED (private)
    ...
    struct channel_message *request)
{
	...
	if (switchboard_fd_budget_check(2, "registered lookup channel") == -1) {
	...
	lookup_channel_sync_events(adopted, switchboard_kq);
    ...
    struct channel_message *request, void *context)
{
	...
	if (op == SVC_OP_AMBIENT_HELLO) {
		 ...
		 * that this really is a switchboard ambient LOOKUP channel.  Only a
	...
	if (client_fd < 0) {
		 ...
		 */
		SWITCHBOARD_PROBE_DOMAIN_LOOKUP_DENY(req->name, kindstr, error);
...
out:
	...
	lookup_channel_sync_events(lc, switchboard_kq);
}

/*
 * Mint a fresh lookup channel bound to (kind, uid).  switchboard keeps one end
 ...
 * SYSTEM channel resolves every registered name (the channel switchboard installs
 * before running /etc/rc); a USER channel resolves only user-visible names.
 * Returns 0 on success, -1 with errno set on failure.
 */
/*
 * Adopt a switchboard-held channel endpoint as a lookup channel scoped to
 ...
 *
 * Takes ownership of switchboard_end: on success it is owned by the returned lc
    ...
    const struct svc_anoint_set *set, int kq)
{
	...
	if (channel_create(switchboard_end, &options, &lc->channel) == -1) {
		error = errno;
		(void)close(switchboard_end);
		free(lc);
		errno = error;
	...
	/* channel_create() consumed switchboard_end (owns its own duplicate). */
    ...
    const struct svc_anoint_set *set, int *out_fd, int kq)
{
	...
	*out_fd = -1;
	...
	if (switchboard_fd_budget_check(4, "user-domain lookup channel") == -1)
		return (-1);

	if (mac_cap_create_channel(&switchboard_end, &client_end) != 0) {
	 ...
	 * ends close-on-exec; relax the caller's end while keeping switchboard's own
	 * end confined.
	 */
