...
ATF_TC_BODY(default_deny, tc)
{
	...
	ATF_REQUIRE_EQ(0, rebootd_policy_load("missing", &policy));
	ATF_CHECK_EQ(0, policy.count);
	ATF_CHECK(!rebootd_policy_allows(&policy, "org.test.client"));
	ATF_CHECK(!rebootd_policy_allows(NULL, "org.test.client"));
	ATF_CHECK(!rebootd_policy_allows(&policy, NULL));
}
...
ATF_TC_BODY(exact_labels, tc)
{
	...
	ATF_REQUIRE_EQ(0, rebootd_policy_load("policy", &policy));
	ATF_CHECK_EQ(2, policy.count);
	ATF_CHECK(rebootd_policy_allows(&policy, "org.test.reboot"));
	ATF_CHECK(rebootd_policy_allows(&policy, "org.test.second"));
	ATF_CHECK(!rebootd_policy_allows(&policy, "org.test"));
	ATF_CHECK(!rebootd_policy_allows(&policy, "org.test.reboot.child"));
}
...
ATF_TC_BODY(wildcard_rejected, tc)
{
	...
	ATF_CHECK_ERRNO(EINVAL,
	    rebootd_policy_load("policy", &policy) == -1);
	ATF_CHECK_EQ(0, policy.count);
}
...
ATF_TC_BODY(malformed_labels, tc)
{
	...
	for (i = 0; i < nitems(invalid); i++) {
		...
		if (strcmp(invalid[i], "\t\n") == 0)
			ATF_CHECK_EQ(0,
			    rebootd_policy_load("policy", &policy));
		else
			ATF_CHECK_ERRNO(EINVAL,
			    rebootd_policy_load("policy", &policy) == -1);
	}
...
ATF_TC_BODY(arguments, tc)
{
	...
	ATF_CHECK_ERRNO(EINVAL,
	    rebootd_policy_load(NULL, &policy) == -1);
	ATF_CHECK_ERRNO(EINVAL,
	    rebootd_policy_load("missing", NULL) == -1);
}
...
ATF_TC_BODY(unsafe_policy_file_rejected, tc)
{
	...
	ATF_CHECK(rebootd_policy_load("policy-dir", &policy) == -1);
	ATF_REQUIRE_EQ(0, symlink("policy-dir", "policy-link"));
	ATF_CHECK(rebootd_policy_load("policy-link", &policy) == -1);
}
