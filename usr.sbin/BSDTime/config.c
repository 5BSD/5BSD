 ...
 *
 * BSDTime per-label policy: which labels may step/slew the clock.  Mirrors the
...
void
timecmp_config_defaults(struct timecmp_config *config)
{

	memset(config, 0, sizeof(*config));
	config->default_set = false;	/* deny by default */
	config->nclients = 0;
}
...
valid_label(const char *label, size_t length)
{
	...
	if (label == NULL || length == 0 || length > TIMECMP_CONFIG_LABEL_MAX)
		return (false);
	for (i = 0; i < length; i++) {
...
static int
config_parse(const char *text, struct timecmp_config *config)
{
	...
	if (def != NULL) {
		if (ucl_object_type(def) != UCL_OBJECT)
			goto out_root;
		config->default_set = object_set_flag(def);
	}
...
int
timecmp_config_load_fd(struct timecmp_config *config, int fd)
{
	struct stat status;
	char *text;
	ssize_t amount;
	...
	if (status.st_size < 0 || status.st_size > TIMECMP_CONFIG_FILE_MAX) {
		errno = EFBIG;
		goto fail;
	}
	text = malloc(TIMECMP_CONFIG_FILE_MAX + 2);
	if (text == NULL)
		goto fail;
	done = 0;
	for (;;) {
		amount = read(fd, text + done, TIMECMP_CONFIG_FILE_MAX + 1 - done);
		...
		if (done > TIMECMP_CONFIG_FILE_MAX) {
			errno = EFBIG;
			goto fail_text;
		}
	}
...
int
timecmp_config_load(struct timecmp_config *config, const char *path)
{
	int fd;
	...
	return (timecmp_config_load_fd(config, fd));
}

bool
timecmp_config_permits_set(const struct timecmp_config *config,
    const char *label)
{
	size_t i;
	...
	for (i = 0; i < config->nclients; i++)
		if (strcmp(config->clients[i].label, label) == 0)
			return (config->clients[i].may_set);
	return (config->default_set);
}
