...
ATF_TC_BODY(peer_authorized_root_only, tc)
{
	...
	ATF_CHECK_MSG(!reclaim_peer_is_authorized(976),
	    "the serviced uid (976) is not root and must not be authorized");
...
ATF_TC_BODY(req_validation_edges, tc)
{
	...
	/* Wrong version. */
	req.version = SERVICED_RECLAIM_VERSION + 1;
...
struct mock_action_state {
	unsigned	calls;
	char		last_label[SERVICED_RECLAIM_LABEL_MAX];
	unsigned	ret;
};
 ...
 */
static int
run_serve(const struct serviced_reclaim_req *req, bool authorized,
