...
if [ "$objtop_set" -eq 0 ] && [ ! -f "$srcroot/Makefile.inc1" ]; then
	oracled_dir=/usr/sbin
	oraclectl_dir=/usr/sbin
	serviced_dir=/usr/libexec
	servicectl_dir=/usr/sbin
	serviced_test_dir=/usr/tests/usr.sbin/serviced
	oracled_test_dir=/usr/tests/usr.sbin/oracled
	oraclert_dir=/usr/lib
	oraclectl_lib_dir=/usr/lib
	...
	oraclert_test_dir=/usr/tests/lib/liboraclert
else
	development_mode=1
	...
	oraclectl_lib_dir="$objtop/lib/liboraclectl"
	...
	oraclert_test_dir="$objtop/lib/liboraclert/tests"
fi

if [ "$development_mode" -eq 1 ]; then
	harness_source="$srcroot/usr.sbin/oracled/tests/capd_test_harness.sh"
else
	harness_source="$service_test_dir/capd_test_harness.sh"
fi

claim_parse_prog="$oraclert_test_dir/claim_parse_test"
req_validate_prog="$oracled_test_dir/req_validate_test"
guardian_prog="$oracled_test_dir/capd_test_guardian"
bootstrap_fixture_prog="$oracled_test_dir/capd_bootstrap_fixture"
...
for executable in \
	...
	"$oraclert_test_dir/claim_parse_test" \
	...
	"$oracled_test_dir/capd_bootstrap_fixture" \
	"$oracled_test_dir/capd_test_guardian" \
	"$oracled_test_dir/capd_test_guardian_test" \
	"$oracled_test_dir/req_validate_test" \
...
for library in \
	...
	"$oraclert_dir/liboraclert.so.1" \
	"$oraclectl_lib_dir/liboraclectl.so.1"
do
	if [ ! -r "$library" ]; then
...
if ! nm -D "$service_lib_dir/libservice.so.1" 2>/dev/null |
    awk '$3 == "service_capability_fd" { found = 1 }
        END { exit !found }'; then
	echo "error: development libservice is missing service_capability_fd" >&2
...
if [ "$development_mode" -eq 1 ]; then
	...
	static_tools_dir=$(mktemp -d "$objtop/.oracle-serviced-tools.XXXXXX") ||
	    exit 1
	trap cleanup_static_tools EXIT
	trap 'exit 1' HUP INT TERM
	echo "Linking coherent static development tools"
	if ! link_static_program "$srcroot/usr.sbin/oracled" \
	    "$oracled_dir" "$static_tools_dir/oracled" ||
	    ! link_static_program "$srcroot/usr.sbin/oraclectl" \
	    "$oraclectl_dir" "$static_tools_dir/oraclectl" ||
	    ...
	    ! link_static_program "$srcroot/lib/liboraclert/tests" \
	    "$oraclert_test_dir" "$static_tools_dir/claim_parse_test" \
	    PROG=claim_parse_test ||
	    ! link_static_program "$srcroot/usr.sbin/oracled/tests" \
	    "$oracled_test_dir" "$static_tools_dir/req_validate_test" \
	    PROG=req_validate_test ||
	    ! link_static_program "$srcroot/usr.sbin/oracled/tests" \
	    "$oracled_test_dir" "$static_tools_dir/capd_test_guardian" \
	    PROG=capd_test_guardian ||
	    ! link_static_program "$srcroot/usr.sbin/oracled/tests" \
	    "$oracled_test_dir" "$static_tools_dir/capd_bootstrap_fixture" \
	...
	oracled_dir=$static_tools_dir
	oraclectl_dir=$static_tools_dir
...
PATH="$oracled_dir:$oraclectl_dir:$serviced_dir:$servicectl_dir:$oraclert_dir:/usr/bin:/bin:/usr/sbin:/sbin"
LD_LIBRARY_PATH="$service_lib_dir:$capability_lib_dir:$capbundle_lib_dir:$oraclert_dir:$oraclectl_lib_dir${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
...
if [ "$development_mode" -eq 1 ]; then
	...
	stage_atf_shell "$srcroot/usr.sbin/oracled/tests/capd_test_guardian_test.sh" \
	    "$suite_dir/capd_test_guardian_test" || exit 1
	stage_atf_shell "$srcroot/usr.sbin/oracled/tests/oracled_bootstrap_test.sh" \
	...
	ln -s "$service_test_dir/libservice_test" \
	    "$suite_dir/libservice_test" || exit 1
	ln -s "$oracled_test_dir/capd_test_guardian_test" \
	    "$suite_dir/capd_test_guardian_test" || exit 1
...
set -- \
	...
	oracle_bluetooth_wire_validation=req_validate_test:bluetooth_network_requests \
	oracle_strict_wire_validation=req_validate_test:strict_wire_shape \
	oracle_kmod_wire_validation=req_validate_test:kmod_request_validation \
	oracle_service_wire_validation=req_validate_test:service_request_validation \
	...
	kmod_prerequisite_uses_oracle=serviced_integration_test:kmod_prerequisite_uses_oracle \
	incomplete_capability_set_prevents_exec=serviced_integration_test:incomplete_capability_set_prevents_exec \
	control_reload_reaches_serviced=oracled_bootstrap_test:control_reload_reaches_serviced \
