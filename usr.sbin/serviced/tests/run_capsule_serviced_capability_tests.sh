#!/bin/sh
#
# Run the focused Capsule/serviced capability suite under Kyua.  Kyua owns
...
if [ -z "$results" ]; then
	results=$(mktemp -d /tmp/capsule-serviced-root.XXXXXX) || exit 1
else
...
if [ "$development_mode" -eq 1 ]; then
	...
	static_tools_dir=$(mktemp -d "$objtop/.capsule-serviced-tools.XXXXXX") ||
	    exit 1
...
PATH="$capsule_dir:$capsulectl_dir:$serviced_dir:$servicectl_dir:$capsulert_dir:/usr/bin:/bin:/usr/sbin:/sbin"
...
set -- \
	...
	kmod_prerequisite_uses_capsule=serviced_integration_test:kmod_prerequisite_uses_capsule \
	incomplete_capability_set_prevents_exec=serviced_integration_test:incomplete_capability_set_prevents_exec \
	control_reload_reaches_serviced=capsule_bootstrap_test:control_reload_reaches_serviced \
	bootstrap_channel_loss_kills_serviced=capsule_bootstrap_test:bootstrap_channel_loss_kills_serviced \
