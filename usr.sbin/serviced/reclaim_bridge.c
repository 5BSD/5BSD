 ...
 * ============================================================================
 *
 * serviced otherwise speaks only over the capability discovery plane (minted
 ...
 * serviced's SERVICED_CONTROL_NAME plane to run the normal `servicectl reclaim`.
 *
 * It confers NO NEW AUTHORITY.  Root can already drive `servicectl reclaim`
 ...
 * (struct serviced_reclaim_req), one fixed reply out
 * (struct serviced_reclaim_reply), connection closed.  There is no other op.
 *
 * Ownership / gating reasoning: serviced runs as uid 976
 ...
 * a warning is logged and serviced runs normally (reclaim is simply not
 ...
 * the existing authorized broadcast.  arg points to the serviced kqueue fd.
 */
static unsigned
...
reclaim_bridge_accept(int kq)
{
	 ...
	 * cannot stall serviced's single-threaded event loop: a receive/send
...
reclaim_bridge_init(int kq)
{
	 ...
	 * EADDRINUSE.  serviced is the sole binder of this path, so unlinking a
	 * leftover node is safe (mirrors activation.c's AF_UNIX bind path).
	 */
	(void)unlink(SERVICED_RECLAIM_SOCK);
	...
	/*
	 * Confine the listener as a serviced-internal descriptor: it must never
