...
Capsule must treat remaining alive as a safety property.  A normal daemon's
...
An Capsule rollout must preserve an independently bootable binary in
...
init's devfs detection and mount.  Therefore a Capsule entered through
...
is intentionally catastrophic and must not be copied blindly by Capsule; a
...
Thus neither syslog nor a working console is assumed.  Capsule needs the same
...
using entries to supervise arbitrary daemon commands.  Capsule must either
...
restart to take effect.  Capsule should test compatible behavior or document
...
coordination primitive used by low-level reboot paths.  Capsule needs an
...
Capsule signal shield blocks it unless the tools and rc integration use an
authorized Capsule control endpoint.  The desired end state is a capability
...
Capsule replacement should preserve equivalent observability even if it uses
...
reliability defect.  Capsule must make an explicit compatibility decision:
...
Capsule cannot claim init compatibility without either implementing this
...
process state.  Capsule's monotonic descriptor policy should close or limit
...
does not decide that `service capsule stop` means `SIGTERM`; the Capsule rc.d
...
capability tokens, activate program capabilities, or understand Capsule's
...
These are documentation defects or historical seams, not additional Capsule
requirements.  Each should receive a regression test and an explicit
compatibility choice before migration.

## 23. Legacy failure seams Capsule must not inherit accidentally
...
Compatibility does not require reproducing these defects.  Capsule should
...
## 24. Capsule compatibility decisions and required work
...
7. Enter initially through `init_rc` for boot-flow testing, then through
   `init_exec` only after Capsule owns all early `/dev`, console, recovery, and
...
8. Treat `/etc/ttys`, reroot, login classes, tracing, fallback init images,
   ...
   silently.
9. Test every state and failure edge in a VM before making Capsule the first
...
- `run_rc_scripts()` does not fail fast or aggregate individual script errors,
  and stock `/etc/rc` ends with `exit 0`.  Capsule therefore needs explicit
...
- `/etc/rc` requests a firstboot reboot with `kill -INT 1`, which mandatory
  Capsule signal shielding would reject unless this call is migrated to the
...
unchanged.  Every retained behavior should map to a Capsule test; every
