# Capsule lifecycle control ABI: moving PID 1 off the signal interface
...
- **untyped** — eight signals encode the entire lifecycle; no payload, no
  reply, no status;
- **in direct conflict with the capability integrity shield** — Capsule's
...
New opcodes on the existing `capsule` control protocol
(`lib/libcapsulert/capsule_ctl.h`; opcodes 7-9 remain reserved):
...
```
capsulectl / reboot(8) → connect capsule.sock → CTL_OP_REBOOT
...
`capsule` daemon is not the system spine and cannot reboot the machine.
...
is minimal. After `establish_capsule` brings up the engine, the shield makes
...
- Both tools now **fall back to the signal ABI on any non-zero result**
  (socket absent *or* a refusal from a non-PID-1 capsule that isn't the real
...
fallback `/sbin/init` binary free of any capsule/socket dependency (it is the
recovery init in `kern.init_path`). Under a shielded capsule, `init N`
silently no-ops. Use `reboot(8)`/`shutdown(8)`/`halt(8)`/`capsulectl` instead;
...
2. Add `capsulectl_lifecycle()` client; teach `reboot`/`shutdown`/`halt` to try
   the socket first and **fall back to the signal ABI** when the socket is
   absent (stock-init systems, and Capsule's own pre-engine early-boot window).
