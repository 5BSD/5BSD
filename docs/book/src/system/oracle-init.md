...
5BSD replaces `init(8)` with **Oracle** as PID 1. The same binary serves two
roles: as `oracled(8)` it is the capability broker — the authority that owns
...
personality (`usr.sbin/oracled/oracle_init.c`). The daemon path (daemonize,
pidfile, exit-on-error) never runs at PID 1:

```c
/* usr.sbin/oracled/oracled.c */
if (getpid() == 1)
...
Oracle deliberately splits the roles launchd combines into one process:
...
`/sbin/oracle-init` is a second installed copy of the oracled binary kept on
the root filesystem, because `/usr` may be a separate, unmounted filesystem
when the kernel starts init (see `usr.sbin/oracled/Makefile`). The shipped
loader configuration selects it with executable fallbacks:

```sh
# /boot/loader.conf.d/oracle-init.conf
init_path="/sbin/oracle-init:/sbin/init:/sbin/init.bak:/rescue/init"
```
...
oracle-init → serviced → rc → native services → converge → login is validated
...
PID 1 compatibility invariants (from `docs/oracle-init-todo.md`): never
...
Oracle replaces that with typed operations on the authenticated control
socket `/var/run/oracled.sock` (design: `docs/oracle-control-abi-design.md`,
opcodes in `lib/liboraclert/oracled_ctl.h`):

```text
CTL_OP_SHUTDOWN    1   graceful oracled shutdown        (root)
...
(stock-init systems and Oracle's own pre-engine early-boot window). See
`sbin/reboot/reboot.c`. The lifecycle opcodes are rejected when
`getpid() != 1`: an ordinary oracled daemon cannot reboot the machine.
Authorization is root via `getpeereid(3)`. Operator usage:

```sh
oraclectl status      # daemon status, active claims, serviced state (any user)
oraclectl reload      # reload oracled.conf claims, SIGHUP serviced (root)
oraclectl shutdown    # graceful oracled shutdown (root)
```

## Signal shield

Oracle's capability integrity shield (`CP_SF_SIGNAL`, plus `SIGKILL` and
...
When Oracle is PID 1, the `CP_SF_SIGNAL` shield is *deferred* until the
...
(`usr.sbin/oracled/mac_capability_claims.c`). Signal handlers remain
...
and oracled's `pdkill(2)` authority over serviced (procdesc signalling
bypasses `p_cansignal`) keep working under the full shield. `serviced` is
shielded the same way; oracled retains explicit stop authority through the
