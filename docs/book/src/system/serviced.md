...
At startup `serviced` runs the transitional `/etc/rc autoboot` oneshot (see
...
is structural. `Config/` is the one static exception: `tzfsd` and `serviced`
...
Each native unit is launched with `pdfork(2)`: `serviced` sets up activation
...
ordinary serviced-managed unit; `login`/`su`/`sshd` ask the agent rather than
...
`service_storage_open()`. `serviced` holds no storage capability; it only
