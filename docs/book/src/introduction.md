...
service adopts the stronger model when it is ready to, one at a time. 5BSD ships
an **SDK** — `libservice`, `libcapbundle` — that lets you write a *capability*:
a program reached by name, launched on demand, sandboxed by construction, and
served to each client in its own isolated worker. See [The Hybrid Model: BSD
plus a Capability SDK](development/hybrid-model.md) for how that is written.

## What 5BSD is

A handful of decisions define the platform; the rest of the Epic is their
consequences (they are laid out in full in [Architecture](architecture.md)).
...
- **Reached by name, through a library.** Capabilities are typed services
  ...
  no `getpeereid(3)`.
- **Policy in manifests, not code.** A service ships as a **capability bundle**
  whose manifest declares only how to launch a program; the program acquires
  whatever authority it needs at runtime, by name, each grant scoped to its own
  unforgeable channel label.
- **Enforcement in the kernel, below the API.** A mandatory-access-control and
  capability framework in the kernel gates every operation; an application sees
  policy only as `EACCES`/`EPERM` and cannot see, map, or disable the layer
  that produced it.
...
On top of that core, 5BSD ships its own userland stacks, each in its own
...
- **ObservableBSD** — OpenTelemetry export, instruments, and hardware telemetry
  in base ([Observability](observability/observablebsd.md)).

## Running Linux and BSD software

5BSD runs the full inherited BSD userland unchanged, and it treats the **Linux
syscall interface as a first-class execution target** — not a legacy-emulation
afterthought. Linux binaries call `clone()`, `mmap()`, `open()`, `sendmsg()`,
and the Linuxulator translates each into a native BSD kernel operation before
it executes.
...
translation boundary, by the MAC and capability framework the Linux code never
touches, on a kernel it does not know is there. A Linux program does not adapt
to this and cannot attack through it; it simply succeeds, or receives
`EACCES`/`EPERM` from a layer it cannot see. The same is true of native BSD
binaries: the capability plane sits under *every* application, whichever ABI it
was compiled for.
...
manual pages. The Epic deliberately does not reiterate that material.

That said, **5BSD is a separate project, not a FreeBSD distribution.** It plans
to eject and modify significant portions of the inherited system over time, and
**FreeBSD 16 is the last version 5BSD adopts wholesale** — later FreeBSD
releases are sources of selectively merged improvements, not a base to track.
The divergence is already underway: init duties have moved to
...
- **Developers** extending the system: start with [The Hybrid Model: BSD plus a
  Capability SDK](development/hybrid-model.md), then the rest of the
  [Developer Guide](development/writing-components.md).
- **Security architects**: start with [Architecture](architecture.md), then the
  [Security](security/mac-capability.md) section.
- **Virtualization operators**: the [WASPNest](virtualization/overview.md)
  section is self-contained.
- **Builders and release engineers**: see [Operations](operations/building.md).
