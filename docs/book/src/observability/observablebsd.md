...
framework), and who is *allowed to look* (`BSDTrace`, the capability broker
...
`bsdtrace`, and the BSDVM virtio device models expose their own USDT
...
introspection. `BSDTrace` replaces that uid gate with a capability gate. It
...
full kernel-introspection power DTrace has — `BSDTrace` narrows *who* can
trace and *how the consumer is obtained*, not *how much* an authorized
tracer can see.

Reference: `BSDTrace(8)`, `libtracecmp(3)`.
