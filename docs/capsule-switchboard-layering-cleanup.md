# Layering cleanup: get leaf-daemon work out of capsule and switchboard

## Principle

Two programs are the most privileged and hardest-to-change in the system:
`capsule` (runs as PID 1) and `switchboard` (the service manager). Neither
...
- **Typed claim arrays in the manifest:** `switchboard_manifest.h` `struct
  svc_manifest` carries `cap_net[]`, `cap_jail[]`, `cap_vsock[]`,
  `cap_storage[]` (+ inline jail fields), each an `ort_*`/`switchboard_*` wire type.
...
**switchboard delivers the consumer a held channel to the owning daemon,
pre-attenuated to the bounds the manifest declares, and the consumer mints for
itself over that channel.** switchboard and capsule never see a
...
manifest (declares BOUNDS: domain, rights ceiling, lifetime)
   │
   ▼
switchboard ── launch · deliver fds/channels · supervise · discovery · launch policy
...
- **The consumer self-serves at runtime** (launchd's decoupling) — but it can
  ...
  the handed-out capability, not by switchboard interpreting a request.
...
**Target = (B) deliver-channel.** (A) broker-forward — switchboard/capsule
...
- switchboard gains one `capsule_delegate(domain, blob)` client that returns
...
`capsule_kmod_req`/`validate_kmod_req`/kmod DTrace probe; switchboard
...
library confines the process; switchboard never touches jails.

- **switchboard is completely jail-free**: removed the `has_jail` create path,
  `svc->jail_fd`, the child `jail_attach_jd`, the cleanup, and `-ljail`.  A
  jailed unit is launched like any other; nothing about jails is switchboard's
...
Once V1–V3 prove the pattern, finish the launchd model: switchboard stops
...
> Transitional note: Phases 1–3 may land as **(A)** (switchboard/capsule relay
...
- Adding a new capability daemon touches **neither** capsule nor switchboard.
