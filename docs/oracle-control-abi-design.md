# Oracle lifecycle control ABI: moving PID 1 off the signal interface

Companion to `oracle-init-todo.md` (Phase 9) and `freebsd-init-behavior-audit.md`
...
- **untyped** — eight signals encode the entire lifecycle; no payload, no
  reply, no status;
- **in direct conflict with the capability integrity shield** — Oracle's
...
| --- | --- | --- | --- |
| **Spine (lower half)** | `oracled`/`oracle-init` (PID 1) | `oraclectl` | system lifecycle (reboot/halt/single-user/reroot/rescan/catatonia), capability authority, reaping, `/etc/rc`, serviced supervision |
...
- `shutdown(8)`, `reboot(8)`, `halt(8)` → **oracled's** control socket. These
...
New opcodes on the existing `oracled` control protocol
(`lib/liboraclert/oracled_ctl.h`; opcodes 7-9 remain reserved):
...
```
oraclectl / reboot(8) → connect oracled.sock → CTL_OP_REBOOT
...
`oracled` daemon is not the system spine and cannot reboot the machine.
...
shutdown timeouts, and oracled's own `pdkill` authority over `serviced`
...
- Both tools now **fall back to the signal ABI on any non-zero result**
  (socket absent *or* a refusal from a non-PID-1 oracled that isn't the real
...
fallback `/sbin/init` binary free of any oracled/socket dependency (it is the
recovery init in `kern.init_path`). Under a shielded oracle-init, `init N`
silently no-ops. Use `reboot(8)`/`shutdown(8)`/`halt(8)`/`oraclectl` instead;
telinit under Oracle PID 1 is unsupported.

## Emergency reboot when the event loop is wedged

The one robustness cost of removing signals: a wedged oracle-init event loop
...
1. **`reboot(2)` direct** — available today. `reboot -q` calls the `reboot(2)`
   ...
   of oracle-init's state. This is the manual emergency path.
...
**Decision: oracle-init does NOT self-pet a `watchdog(4)`.** Neither launchd
...
2. Add `oraclectl_lifecycle()` client; teach `reboot`/`shutdown`/`halt` to try
   the socket first and **fall back to the signal ABI** when the socket is
   absent (stock-init systems, and Oracle's own pre-engine early-boot window).
