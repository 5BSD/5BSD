...
These go through `usr.sbin/capsule/mac_capability_mint.c` to the
`mac_capability` isolation fd (`FI_OP_MINT*`). capsule owns that device;
...
| **V2** | **Module loading** — raw privileged syscall as policy | `capsule_proto.c:891-893` `modfind()`/`kldload()` in `handle_ensure_kmod`; `serviced/kldmgr_client.c` + `execute.c:1214` decide *which* modules | The module broker's job — now owned by `sysextd` (`system.SystemExtension`) — leaked back across PID 1 + serviced after the standalone in-PID1 loader was removed. `control.c:10` already admits "kldload … here temporarily". |
| **V3** | **Jail construction** — raw privileged syscall | `capsule_proto.c:682` `jail_set(iov, niov)` in `handle_create_jail`; serviced carries inline `jail_name/hostname/ip4/path` in `svc_manifest` | PID 1 assembles jailparams and creates jails — that's a jail broker's job. |
...
| **serviced** | Yes | storage coupling **REMOVED** (`5583478`): links no libtzfsd, no storage code (`cap_storage` is declaration-only). Remaining typed scaffolding is net/jail/vsock — **kernel** isolation mints via capsule, not leaf-daemon clients (Phase 4 generalizes them; still links libjail). Plane-conformant otherwise |
...
(a Mach port brokered by name) with kernel-enforced attenuation on top. Capsule
...
itself over that channel.** serviced and capsule never see a
...
serviced ── launch · deliver fds/channels · supervise · discovery · launch policy
   ...
   ▼
capsule ── kernel mints only (mac_capability) + generic delegate ROUTE
   │
...
**Target = (B) deliver-channel.** (A) broker-forward — serviced/capsule
...
- Add `CAPSULE_OP_DELEGATE` to `capsule_svc_proto.h`: `{uint32_t domain;
  uint32_t blob_len; uint8_t blob[]; }`, reply carries status + one optional fd.
- capsule gains a `domain → owning-daemon socket` route table (data, not
  per-op code). Start empty.
- serviced gains one `capsule_delegate(domain, blob)` client that returns
...
- **Root alone is not enough** — the mac_capability system gate denies even root
  ...
  ["kldload","kldstat"] }`; serviced mints the token (capsule claims the gates
...
- Extracted the mac_capability service-call layer into **libcapability**
  (`capability_service_connect/_call/_call_fds/_confine_fd`); capsule's
  `mac_capability_*` primitives now delegate to it.

Deleted: capsule `handle_ensure_kmod`/`CAPSULE_OP_ENSURE_KMOD`/
`capsule_kmod_req`/`validate_kmod_req`/kmod DTrace probe; serviced
...
- **capsule**: deleted `handle_create_jail`/`jail_set`/`existing_jail_
  descriptor`/`jail_path_allowed`/`CAPSULE_OP_CREATE_JAIL`/`-ljail`.  The
...
- Replace serviced's `fill_<domain>_req`/`capsule_mint_<domain>` family with a
...
> Transitional note: Phases 1–3 may land as **(A)** (serviced/capsule relay
...
capsule's per-domain **kernel** mints (`MINT_PATH/FILE/NET/VSOCK/JAIL`)
could fold behind a single generic isolation-mint. Low priority: they pull in
no daemon dependencies and are capsule's genuine role. Defer unless the
...
- Each leaf protocol (tzfsd storage, jail-create, kmod) is spoken **only**
  ...
  channel — never relayed through serviced or capsule (end-state **B**).
- serviced/capsule's currency is **names + capabilities + a bounds tuple**,
  exactly launchd's model — they interpret no per-daemon request content.
- Adding a new capability daemon touches **neither** capsule nor serviced.
