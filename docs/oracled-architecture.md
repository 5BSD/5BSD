...
After OracleD finishes initialization, the system is locked.
Root is no longer sufficient to modify critical system state.
Authority flows through capabilities -- file descriptors
minted by the Oracle and passed to the processes that need them.

### Default Hardened Configuration

Ships as `/etc/oracled.conf` and provides a reasonable secure
default.  Operators can add or remove claims based on their
deployment.
...
claims {
    ...
    paths = [
        ...
        # Boot — prevent kernel/bootloader/module replacement
        "/boot",
        ...
        # Root home — prevent SSH key injection
        "/root",
    ];
...
**Compromised root process**: A service running as root is
exploited.  The attacker has uid 0 but cannot load kernel
modules (rootkit), read `/dev/mem`, replace system binaries,
modify `/etc/master.passwd`, delete `/var/log`, reboot,
lower `kern.securelevel`, set `kenv init_path=/tmp/evil`,
bind port 22 (impersonate sshd), or inject SSH keys into
`/root/.ssh/authorized_keys`.

**Insider with root shell**: An administrator with legitimate
root access cannot perform destructive or unauthorized
operations without going through the Oracle's control interface.

**Malware persistence**: Malware that achieves root cannot
install itself in `/usr/sbin`, modify rc scripts, load a
kernel module to hide itself, or tamper with audit logs.

### What Hardening Does NOT Cover

- **Pre-OracleD state**: Anything that happened before OracleD
  started is not controlled (e.g., offline disk modification).
  Filesystem integrity verification (mtree, IMA) is complementary.
- **Already-open file descriptors**: A process that opened
  `/dev/mem` before OracleD claimed it retains the fd.
- **Already-bound sockets**: Rogue pre-claim binds persist.
- **Kernel exploits**: OracleD's MACF protections do not stop
  direct kernel memory corruption.
- **Physical access**: DMA, cold boot, offline disk modification
  require hardware security (IOMMU, FDE, secure boot).

### Startup Order

Services that start before OracleD have unrestricted access.
Services that start after are subject to claims.

**Must start before OracleD**:
- `init` -- PID 1
- `FILESYSTEMS` -- OracleD needs `/etc`, `/var/run`
- `syslogd` -- OracleD logs to syslog
- `devd` -- if cap_rt is a loadable module

**Should start after OracleD**: everything else.
...
**Adding claims for specific deployments** (e.g., database server):
