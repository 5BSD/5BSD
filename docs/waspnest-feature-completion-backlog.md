...
| New 5BSD driver work this cycle | virtio-sound has an in-tree driver and scheduled live lanes; virtio-mem, virtio-pmem, and virtio-IOMMU have build- and model-verified protocol prototypes in the -Werror module gate | Keep sound activation `pending` until its scheduled live Intel-host pass.  Keep mem, pmem, and IOMMU activation at `driver-gap` until they gain supported runtime memory-onlining, NVDIMM, and busdma endpoint paths respectively, plus corresponding live cases. |
