# Oracle as PID 1: Implementation TODO
...
what Oracle must retain, replace deliberately, test, and document.

## Objective

Make Oracle capable of replacing `init(8)` as PID 1 without losing FreeBSD's
boot compatibility, console recovery, process reaping, login-session
management, orderly shutdown, or reboot semantics.

This is not complete when Oracle can merely start `serviced`. It is complete
when Oracle remains a safe system spine through partial boot, configuration
...
- The system must retain a recovery console when Oracle configuration,
...
- [ ] Oracle daemonizes unless foreground mode is requested.
- [ ] Normal Oracle shutdown ends in `exit(0)`, which would panic the kernel
      when Oracle is PID 1.
- [ ] Oracle calls `PROC_REAP_ACQUIRE`; real init is already the permanent
      reaper, so that operation returns `EBUSY`.
- [ ] Oracle requires a pidfile under `/var/run`, even though `/var` may not be
      mounted during early boot.
- [ ] Oracle assumes syslog is available.
- [ ] Oracle requires `/dev/mac_capability` before it has performed early
      devfs and module bootstrap.
- [ ] The Oracle binary and its shared-library dependencies may reside under
      `/usr`, which may be a separate unmounted filesystem.
- [ ] Oracle has no single-user or emergency-console state.
- [ ] Oracle does not run `/etc/rc`, `/etc/rc.shutdown`, or `/etc/rc.final`.
- [ ] Oracle does not manage `/etc/ttys`, getty, or console login sessions.
- [ ] Oracle handles only a subset of the traditional PID 1 signal protocol,
...
- [ ] Oracle's subtree cleanup is not yet a complete global-system shutdown.
...
- [ ] Several Oracle fatal-error paths call `exit(3)` directly.
- [ ] Oracle does not support `-s`, `-f`, and `-r` with init-compatible boot
      meanings.
...
During migration, Oracle has two simultaneous responsibilities: it is the
...
- [ ] Assign every item in the companion audit to one of: preserve exactly,
      replace with a documented Oracle mechanism, or intentionally remove
      with a migration path.
- [ ] Maintain a requirement-to-test matrix keyed to the audit's numbered
      sections.
- [ ] Document the exact Oracle PID 1 state machine.
...
Use the existing `init_rc` loader variable to test an Oracle-aware startup
script while stock `init(8)` remains PID 1.

- [ ] Create an experimental Oracle rc entry point separate from `/etc/rc`.
- [ ] Preserve a path that runs the stock rc sequence.
- [ ] Ensure the rc script does not recursively start a second Oracle.
- [ ] Split trusted early boot from capability-world service startup.
- [ ] Identify the minimum rc providers required before Oracle can establish
      capability authority:
  ...
  - console access.
- [ ] Ensure no untrusted or capability-managed daemon starts before Oracle
...
## Phase 2: Introduce an Oracle PID 1 personality
...
- [ ] Preserve useful lifecycle tracing for boot, multi-user entry, shutdown,
      and reboot even if Oracle does not use stock `BOOTTRACE` macros.

### Suggested source structure

- [ ] Add a PID 1 module, for example `oracle_init.c` and `oracle_init.h`.
- [ ] Keep early console and recovery code independent of UCL and service
      manifests.
- [ ] Make the ordinary Oracle authority engine callable from the PID 1 state
...
- [ ] Install the PID 1-capable Oracle binary on the root filesystem, normally
      under `/sbin`.
...
- [ ] Assume that `init_exec` may enter Oracle before stock init has detected
...
- [ ] Verify MAC initialization gives PID 1 a valid initial nonce and that exec
      into Oracle rotates or establishes the intended program identity.
- [ ] Verify the complete early path works after `init_chroot`, and decide
      whether Oracle preserves `init_script`, `init_shell`, and `init_rc`
      loader-environment compatibility.

### Exit gate

- [ ] Oracle reaches recovery mode with `/usr` and `/var` unavailable.
- [ ] Oracle reaches recovery mode when `/dev/mac_capability` is unavailable.
...
- [ ] Do not mistake an individual rc.d command failure for a top-level rc
      ...
      Oracle's required multi-user target.
- [ ] Support autoboot and fastboot semantics.
- [ ] Prevent `/etc/rc` from starting an ordinary daemon instance of Oracle
      when Oracle is already PID 1.
- [ ] Initially let rc start every enabled legacy service.  Suppress an rc
      ...
      must not launch a second Oracle when Oracle is PID 1).
- [ ] Define the boundary between the Unix compatibility world and the Oracle
      capability world.
...
- [ ] Existing rc.d boot succeeds without duplicate Oracle instances.
...
Oracle must continue running the complete required rc sequence on every boot
...
- [ ] Have Oracle execute `/etc/rc` exactly once, with init-compatible
      `autoboot`/fastboot arguments.  Oracle must not reimplement the loop over
...
- [ ] Account for stock rc's non-fail-fast behavior.  A migrated adapter records
      failure with Oracle; dependent migrated adapters refuse to start without
      their provider; and Oracle validates the required multi-user target when
...
- [ ] Keep `service <name> start|stop|restart|reload|status` working throughout
      ...
      authorized Oracle/`serviced` control path rather than signaling a pidfile.
- [ ] Ensure adapter operations are idempotent so rc shutdown and Oracle's
...
- [ ] Replace `/etc/rc`'s firstboot `kill -INT 1` reboot request, and inventory
      equivalent requests in rc.d scripts, with the authorized Oracle
...
- [ ] Keep token activation in the executed program, never in Oracle or
...
- [ ] After rc.shutdown returns or times out, ask `serviced` to drain any
      ...
      adapter failure.
- [ ] Then terminate `serviced`, escalating through Oracle's exact retained
...
- [ ] An unmodified legacy service set boots and shuts down under Oracle PID 1.
...
- [ ] Provide recovery commands for:
  ...
  - retry rc startup;
  - inspect Oracle state;
  - disable optional policy temporarily;
  - reboot or halt safely;
  - select the fallback init for the next boot.
- [ ] Ensure recovery does not depend on `serviced` or the Oracle control
      socket.

### Exit gate

- [ ] Boot with malformed Oracle configuration reaches a usable console.
...
- [ ] Port the `/etc/ttys` session manager into Oracle's PID 1 personality; or
...
- [ ] Extend the Oracle control protocol with equivalent lifecycle operations.
- [ ] Update `shutdown(8)`, `reboot(8)`, and `halt(8)` to prefer the Oracle
      control protocol when Oracle is PID 1.
...
- [ ] Normal and forced shutdown tools work with Oracle's signal shield active.
...
- [ ] Decide explicitly whether `/etc/rc.final` retains stock init's unbounded
      wait or receives a documented Oracle deadline; test a hung final script.
...
- [ ] Shutdown order proves managed services exit before Oracle releases their
      capability authority.
...
- [ ] Use `init_exec=/sbin/oracle-init` only after recovery and shutdown work.
- [ ] Test direct kernel launch with an ordered fallback path such as:

  ```text
  /sbin/oracle-init:/sbin/init:/sbin/init.bak:/rescue/init
  ```
...
- [ ] Preserve the install-time backup behavior and verify that fallback
      entries are independently runnable rather than aliases of Oracle.
- [ ] Remember that fallback only helps if Oracle cannot be exec'd. Once Oracle
      starts successfully, a later PID 1 exit panics rather than trying the next
      path.
- [ ] Do not make Oracle the first default `init_path` entry until all VM boot
      gates pass.
...
- [ ] Serial console.
- [ ] Missing and malformed Oracle configuration.
...
- [ ] Firstboot reboot request with Oracle signal shielding active.
...
- [ ] Add an Oracle PID 1 mode section to `oracled(8)`.
...
- [ ] Document the distinction between normal Oracle daemon mode and PID 1
      mode.
...
2. Test an Oracle-aware `init_rc` under stock init.
...
14. Only then consider making Oracle the default init.

## Definition of done

Oracle may replace init only when all of the following are true:
