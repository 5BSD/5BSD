...
This book explains the stock FreeBSD mechanisms and then applies them to capability-oriented services such as Capsule. It is written for developers integrating base-system daemons, although most of it also applies to ports installed under `/usr/local/etc/rc.d`.
...
```sh
service capsule stop
```

it locates the `capsule` rc.d script, constructs a boot-like restricted environment, and invokes the script with `stop`. It does not inherently send a signal.
...
```sh
sysrc capsule_enable=YES
sysrc capsule_flags="-d"
```
...
This is the mechanism Capsule uses. The existence of `stop_cmd` prevents the generic stop branch from issuing `kill -TERM`.
...
If `examplectl shutdown` only reports that shutdown began, the rc method should wait for a completion condition. Capsule waits for its pidfile to disappear. A stronger design could wait on a process descriptor or an explicit control reply.
...
## 11. Capsule as a worked example

Capsule deliberately rejects foreign-nonce ambient PID signals. Its rc integration therefore replaces every standard operation that would otherwise depend on a signal.

The essential declarations are:

```sh
pidfile="/var/run/capsule.pid"
extra_commands="reload"
...
No signal from `service(8)` needs to pass Capsule's capability shield.
...
Capsule cannot use `kill -0` as an ambient liveness probe because signal zero goes through signal authorization. Status therefore queries the control socket. If a pidfile remains but the socket is absent, the script reports uncertainty rather than declaring the pidfile stale and deleting it.

### What remains impossible without a supervisor

If Capsule's control loop is permanently wedged, rc cannot force-kill it while all foreign PID signals are denied. This is not an rc defect; it is the chosen authority boundary.

The capability-correct emergency authority would be a process descriptor held by init or another supervisor. Until such a holder exists, normal `service capsule stop` works, while forced termination of a nonresponsive Capsule does not.
...
“Allow signals only from `service(8)`” is not a stable kernel authority model. `service` is a dispatcher that exits after invoking the script. The actual operation may be performed by `capsulectl`, shell code, or another helper.
...
```text
/usr/src/libexec/rc/rc.d/capsule
```
...
Likewise, building a new `capsule` under `/usr/obj` does not replace `/usr/sbin/capsule`.

Always establish which copy a test is exercising:

```sh
service -v capsule status
command -v capsule
ls -l /etc/rc.d/capsule /usr/sbin/capsule
```
...
```sh
service capsule extracommands
service capsule rcvar
service capsule describe
```

### Enable rc tracing

```sh
service -d capsule status
sysrc rc_debug=YES
```
...
```sh
sysrc capsule_enable
sysrc -a | grep '^capsule_'
service capsule rcvar
```
...
```sh
service -r | grep -n capsule
rcorder /etc/rc.d/* | grep -n capsule
```

### Inspect runtime artifacts

```sh
ls -l /var/run/capsule.pid /var/run/capsule.sock
capsulectl status
sockstat -u -l
ps -axww
```
...
For Capsule, that produces a precise answer:

- rc ordering places Capsule early enough to establish the capability plane;
- rc configuration determines whether and how it starts;
- custom status, reload, and stop methods use `capsulectl`;
- generic signal operations are never selected for those commands;
- Capsule remains free to deny ambient signals without breaking normal `service capsule stop`.
...
Capsule should execute `/etc/rc` as the one init-compatible child; it should not
...
names and pidfiles can be stale, ambiguous, or reused. Capsule should reject
...
Capsule, dependent managed adapters must refuse to start without their required
provider, and Capsule must validate its required multi-user target after rc
...
The stock firstboot path ends by signaling PID 1 with `SIGINT`. Before Capsule
enables mandatory signal shielding, that request and equivalent rc.d uses must
be converted to the authorized Capsule lifecycle operation.
