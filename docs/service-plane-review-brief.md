...
1. **Execute the service-architecture plan** (`service-architecture-plan.md`)
   Phases 1–6: move switchboard from a dependency-graph service manager to a
...
2. **Build a defensible service-discovery + management model** on top of that:
   every process inherits an unforgeable **ambient lookup channel** to switchboard;
...
(switchboard mints it and records the principal), and confinement is capability-
descriptor-based rather than ambient.

## 2. Key concepts (see the model doc for detail)

- **Ambient lookup channel** — a mac_capability channel to switchboard, inherited
...
|---|---|---|---|
| **capsule (PID 1)** | `usr.sbin/capsule/capsule.c`, `capsule_proto.c`, `capsule.h`, `Makefile` | Receives the ambient channel from switchboard (`CAPSULE_OP_SET_AMBIENT_LOOKUP`); spawns the console getty with the channel on fixed fd 3. | Root of the carry chain — every init-spawned login inherits the channel from here. Nothing downstream has a channel without it. |
...
| `8cf313844b6` | **Phase 4**: manager-owned listeners / socket activation — switchboard binds+holds listeners, connection = demand, delivered by logical name via the bootstrap `capabilities[]` array (`type="socket"`); backlog survives restart. **Touches the launch path for every service** (execute.c). |
| `b56ab8808af` | **§21** ambient channel reaches interactive logins: fixed-fd carry across capsule→getty→login; `CAPSULE_OP_SET_AMBIENT_LOOKUP`; fixed a real bug — `SVC_OP_MINT_DOMAIN` was only served on unit control channels, now on the ambient channel too. |
| `5e03d10b112` | **Management class** `core|system|user` — manifest field + switchboard enforcement (`management.c`); core unstoppable. |
...
| `832152c015d` | **rc adoption of cron** as a supervised `SVC_KIND_RC` unit; `switchboardctl restart`. Current startup probes `onestatus` to adopt an rc-started instance and runs `onestart` only when absent. Also fixed the RC **stop** path (rc daemons detach → must `service <name> onestop`, not pdkill the wrapper). |
| `1dc6aaa329e` | **socket-authenticated session provisioning** (`SCTL_OP_PROVISION_SESSION`): sshd asks switchboard over its `getpeereid`-authenticated control socket to mint a session channel. #30 rescoped: keep the socket (getpeereid is the correct userspace peer-cred primitive), don't delete it. **RETIRED (2026-08-30):** this mechanism is gone — `SCTL_OP_PROVISION_SESSION` and all three getpeereid control sockets have since been deleted; sshd now provisions over a minted per-connection SYSTEM channel, not a socket. This row is historical. |
| `f3875e4d124` | **capsule**: stop claiming `/etc/master.passwd` (and the demo path stubs) — a global isolation claim hid it from root too, breaking `pw`/`passwd`/`vipw`/`adduser`. |
...
delivery chain, a wedged switchboard hanging a login, a non-lookup channel spoofing
the ambient probe, and bounds/validation gaps in the wire protocols
(`switchboard_svc_proto.h`, `switchboard_ctl.h`, the monitor RPC).
...
- **Packaging correctness — AUDITED (pass), 2026-08-28.** The built `5BSD-*`
  ...
  - `5BSD-capsule` ships the boot integration a fresh install needs —
    `/boot/loader.conf.d/capsule.conf` (the `init_path` snippet), `/sbin/capsule`,
    `/usr/sbin/capsule`, the de-isolated `/etc/capsule.conf` — deps `switchboard` +
    `libcapability` + `libcapsulert`.
...
- **Boot-from-pkgbase-install — PROVEN, 2026-08-28.** A real `pkg`-installed root
  ...
  - `switchboardctl status` works over the ambient channel in both sessions.
...
- **Fixes landed + VALIDATED live, 2026-08-28** (task #39):
  - **A — permissive test mode**: `mac_capability_isolation.c` gains a boot-only
    ...
    /dev/mac_capability` now succeeds (was `Permission denied`), and the switchboard
    ...
    `capsule` (bootstrap cases) still can't run against a live PID 1 plane (it owns
...
session.c}`, `switchboard/sctl.c`, `libservice/service_client.c`, `switchboard/domain.c`;
(b) domain/mint/D1 — `switchboard/domain.c`, `switchboard/svc_proto.c`,
`libservice/service_ambient.c`; (c) fd-hygiene — `cron/do_command.c`,
`atrun/atrun.c`, `su.c`, `login.c`, `capsule.c`, `switchboard/startup.c`;
(d) launch/rc/mgmt — `switchboard/{execute.c,activation.c,switchboard.c,rc_adopt.c,
