# bsdfilesystem — the `[TZFS]` storage daemon

Status: **built** (designed 2026-08-14; daemon, `tzfsctl`, `libbsdfilesystem`,
...
`bsdfilesystem` is the system component that owns the **storage plane**. It takes the
...
but the capability storage plane assumes a ZFS pool is present). bsdfilesystem is not
optional; it comes up in the PID-1 boot chain before switchboard hands out
storage.

---

## 1. What bsdfilesystem owns

Two responsibilities, neither of which belongs in the service manager:

1. **The pool.** bsdfilesystem holds the root-pool handle (`zpd`) for the pool that
   backs `/Capabilities`. All dataset creation/destruction flows
   through capability handles derived from it — bsdfilesystem never shells out to
   `zfs(8)`/`zpool(8)`.
...
`libtrustedzfs` verb surface (snapshot, mount, props, send/recv, …). bsdfilesystem
...
## 3. Configuration — `/Capabilities/Config/bsdfilesystem.ucl`
...
Config is optional: with no file, bsdfilesystem uses the defaults above against the
first imported pool. The schema is deliberately shaped so switchboard-holds-it
(the old `handle_mint_storage`) and bsdfilesystem-holds-it are the *same* grant
...
- **`libbsdfilesystem`** (new) — the client to bsdfilesystem, function prefix `bsdfilesystem_`. What
...
### 4.1 `libbsdfilesystem` API

```c
/* Connect the bsdfilesystem socket (non-sandboxed callers); sandboxed callers pass
 * a channel fd delivered at bootstrap instead. Returns fd or -1. */
int bsdfilesystem_connect(void);
...
int bsdfilesystem_request(int chan, const struct bsdfilesystem_req *req,
...
struct bsdfilesystem_req {
    ...
    uint8_t   lifetime;        /* BSDFILESYSTEM_PERSISTENT | BSDFILESYSTEM_EPHEMERAL      */
};
struct bsdfilesystem_grant {
    int       handle_fd;       /* the zfd (SCM_RIGHTS from bsdfilesystem)         */
    char      dataset[256];    /* resolved dataset name (for audit)       */
};

int bsdfilesystem_release(int chan, const char *name);   /* ephemeral teardown   */
int bsdfilesystem_mount_dir(int handle_fd);              /* handle -> dirfd       */
```

A request is a claim on an existing (or created) plain dataset under the
bundle's layout.

### 4.2 Wire protocol (`bsdfilesystem_proto.h`)
...
Socket: `/var/run/bsdfilesystem.sock` (root-owned; peers are capsule/switchboard, and —
via a passed channel — sandboxed services). bsdfilesystem `cap_enter()`s after opening
...
- The `storage_open_handle` / `storage_create_ephemeral` / `storage_split`
  logic **moved into bsdfilesystem**. **Built.**
- capsule keeps `CAPSULE_OP_MINT_STORAGE` as a thin **forwarder** to bsdfilesystem via
  `libbsdfilesystem` (`handle_mint_storage`/`handle_destroy_storage` in
  `capsule_proto.c` now call `bsdfilesystem_request`/`bsdfilesystem_release`; capsule no longer
  ...
  self-serves storage on demand by opening the `system.Filesystem` broker (bsdfilesystem)
  by name — bsdfilesystem derives the dataset from the caller's unforgeable channel
  label. Capsule remains the one isolation authority for the kernel
  `mac_capability` claims; storage ownership lives wholly in bsdfilesystem.
- **Startup:** capsule starts bsdfilesystem on demand the first time a service needs
  storage (`posix_spawn` + `waitpid`; bsdfilesystem provisions synchronously then
  daemonizes, so the wait returns once it is ready). A boot-time
  `bsdfilesystem.ready` gate for services that need storage before first-mint is a
  Phase 4 refinement.

Services no longer declare storage in the manifest at all; a service self-mints
its dataset from `bsdfilesystem` at runtime, addressed by its own unforgeable channel
...
1. **Auto-provisioned layout** — bsdfilesystem lays down `/Capabilities` roots itself;
...
- **`usr.sbin/bsdfilesystem/`** — daemon (config, pool handle, layout,
  request loop), `[TZFS]` tagged.
- **`lib/libtzfs/`** — client library + `tzfs.h` + `libtzfs.3`.
- **`lib/libcapsulert/bsdfilesystem_proto.h`** — wire protocol (lives with the other
  capability protocols).
- **`usr.sbin/tzfsctl/`** — inspector (`list`, `request`).
- **Man pages:** `bsdfilesystem.8`, `tzfs.conf.5`, `libtzfs.3`, `tzfsctl.8`.
...
1. **Protocol + `libtzfs`** — `bsdfilesystem_proto.h`, client lib, plain-dataset path.
2. **Daemon core** — config, pool `zpd`, layout auto-provision, request loop,
   dataset create+mint, ephemeral teardown.
3. **Responsibility transfer** — switchboard → bsdfilesystem channel; capsule out of the
