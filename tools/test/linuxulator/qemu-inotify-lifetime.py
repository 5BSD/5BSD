...
import argparse
p=argparse.ArgumentParser(description='Run inotify lifetime tests in a disposable QEMU VM.')
...
cases = set("dup opens directory shared-map private-map late-watch hardlink replace remove-watch close-watches fork-exit".split())
rows = re.findall(r'^LIFETIME_RESULT (\d+) (\S+) (\S+) (\S+) (\d+)$', s, re.M)
expected = {(str(i), program, user, case, '0') for i in range(1, 6)
    for program in ['lifetime-linux', 'lifetime-native']
    for user in ['root', 'unprivileged'] for case in cases}
...
watch = re.findall(r'^LIFETIME_WATCHES (\d+) (\d+)$', s, re.M)
passed = (p.returncode == 0 and len(rows) == len(expected) and set(rows) == expected
    ...
    and 'LIFETIME_DONE' in s and 'LIFETIME_EXIT 0' in s
...
print('Inotify lifetime:', 'PASS' if passed else 'FAIL', len(rows), 'probes,', len(native), 'native cases')
