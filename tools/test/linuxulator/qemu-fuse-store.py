...
import argparse
p=argparse.ArgumentParser(description='Run Linux64 libfuse STORE tests in a disposable QEMU VM.')
...
rows=re.findall(r'^FUSE_STORE_ROUND ([1-5]) (0)$',s,re.M)
passed = (p.returncode == 0 and len(rows) == 5 and len(set(rows)) == 5
    and s.count('FUSE_STORE_PASS') == 5 and 'FUSE_STORE_DONE' in s
    and 'FUSE_STORE_EXIT 0' in s and 'Powering system off' in s
...
print('Linux FUSE STORE:', 'PASS' if passed else 'FAIL')
