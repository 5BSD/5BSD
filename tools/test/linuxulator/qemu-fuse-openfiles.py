...
import argparse
p=argparse.ArgumentParser(description='Run Linux64 libfuse open-description lifetime tests in a disposable QEMU VM.')
...
rows=re.findall(r'^OPENFILES_RESULT ([1-3]) (direct|cached|writeback) (0)$',s,re.M)
expected={(str(n), mode, '0') for n in range(1, 4) for mode in ['direct', 'cached', 'writeback']}
passed = (p.returncode == 0 and len(rows) == 9 and set(rows) == expected
    and s.count('FUSE_OPENFILES_PASS') == 9 and s.count('FUSE_STREAM_PASS') == 9
    and 'OPENFILES_DONE' in s
    and 'OPENFILES_EXIT 0' in s and 'Powering system off' in s
...
print('Linux FUSE open-description lifetime:', 'PASS' if passed else 'FAIL')
