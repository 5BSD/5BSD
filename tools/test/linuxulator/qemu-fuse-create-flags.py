...
import argparse
p=argparse.ArgumentParser(description='Run Linux64 libfuse CREATE flags tests in a disposable QEMU VM.')
...
rows=re.findall(r'^FUSE_FLAGS_ROUND ([1-5]) (0)$',s,re.M)
passed = (p.returncode == 0 and len(rows) == 5 and len(set(rows)) == 5
    and s.count('FUSE_FLAGS_PASS') == 10 and 'FUSE_FLAGS_DONE' in s
    and 'FUSE_FLAGS_EXIT 0' in s and 'Powering system off' in s
...
print('Linux FUSE CREATE flags:', 'PASS' if passed else 'FAIL')
