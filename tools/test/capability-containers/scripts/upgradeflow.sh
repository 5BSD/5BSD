...
V "zfs list $DS >/dev/null 2>&1 && echo UPGRADE_CONTAINER_KEPT_PASS || echo UPGRADE_CONTAINER_REAPED_FAIL; $(OBS $DS); n=\$(wc -l < /mnt/obs/marker 2>/dev/null | tr -d ' '); echo LAUNCHES_AFTER_UPGRADE=\$n; [ \"\$n\" = 2 ] && echo UPGRADE_DATA_SURVIVED_PASS || echo UPGRADE_DATA_LOST_FAIL; $(DROP_OBS $DS)" 50
...
V "zfs list $DS >/dev/null 2>&1 && echo REBOOT_CONTAINER_PRESERVED_PASS || echo REBOOT_CONTAINER_REAPED_FAIL; $(OBS $DS); n=\$(wc -l < /mnt/obs/marker 2>/dev/null | tr -d ' '); echo LAUNCHES_AFTER_REBOOT=\$n; [ \"\$n\" -ge 2 ] && echo REBOOT_DATA_PRESERVED_PASS || echo REBOOT_DATA_LOST_FAIL; $(DROP_OBS $DS)" 50
