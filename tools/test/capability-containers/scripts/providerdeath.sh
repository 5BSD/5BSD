#!/bin/sh
# Provider death: system.Filesystem (bsdfilesystem) is killed under running units.
#  - the stores stay mounted (bsdfilesystem's per-connection workers are plain
#    children and outlive it; the kernel keeps the mount while anchors exist):
#    the reader keeps reading its shared env;
#  - switchboard relaunches bsdfilesystem (restart=on-failure) and a bundle installed
#    afterwards claims its container from the new instance;
#  - no bsdfilesystem processes leak (the old reconcile child must not linger).
# The shipped bsdfilesystem manifest shields SIGKILL; the test drops that shield in
...
TU=$R/Capabilities/System/Filesystem.cap/Units/bsdfilesystem.unit/Unit.ucl
rm -f "$WORK/bsdfilesystem.Unit.ucl.orig"; cp "$TU" "$WORK/bsdfilesystem.Unit.ucl.orig"; trap 'cp "$WORK/bsdfilesystem.Unit.ucl.orig" "$TU"' EXIT
...
grep -q '^protect = \[\];' "$TU" && ! grep -q '"sigkill"' "$TU" || { echo "could not unshield bsdfilesystem"; exit 2; }
chmod u+w "$R/METALOG"; sed -i '' 's#^\(\./Capabilities/System/Filesystem.cap/Units/bsdfilesystem.unit/Unit.ucl type=file.*\) size=[0-9]*#\1#' "$R/METALOG"	# makefs truncates to the spec size: drop it
...
V "sleep 12; pgrep -f '[/ ]envreader( |\$)' >/dev/null && echo READER_UP; pgrep -xc bsdfilesystem | sed 's/^/BSDFILESYSTEM_PROCS_BEFORE=/'; pgrep -x bsdfilesystem | sort -n | tr '\n' ' '; echo; mount | grep -c 'Data/Env/shared' | sed 's/^/ENV_MOUNTS=/'" 30
...
echo "==> kill -9 bsdfilesystem (the provider), then watch the units and the relaunch"
V "p=\$(pgrep -x bsdfilesystem | sort -n | head -1); ps -o ppid= -p \$p | sed 's/^/BSDFILESYSTEM_PPID=/'; kill -9 \$p && echo KILLED_\$p || echo KILL_DENIED_FAIL; sleep 8; q=\$(pgrep -x bsdfilesystem | sort -n | head -1); echo NEW_BSDFILESYSTEM=\$q; [ -n \"\$q\" ] && [ \"\$q\" != \"\$p\" ] && echo BSDFILESYSTEM_RELAUNCHED_PASS || echo BSDFILESYSTEM_NOT_RELAUNCHED_FAIL; grep -a 'bsdfilesystem\|Filesystem' /var/log/messages | tail -4 | cut -c1-150" 40
...
echo "==> a bundle installed AFTER the relaunch claims from the new bsdfilesystem"
...
V "sleep 3; pgrep -xc bsdfilesystem | sed 's/^/BSDFILESYSTEM_PROCS_AFTER=/'; ps ax -o pid,ppid,command | grep '[t]zfsd' | cut -c1-100" 20
