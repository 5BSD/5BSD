#!/bin/sh
# Container-model proof: bsdextension as a reconcile client.  Kernel modules loaded
...
# bsdextension itself loaded, only when no other bundle still claims them, and a
# busy module is left loaded and retried.  A reboot resets the map (modules do
# not survive one).
#
#   M2 (boot):           geom_nop if_edsc nullfs
#   M3 (boot):           if_disc
#   M1 (installed late): geom_nop (found loaded: M2's load, so bsdextension's)
#                        zfs      (found loaded: the loader's, never bsdextension's)
#
...
# afterwards) and bsdextension's timer/grace is shortened to 15s through its unit
...
scrub_stage
SU=$R/Capabilities/System/SystemExtension.cap/Units/bsdextension.unit/Unit.ucl
SC=$R/Capabilities/Config/bsdextension.ucl
rm -f "$WORK/bsdextension.Unit.ucl.orig" "$WORK/bsdextension.conf.orig"
cp "$SU" "$WORK/bsdextension.Unit.ucl.orig"; cp "$SC" "$WORK/bsdextension.conf.orig"
trap 'chmod u+w "$SU" "$SC"; cp "$WORK/bsdextension.Unit.ucl.orig" "$SU"; cp "$WORK/bsdextension.conf.orig" "$SC"' EXIT
chmod u+w "$SU" "$SC" "$R/METALOG"
grep -q '^environment' "$SU" || printf 'environment { BSDEXTENSION_RECLAIM_INTERVAL = "15"; }\n' >> "$SU"
sed -i '' 's#^\(\./Capabilities/System/SystemExtension.cap/Units/bsdextension.unit/Unit.ucl type=file.*\) size=[0-9]*#\1#' "$R/METALOG"
cat > "$SC" <<'EOF'
...
build_image mr
boot rw || exit 1
MAP=/var/run/bsdextension/modules.meta
...
echo "=== M1 installed late: geom_nop found loaded (bsdextension's, through M2), zfs found loaded (the loader's, not bsdextension's) ==="
...
echo "=== M1 uninstalled: geom_nop stays (M2 still claims it), zfs stays (not bsdextension's), M1's attributions go ==="
V "chmod -R u+w /Capabilities/System/M1.cap; rm -rf /Capabilities/System/M1.cap; sleep 50; grep -a 'bsdextension.*reclaim' /var/log/messages | tail -5 | cut -c1-170; echo '--- map:'; cat $MAP" 70
V "kldstat -q -n geom_nop && echo M1_SHARED_KEPT_PASS || echo M1_SHARED_UNLOADED_FAIL; grep -aq 'geom_nop (bundle M1): still claimed by another bundle' /var/log/messages && echo M1_SHARED_LOGGED_PASS || echo M1_SHARED_NOT_LOGGED_FAIL; kldstat -q -n zfs && grep -aq 'zfs (bundle M1): found already loaded, not bsdextension' /var/log/messages && echo M1_FOREIGN_KEPT_PASS || echo M1_FOREIGN_FAIL; grep -q ' M1 ' $MAP && echo M1_ATTRIBUTION_STUCK_FAIL || echo M1_ATTRIBUTION_DROPPED_PASS" 20

echo "=== M2 uninstalled while nullfs is busy (a mount): geom_nop and if_edsc unload, nullfs is kept and retried ==="
V "mkdir -p /mnt/nb && mount -t nullfs /tmp /mnt/nb && mount | grep -c '/mnt/nb' | sed 's/^/NULLFS_MOUNTS=/'; chmod -R u+w /Capabilities/System/M2.cap; rm -rf /Capabilities/System/M2.cap; sleep 50; grep -a 'bsdextension.*reclaim' /var/log/messages | tail -5 | cut -c1-170; echo '--- map:'; cat $MAP" 80
...
V "umount /mnt/nb; sleep 40; grep -a 'bsdextension.*reclaim' /var/log/messages | tail -3 | cut -c1-170; echo '--- map:'; cat $MAP" 60
