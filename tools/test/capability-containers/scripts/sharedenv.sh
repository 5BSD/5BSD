...
echo "=== crash-restart churn: the reader (restart=always) is killed 3x; each relaunch must re-claim (both its state store and the env, remounted after the last-holder teardown) before the next kill ==="
V "D=$RD; c0=\$(grep -c ENV_CLAIMED /mnt/obs/result); k=0; c=\$c0; lat=; for i in 1 2 3; do pgrep -f '[/ ]envreader( |\$)' >/dev/null || break; pkill -f '[/ ]envreader( |\$)'; k=\$((k+1)); j=0; while [ \$j -lt 45 ]; do sleep 1; j=\$((j+1)); $RES >/dev/null 2>&1; n=\$(grep -c ENV_CLAIMED /mnt/obs/result); [ \$n -gt \$c ] && break; done; lat=\"\$lat \$j\"; c=\$n; done; echo KILLS=\$k RECLAIM_SECS=\$lat; c1=\$c; echo CLAIMS_BEFORE=\$c0 CLAIMS_AFTER=\$c1; [ \$c1 -ge \$((c0+3)) ] && echo CHURN_RECLAIMS_PASS || echo CHURN_RECLAIM_FAIL; grep -q 'ENV_CLAIM_FAILED\|ENV_CLAIM_ERROR' /mnt/obs/result && echo CHURN_CLAIM_ERROR_FAIL; pgrep -f '[/ ]envreader( |\$)' >/dev/null && echo READER_BACK_PASS || echo READER_DOWN_FAIL; [ \"\$(mount | grep -c 'Data/Env/shared/persistent/env')\" = 1 ] && echo CHURN_ONE_MOUNT_PASS || echo CHURN_MOUNT_COUNT_FAIL; grep -a 'busy retr' /var/log/messages | tail -2 | cut -c1-120; tail -5 /mnt/obs/result" 200
