 ...
 *
 * io_uring UAPI + kernel KPI for the 5BSD native io_uring subsystem.
 * The UAPI section is transcribed from Linux include/uapi/linux/io_uring.h
 * to guarantee a byte-identical ABI; both the native syscalls and the
 * Linuxulator front-end use these definitions.  See
 * docs/linuxulator-io_uring-design.md.
 */
#ifndef _SYS_IO_URING_H_
#define	_SYS_IO_URING_H_

#include <sys/types.h>

/* Linux uapi flex-array-in-union helper (uapi/linux/stddef.h), same layout. */
#ifndef __DECLARE_FLEX_ARRAY
#define	__DECLARE_FLEX_ARRAY(TYPE, NAME)	\
	struct { struct { } __empty_ ## NAME; TYPE NAME[]; }
#endif

/* Linux 64-bit timespec used by io_uring EXT_ARG / reg-wait. */
...
struct io_uring_sqe {
	...
	union {
		...
		struct {
			uint64_t	attr_ptr; /* pointer to attribute information */
			uint64_t	attr_type_mask; /* bit mask of attributes */
		};
		uint64_t	optval;
		/*
 ...
 */
#define SPLICE_F_FD_IN_FIXED	(1U << 31) /* the last bit of uint32_t */
...
struct io_uring_files_update {
	uint32_t offset;
	uint32_t resv;
	uint64_t __aligned(8) /* int32_t * */ fds;
};
...
struct io_uring_mem_region_reg {
	uint64_t region_uptr; /* struct io_uring_region_desc * */
	uint64_t flags;
	uint64_t __resv[2];
};
...
struct io_uring_rsrc_register {
	...
	uint32_t flags;
	uint64_t resv2;
	uint64_t __aligned(8) data;
	uint64_t __aligned(8) tags;
};
...
#define IORING_CQE_F_TSTAMP_HW		((uint32_t)1 << IORING_TIMESTAMP_HW_SHIFT)
