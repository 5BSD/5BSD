 ...
 *
 * mac_capability_mount — capability-based filesystem mounting.
 *
 * Allows a Capsicum-sandboxed init daemon to mount filesystems within
 * its jail via MAC_CAPABILITY_CALL.  The co_call runs in the caller's thread,
...
#include "mac_capability_mount_proto.h"

SDT_PROVIDER_DEFINE(mac_capability_mount);
SDT_PROBE_DEFINE6(mac_capability_mount, , , state,
    "const char *", "const char *", "const char *", "uint32_t", "pid_t", "int");
SDT_PROBE_DEFINE3(mac_capability_mount, , , deny,
    ...
    void *reply, size_t *replylenp)
{
	...
	*replylenp = sizeof(*rp);
	...
	if (!mount_fstype_allowed(fstype)) {
		...
		SDT_PROBE3(mac_capability_mount, , , deny, (uintptr_t)"mount", (uintptr_t)fstype, (uintptr_t)fspath);
		return (0);
	}
	...
	if (strcmp(fstype, "nullfs") == 0 &&
		...
		rp->status = MOUNT_STATUS_ERR;
		SDT_PROBE3(mac_capability_mount, , , deny, (uintptr_t)"mount", (uintptr_t)fstype, (uintptr_t)from);
		return (0);
	}
	...
	SDT_PROBE6(mac_capability_mount, , , state, (uintptr_t)"mount", (uintptr_t)fstype, (uintptr_t)fspath,
    ...
    void *reply, size_t *replylenp)
{
	...
	*replylenp = sizeof(*rp);
	...
	if (!mount_path_valid(fspath, sizeof(fspath))) {
		memset(rp, 0, sizeof(*rp));
		rp->status = MOUNT_STATUS_ERR;
		SDT_PROBE3(mac_capability_mount, , , deny, (uintptr_t)"unmount", (uintptr_t)"", (uintptr_t)fspath);
		return (0);
	}
	...
	memset(rp, 0, sizeof(*rp));
	rp->status = mount_error_status(error);
	SDT_PROBE6(mac_capability_mount, , , state, (uintptr_t)"unmount", (uintptr_t)"", (uintptr_t)fspath,
	    rp->status, curthread->td_proc->p_pid, error);
	return (0);
}
...
static int
mount_call(struct mac_capability_instance *s __unused,
...
static struct mac_capability_ops mount_ops = {
	.co_call = mount_call,
};

static struct mac_capability_service *mount_svc;

static int
mac_capability_mount_modevent(module_t mod __unused, int type, void *data __unused)
{
	...
	case MOD_LOAD:
		...
		error = mac_capability_service_create(&params, &mount_svc);
		if (error != 0)
			printf("mac_capability_mount: create failed: %d\n", error);
		return (error);
	case MOD_UNLOAD:
		return (EBUSY);
	default:
...
static moduledata_t mac_capability_mount_mod = {
	"mac_capability_mount",
	mac_capability_mount_modevent,
	NULL,
};

DECLARE_MODULE(mac_capability_mount, mac_capability_mount_mod, SI_SUB_PSEUDO, SI_ORDER_ANY);
MODULE_DEPEND(mac_capability_mount, mac_capability, 1, 1, 1);
MODULE_VERSION(mac_capability_mount, 1);
