 ...
 *
 * mac_capability_node — capability service for process observation and control.
 *
 * Provides get/set operations on a process via MAC_CAPABILITY_CALL with an
...
#include "mac_capability_node_proto.h"

SDT_PROVIDER_DEFINE(mac_capability_node);
SDT_PROBE_DEFINE6(mac_capability_node, , , call__done,
    "uint32_t", "pid_t", "pid_t", "uint32_t", "int", "sbintime_t");
SDT_PROBE_DEFINE3(mac_capability_node, , , deny,
...
node_op_cred(struct proc *p, void *reply, size_t *replylenp)
{
	...
	rp->nonce = mac_capability_proc_nonce(cr);
    ...
    void *reply, size_t *replylenp)
{
	...
	*replylenp = sizeof(*rp);
	...
	if (error == 0) {
		...
		SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"set-rlimit",
    ...
    void *reply, size_t *replylenp)
{
	...
	if (rp->status != NODE_STATUS_OK)
		SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"reap",
		    p->p_pid, curthread->td_proc->p_pid);
    ...
    void *reply, size_t *replylenp)
{
	...
	*replylenp = sizeof(*rp);
	...
	if (error == 0) {
		...
		rp->status = NODE_STATUS_ERR;
		SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"set-affinity",
		    p->p_pid, curthread->td_proc->p_pid);
	}
	...
	*replylenp = sizeof(*rp);
	...
	if (error == 0) {
		...
		if (rp->status == NODE_STATUS_EPERM)
			SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"set-cred",
			    p->p_pid, curthread->td_proc->p_pid);
	}
...
again_sess:
	...
	if (error == 0) {
		...
		rp->status = NODE_STATUS_EPERM;
		SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"set-session",
		    p->p_pid, curthread->td_proc->p_pid);
	}
...
again_pgrp:
	...
	if (error == 0) {
		...
		rp->status = NODE_STATUS_EPERM;
		SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"set-pgrp",
		    p->p_pid, curthread->td_proc->p_pid);
	}
    ...
    void *reply, size_t *replylenp)
{
	...
	*replylenp = sizeof(*rp);
	...
	if (error != 0) {
		rp->status = NODE_STATUS_EPERM;
		SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"signal",
		    p->p_pid, curthread->td_proc->p_pid);
		return (0);
	}
...
static int
node_call(struct mac_capability_instance *s __unused,
    const void *req, size_t reqlen,
    ...
    void *arg __unused)
{
	...
	if (error == ESRCH) {
		if (*replylenp >= sizeof(uint32_t)) {
			...
			*replylenp = sizeof(uint32_t);
			SDT_PROBE3(mac_capability_node, , , deny, (uintptr_t)"resolve-proc",
			    0, curthread->td_proc->p_pid);
			SDT_PROBE6(mac_capability_node, , , call__done, nr->op,
			    0, curthread->td_proc->p_pid, NODE_STATUS_DEAD,
			    0, getsbinuptime() - start);
			return (0);
		}
		SDT_PROBE6(mac_capability_node, , , call__done, nr->op, 0,
	...
	_PRELE(p);
	PROC_UNLOCK(p);
	SDT_PROBE6(mac_capability_node, , , call__done, nr->op, target_pid,
...
static struct mac_capability_ops node_ops = {
	.co_call = node_call,
};

static struct mac_capability_service *node_svc;

static int
mac_capability_node_modevent(module_t mod __unused, int type, void *data __unused)
{
	...
	case MOD_LOAD:
		...
		params.name = "node";
		params.ops = &node_ops;
		error = mac_capability_service_create(&params, &node_svc);
		if (error != 0)
			printf("mac_capability_node: create failed: %d\n", error);
		return (error);
	case MOD_UNLOAD:
		if (node_svc != NULL)
			mac_capability_service_destroy(node_svc);
		return (0);
	default:
		return (EOPNOTSUPP);
	}
...
static moduledata_t mac_capability_node_mod = {
	"mac_capability_node",
	mac_capability_node_modevent,
	NULL,
};

DECLARE_MODULE(mac_capability_node, mac_capability_node_mod, SI_SUB_PSEUDO, SI_ORDER_ANY);
MODULE_DEPEND(mac_capability_node, mac_capability, 1, 1, 1);
MODULE_VERSION(mac_capability_node, 1);
