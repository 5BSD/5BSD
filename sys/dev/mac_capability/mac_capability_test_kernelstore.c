 ...
 *
 * mac_capability_test_kernelstore — test fixture: shared key-value store
 *
 * Sync-only MAC_CAPABILITY service.  Connecting creates a new empty store.
 ...
 * MAC_CAPABILITY_TERMINATE on that fd to revoke the member's access.
 *
...
MALLOC_DEFINE(M_MAC_CAPABILITY_KSTORE, "mac_capability_test_kstore", "mac_capability test kernelstore");
...
static struct mac_capability_service *kstore_svc;
...
ks_store_alloc(void)
{
	...
	store = malloc(sizeof(*store), M_MAC_CAPABILITY_KSTORE, M_WAITOK | M_ZERO);
	mtx_init(&store->ks_mtx, "mac_capability_kstore", NULL, MTX_DEF);
...
ks_store_rele(struct ks_store *store)
{
	...
	while ((ke = LIST_FIRST(&store->ks_entries)) != NULL) {
		...
		free(ke->ke_data, M_MAC_CAPABILITY_KSTORE);
		free(ke, M_MAC_CAPABILITY_KSTORE);
	}
	mtx_destroy(&store->ks_mtx);
	free(store, M_MAC_CAPABILITY_KSTORE);
}
    ...
    uint64_t *badge_out)
{
	...
	return (MAC_CAPABILITY_CONNECT_BADGE(kstore_next_badge, badge_out));
}

static int
kstore_init(struct mac_capability_instance *s, void *arg __unused)
{
	struct ks_priv *kp;

	kp = malloc(sizeof(*kp), M_MAC_CAPABILITY_KSTORE, M_WAITOK | M_ZERO);
	kp->kp_role = KS_ROLE_OWNER;
	kp->kp_store = ks_store_alloc();
	mac_capability_instance_set_priv(s, kp);
	return (0);
}
    ...
    void *arg __unused)
{
	...
	kp = mac_capability_instance_get_priv(s);
	if (kp == NULL)
		return (EINVAL);
	store = kp->kp_store;
	...
	case KSTORE_OP_PUT: {
		...
		newdata = NULL;
		if (vallen > 0) {
			newdata = malloc(vallen, M_MAC_CAPABILITY_KSTORE, M_WAITOK);
		...
		if (ke != NULL) {
			...
			free(olddata, M_MAC_CAPABILITY_KSTORE);
		} else {
			if (store->ks_nentries >= KSTORE_MAX_KEYS) {
				mtx_unlock(&store->ks_mtx);
				free(newdata, M_MAC_CAPABILITY_KSTORE);
				sr = (struct kstore_status_reply *)reply;
				sr->status = KSTORE_STATUS_FULL;
				*replylenp = sizeof(*sr);
				return (0);
			}
			ke = malloc(sizeof(*ke), M_MAC_CAPABILITY_KSTORE,
			    M_NOWAIT | M_ZERO);
			if (ke == NULL) {
				mtx_unlock(&store->ks_mtx);
				free(newdata, M_MAC_CAPABILITY_KSTORE);
	...
	case KSTORE_OP_GET: {
		...
		valbuf = NULL;
		if (vallen > 0) {
			valbuf = malloc(vallen, M_MAC_CAPABILITY_KSTORE, M_WAITOK);
			mtx_lock(&store->ks_mtx);
			ke = ks_find(store, key);
			if (ke == NULL || ke->ke_datalen != vallen) {
				mtx_unlock(&store->ks_mtx);
				free(valbuf, M_MAC_CAPABILITY_KSTORE);
				sr = (struct kstore_status_reply *)reply;
				sr->status = KSTORE_STATUS_NOTFOUND;
				*replylenp = sizeof(*sr);
				return (0);
	...
	case KSTORE_OP_MINT: {
		...
		error = mac_capability_mint_fp(kstore_svc, 0, &member_fp);
		if (error != 0)
			return (error);

		mkp = mac_capability_instance_get_priv(member_fp->f_data);
		if (mkp != NULL) {
...
static void
kstore_revoke(struct mac_capability_instance *s, uint64_t badge __unused,
    enum mac_capability_revoke_reason reason __unused, void *arg __unused)
{
	...
	free(kp, M_MAC_CAPABILITY_KSTORE);
}

static const struct mac_capability_ops kstore_ops = {
...
static int
mac_capability_test_kernelstore_modevent(module_t mod __unused, int type,
	...
	case MOD_LOAD:
		{
			struct mac_capability_service_params p = {
				.name = "test_kernelstore",
				.ops = &kstore_ops,
			};
			error = mac_capability_service_create(&p, &kstore_svc);
		}
		if (error != 0)
			return (error);
		if (bootverbose)
			printf("mac_capability_test_kernelstore: loaded\n");
		return (0);

	case MOD_UNLOAD:
		if (kstore_svc != NULL)
			mac_capability_service_destroy(kstore_svc);
		if (bootverbose)
			printf("mac_capability_test_kernelstore: unloaded\n");
		return (0);
...
static moduledata_t mac_capability_test_kernelstore_mod = {
	"mac_capability_test_kernelstore",
	mac_capability_test_kernelstore_modevent,
	NULL,
};

DECLARE_MODULE(mac_capability_test_kernelstore, mac_capability_test_kernelstore_mod, SI_SUB_PSEUDO,
    SI_ORDER_ANY);
MODULE_VERSION(mac_capability_test_kernelstore, 1);
MODULE_DEPEND(mac_capability_test_kernelstore, mac_capability, 1, 1, 1);
