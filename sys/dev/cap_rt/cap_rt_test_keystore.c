 ...
 *
 * cap_rt_test_keystore — test fixture: async key-value store
 *
...
MALLOC_DEFINE(M_CAP_RT_KS, "cap_rt_test_ks", "cap_rt test keystore");
...
static struct cap_rt_service *ks_svc;
...
static int
keystore_handler(struct cap_rt_instance *s, const struct cap_rt_msg *msg,
    void *arg __unused)
{
	...
	if (cap_rt_msg_datalen(msg) < sizeof(struct ks_request)) {
		reply_hdr.status = KS_STATUS_ERR;
		cap_rt_reply(s, cap_rt_msg_token(msg), &reply_hdr,
		    sizeof(reply_hdr), NULL, NULL, 0);
		return (0);
	}

	req = (const struct ks_request *)cap_rt_msg_data(msg);
	uid = cap_rt_msg_cred(msg)->cr_uid;
	...
	case KS_OP_STORE: {
		...
		datalen = cap_rt_msg_datalen(msg) - sizeof(struct ks_request);
		...
		newdata = NULL;
		if (datalen > 0) {
			newdata = malloc(datalen, M_CAP_RT_KS, M_WAITOK);
			memcpy(newdata,
			    (const uint8_t *)cap_rt_msg_data(msg) +
			    sizeof(struct ks_request), datalen);
		}
		newke = malloc(sizeof(*newke), M_CAP_RT_KS,
		    M_WAITOK | M_ZERO);
		...
		if (ke != NULL) {
			...
			mtx_unlock(&ks_mtx);
			free(olddata, M_CAP_RT_KS);
			free(newke, M_CAP_RT_KS);
		} else {
			if (ks_nentries >= KS_MAX_KEYS) {
				mtx_unlock(&ks_mtx);
				free(newdata, M_CAP_RT_KS);
				free(newke, M_CAP_RT_KS);
				reply_hdr.status = KS_STATUS_ERR;
				cap_rt_reply(s, cap_rt_msg_token(msg), &reply_hdr,
				    sizeof(reply_hdr), NULL, NULL, 0);
	...
	case KS_OP_FETCH: {
		...
		valbuf = NULL;
		if (vallen > 0) {
			valbuf = malloc(vallen, M_CAP_RT_KS, M_WAITOK);
			mtx_lock(&ks_mtx);
			ke = ks_find(uid, req->keyid);
			if (ke == NULL || ke->ke_datalen < vallen) {
				mtx_unlock(&ks_mtx);
				free(valbuf, M_CAP_RT_KS);
				reply_hdr.status = KS_STATUS_NOTFOUND;
				cap_rt_reply(s, cap_rt_msg_token(msg), &reply_hdr,
				    sizeof(reply_hdr), NULL, NULL, 0);
				return (0);
		...
		reply_buf = malloc(reply_len, M_CAP_RT_KS, M_WAITOK);
		((struct ks_reply *)reply_buf)->status = KS_STATUS_OK;
		if (vallen > 0)
			memcpy(reply_buf + sizeof(struct ks_reply),
			    valbuf, vallen);

		cap_rt_reply(s, cap_rt_msg_token(msg), reply_buf,
		    reply_len, NULL, NULL, 0);
		free(reply_buf, M_CAP_RT_KS);
		free(valbuf, M_CAP_RT_KS);
		return (0);
	}
...
static const struct cap_rt_ops keystore_ops = {
...
static int
cap_rt_test_keystore_modevent(module_t mod __unused, int type,
    void *unused __unused)
{
	...
	switch (type) {
	case MOD_LOAD:
		mtx_init(&ks_mtx, "cap_rt_test_ks", NULL, MTX_DEF);
		{
			struct cap_rt_service_params p = {
				.name = "test_keystore",
				.ops = &keystore_ops,
			};
			error = cap_rt_service_create(&p, &ks_svc);
		}
		if (error != 0) {
			mtx_destroy(&ks_mtx);
			return (error);
...
static moduledata_t cap_rt_test_keystore_mod = {
	"cap_rt_test_keystore",
	cap_rt_test_keystore_modevent,
	NULL,
};

DECLARE_MODULE(cap_rt_test_keystore, cap_rt_test_keystore_mod, SI_SUB_PSEUDO,
    SI_ORDER_ANY);
MODULE_VERSION(cap_rt_test_keystore, 1);
MODULE_DEPEND(cap_rt_test_keystore, cap_rt, 1, 1, 1);
