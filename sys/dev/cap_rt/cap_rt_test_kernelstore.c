 ...
 *
 * cap_rt_test_kernelstore — test fixture: shared key-value store
 *
 * Sync-only CAP_RT service.  Connecting creates a new empty store.
 ...
 * CAP_RT_TERMINATE on that fd to revoke the member's access.
 *
...
MALLOC_DEFINE(M_CAP_RT_KSTORE, "cap_rt_test_kstore", "cap_rt test kernelstore");
...
static struct cap_rt_service *kstore_svc;
...
ks_store_alloc(void)
{
	...
	store = malloc(sizeof(*store), M_CAP_RT_KSTORE, M_WAITOK | M_ZERO);
	mtx_init(&store->ks_mtx, "cap_rt_kstore", NULL, MTX_DEF);
...
ks_store_rele(struct ks_store *store)
{
	...
	while ((ke = LIST_FIRST(&store->ks_entries)) != NULL) {
		...
		free(ke->ke_data, M_CAP_RT_KSTORE);
		free(ke, M_CAP_RT_KSTORE);
	}
	mtx_destroy(&store->ks_mtx);
	free(store, M_CAP_RT_KSTORE);
}
    ...
    uint64_t *badge_out)
{
	...
	return (CAP_RT_CONNECT_BADGE(kstore_next_badge, badge_out));
}

static int
kstore_init(struct cap_rt_instance *s, void *arg __unused)
{
	struct ks_priv *kp;

	kp = malloc(sizeof(*kp), M_CAP_RT_KSTORE, M_WAITOK | M_ZERO);
	kp->kp_role = KS_ROLE_OWNER;
	kp->kp_store = ks_store_alloc();
	cap_rt_instance_set_priv(s, kp);
	return (0);
}

static int
kstore_call(struct cap_rt_instance *s,
    ...
    void *arg __unused)
{
	...
	case KSTORE_OP_PUT: {
		...
		newdata = NULL;
		if (vallen > 0) {
			newdata = malloc(vallen, M_CAP_RT_KSTORE, M_WAITOK);
		...
		if (ke != NULL) {
			...
			free(olddata, M_CAP_RT_KSTORE);
		} else {
			if (store->ks_nentries >= KSTORE_MAX_KEYS) {
				mtx_unlock(&store->ks_mtx);
				free(newdata, M_CAP_RT_KSTORE);
				sr = (struct kstore_status_reply *)reply;
				sr->status = KSTORE_STATUS_FULL;
				*replylenp = sizeof(*sr);
				return (0);
			}
			ke = malloc(sizeof(*ke), M_CAP_RT_KSTORE,
			    M_NOWAIT | M_ZERO);
			if (ke == NULL) {
				mtx_unlock(&store->ks_mtx);
				free(newdata, M_CAP_RT_KSTORE);
	...
	case KSTORE_OP_GET: {
		...
		valbuf = NULL;
		if (vallen > 0) {
			valbuf = malloc(vallen, M_CAP_RT_KSTORE, M_WAITOK);
			mtx_lock(&store->ks_mtx);
			ke = ks_find(store, key);
			if (ke == NULL || ke->ke_datalen != vallen) {
				mtx_unlock(&store->ks_mtx);
				free(valbuf, M_CAP_RT_KSTORE);
				sr = (struct kstore_status_reply *)reply;
				sr->status = KSTORE_STATUS_NOTFOUND;
				*replylenp = sizeof(*sr);
				return (0);
	...
	case KSTORE_OP_MINT: {
		...
		error = cap_rt_mint_fp(kstore_svc, 0, &member_fp);
		if (error != 0)
			return (error);

		mkp = cap_rt_instance_get_priv(member_fp->f_data);
		if (mkp != NULL) {
...
static void
kstore_revoke(struct cap_rt_instance *s, uint64_t badge __unused,
    enum cap_rt_revoke_reason reason __unused, void *arg __unused)
{
	...
	free(kp, M_CAP_RT_KSTORE);
}

static const struct cap_rt_ops kstore_ops = {
...
static int
cap_rt_test_kernelstore_modevent(module_t mod __unused, int type,
	...
	case MOD_LOAD:
		{
			...
			error = cap_rt_service_create(&p, &kstore_svc);
		}
		if (error != 0)
			return (error);
		if (bootverbose)
			printf("cap_rt_test_kernelstore: loaded\n");
		return (0);

	case MOD_UNLOAD:
		if (kstore_svc != NULL)
			cap_rt_service_destroy(kstore_svc);
		if (bootverbose)
			printf("cap_rt_test_kernelstore: unloaded\n");
		return (0);
...
static moduledata_t cap_rt_test_kernelstore_mod = {
	"cap_rt_test_kernelstore",
	cap_rt_test_kernelstore_modevent,
	NULL,
};

DECLARE_MODULE(cap_rt_test_kernelstore, cap_rt_test_kernelstore_mod, SI_SUB_PSEUDO,
    SI_ORDER_ANY);
MODULE_VERSION(cap_rt_test_kernelstore, 1);
MODULE_DEPEND(cap_rt_test_kernelstore, cap_rt, 1, 1, 1);
