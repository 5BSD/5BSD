    ...
    struct thread *td)
{
	...
	case IORING_OP_TEE: {
		...
		return (sq_result(ctx, td, linux_tee(td, &a)));
	}
	case IORING_OP_PIPE: {
	...
	case IORING_OP_SPLICE: {
		...
		a.len = sqe->len;
		a.flags = sqe->splice_flags;
		return (sq_result(ctx, td, linux_splice(td, &a)));
	}
	case IORING_OP_EPOLL_WAIT: {
		...
		a.mask = NULL;
		a.sigsetsize = 0;
		return (sq_result(ctx, td, linux_epoll_pwait(td, &a)));
	}
	...
	case IORING_OP_SENDMSG_ZC: {
		...
		mtx_lock(&ctx->mtx);
		if (r < 0) {
			sq_post_cqe(ctx, req->user_data, r, 0);
		} else {
			sq_post_cqe(ctx, req->user_data, r, IORING_CQE_F_MORE);
			sq_post_cqe(ctx, req->user_data,
	...
	case IORING_OP_FUTEX_WAIT: {
		...
		a.timeout = NULL;
		a.clockid = 0;
		return (sq_result(ctx, td, linux_futex_wait(td, &a)));
	}
	case IORING_OP_FUTEX_WAITV: {
		...
		a.timeout = NULL;
		a.clockid = 0;
		return (sq_result(ctx, td, linux_futex_waitv(td, &a)));
	}
	case IORING_OP_WAITID: {
		...
		a.rusage = NULL;
		return (sq_result(ctx, td, linux_waitid(td, &a)));
	}
	case IORING_OP_OPENAT: {
		...
		a.mode = sqe->len;
		return (sq_result(ctx, td, linux_openat(td, &a)));
	}
	case IORING_OP_OPENAT2: {
		...
		a.size = sqe->len;
		return (sq_result(ctx, td, linux_openat2(td, &a)));
	}
	...
	case IORING_OP_SOCKET: {
		...
		return (sq_result(ctx, td, linux_socket(td, &a)));
	}
	...
	case IORING_OP_ACCEPT: {
		...
		return (sq_result(ctx, td, linux_accept4(td, &a)));
	}
	case IORING_OP_BIND: {
	...
	case IORING_OP_RECV: {
		...
		a.len = sqe->len;
		a.flags = sqe->msg_flags | SQ_MSG_DONTWAIT;
		return (sq_result(ctx, td, linux_recvfrom(td, &a)));
	}
...
linux_io_uring_enter(struct thread *td, struct linux_io_uring_enter_args *args)
{
	...
	return (kern_squeue_enter(td, args->fd, args->to_submit,
    ...
    struct linux_io_uring_register_args *args)
{
	...
	return (kern_squeue_register(td, args->fd, args->opcode,
