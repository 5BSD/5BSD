    ...
    struct image_params *imgp __unused, struct label *execlabel __unused)
{
	...
	/*
	 * Determine the new process label in order of priority, writing the
	 * result directly into newvl.  newvl already holds a copy of the parent
	 * label (from cred_copy_label), and abac_rules_get_transition only
	 * writes its output on a matching rule (leaving newvl untouched on
	 * ENOENT), so no temporary is needed.
	 *
	 * This matters for correctness, not just tidiness: the hook runs during
	 * the execve credential swap with the process lock (a sleepable mutex)
	 * held, so it must not sleep.  Writing in place avoids the large
	 * M_WAITOK allocation the transition path previously made under the lock
	 * (struct abac_label is ~5KB, from ABAC_MAX_PAIRS * key/value sizes).
	 *
	 * 1. Explicit transition rule: if one matches, its label is written to
	 *    newvl by get_transition.
	 * 2. Vnode label: otherwise, if the executable is labeled, inherit it.
	 * 3. Parent label: otherwise keep the parent label already in newvl.
	 */
	error = abac_rules_get_transition(old, oldvl, objvl, newvl);
	if (error == 0) {
		/* Priority 1: Explicit transition rule (now in newvl) */
		...
		SDT_PROBE4(abac, cred, transition, exec,
		    oldbuf, newbuf, objbuf, curproc ? curproc->p_pid : 0);
	} else if (objvl->vl_npairs > 0) {
		/* Priority 2: Inherit vnode label */
		abac_label_copy(objvl, newvl);
	}
	/* Priority 3: Keep parent label (already in newvl) */
}
