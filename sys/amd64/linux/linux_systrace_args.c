...
systrace_args(int sysnum, void *params, uint64_t *uarg, int *n_args)
{
	...
	/* linux_mlockall */
	case 151: {
		struct linux_mlockall_args *p = params;
		iarg[a++] = p->how; /* l_int */
		*n_args = 1;
		break;
	}
	...
	/* linux_adjtimex */
	case 159: {
		struct linux_adjtimex_args *p = params;
		uarg[a++] = (intptr_t)p->txc; /* struct l_timex * */
		*n_args = 1;
		break;
	}
	/* linux_setrlimit */
	...
	case 167: {
		...
		uarg[a++] = (intptr_t)p->special; /* char * */
		iarg[a++] = p->swap_flags; /* l_int */
		*n_args = 2;
		break;
	}
	/* linux_swapoff */
	case 168: {
		struct linux_swapoff_args *p = params;
		uarg[a++] = (intptr_t)p->special; /* char * */
		*n_args = 1;
		break;
	}
	...
	case 173: {
		...
		iarg[a++] = p->from; /* l_ulong */
		iarg[a++] = p->num; /* l_ulong */
		iarg[a++] = p->turn_on; /* l_int */
		*n_args = 3;
		break;
	}
	/* linux_init_module */
	...
	/* linux_quotactl */
	case 179: {
		struct linux_quotactl_args *p = params;
		iarg[a++] = p->cmd; /* l_uint */
		uarg[a++] = (intptr_t)p->special; /* const char * */
		iarg[a++] = p->id; /* l_uint */
	...
	/* linux_io_setup */
	case 206: {
		struct linux_io_setup_args *p = params;
		iarg[a++] = p->nr_events; /* l_uint */
		uarg[a++] = (intptr_t)p->ctxp; /* l_ulong * */
		*n_args = 2;
		break;
	}
	/* linux_io_destroy */
	case 207: {
		struct linux_io_destroy_args *p = params;
		iarg[a++] = p->ctx; /* l_ulong */
		*n_args = 1;
		break;
	}
	/* linux_io_getevents */
	case 208: {
		struct linux_io_getevents_args *p = params;
		iarg[a++] = p->ctx; /* l_ulong */
		iarg[a++] = p->min_nr; /* l_long */
		iarg[a++] = p->nr; /* l_long */
		uarg[a++] = (intptr_t)p->events; /* struct l_aio_event * */
		uarg[a++] = (intptr_t)p->timeout; /* struct l_timespec * */
		*n_args = 5;
		break;
	}
	/* linux_io_submit */
	case 209: {
		struct linux_io_submit_args *p = params;
		iarg[a++] = p->ctx; /* l_ulong */
		iarg[a++] = p->nr; /* l_long */
		uarg[a++] = (intptr_t)p->iocbpp; /* struct l_aio_iocb ** */
		*n_args = 3;
		break;
	}
	/* linux_io_cancel */
	case 210: {
		struct linux_io_cancel_args *p = params;
		iarg[a++] = p->ctx; /* l_ulong */
		uarg[a++] = (intptr_t)p->iocb; /* struct l_aio_iocb * */
		uarg[a++] = (intptr_t)p->result; /* struct l_aio_event * */
		*n_args = 3;
		break;
	}
	...
	case 216: {
		struct linux_remap_file_pages_args *p = params;
		iarg[a++] = p->start; /* l_ulong */
		iarg[a++] = p->size; /* l_ulong */
		iarg[a++] = p->prot; /* l_ulong */
		iarg[a++] = p->pgoff; /* l_ulong */
		iarg[a++] = p->flags; /* l_ulong */
		*n_args = 5;
	...
	/* linux_unshare */
	case 272: {
		struct linux_unshare_args *p = params;
		iarg[a++] = p->flags; /* l_ulong */
		*n_args = 1;
		break;
	}
	...
	/* linux_vmsplice */
	case 278: {
		struct linux_vmsplice_args *p = params;
		iarg[a++] = p->fd; /* l_int */
		uarg[a++] = (intptr_t)p->iov; /* const struct iovec * */
		iarg[a++] = p->nr_segs; /* l_ulong */
		iarg[a++] = p->flags; /* l_uint */
		*n_args = 4;
		break;
	}
	...
	/* linux_signalfd */
	case 282: {
		struct linux_signalfd_args *p = params;
		iarg[a++] = p->fd; /* l_int */
		uarg[a++] = (intptr_t)p->mask; /* l_sigset_t * */
		iarg[a++] = p->sizemask; /* l_size_t */
		*n_args = 3;
		break;
	}
	...
	/* linux_signalfd4 */
	case 289: {
		struct linux_signalfd4_args *p = params;
		iarg[a++] = p->fd; /* l_int */
		uarg[a++] = (intptr_t)p->mask; /* l_sigset_t * */
		iarg[a++] = p->sizemask; /* l_size_t */
		iarg[a++] = p->flags; /* l_int */
	...
	case 298: {
		struct linux_perf_event_open_args *p = params;
		uarg[a++] = (intptr_t)p->attr; /* void * */
		iarg[a++] = p->pid; /* l_pid_t */
		iarg[a++] = p->cpu; /* l_int */
		iarg[a++] = p->group_fd; /* l_int */
		iarg[a++] = p->flags; /* l_ulong */
		*n_args = 5;
		break;
	}
	...
	/* linux_clock_adjtime */
	case 305: {
		struct linux_clock_adjtime_args *p = params;
		iarg[a++] = p->which; /* l_int */
		uarg[a++] = (intptr_t)p->tx; /* struct l_timex * */
		*n_args = 2;
		break;
	}
	/* linux_syncfs */
	...
	/* linux_io_pgetevents */
	case 333: {
		struct linux_io_pgetevents_args *p = params;
		iarg[a++] = p->ctx; /* l_ulong */
		iarg[a++] = p->min_nr; /* l_long */
		iarg[a++] = p->nr; /* l_long */
		uarg[a++] = (intptr_t)p->events; /* struct l_aio_event * */
		uarg[a++] = (intptr_t)p->timeout; /* struct l_timespec * */
		uarg[a++] = (intptr_t)p->sig; /* struct l_aio_sigset * */
		*n_args = 6;
		break;
	}
	/* linux_rseq */
	...
	/* linux_pidfd_open */
	case 434: {
		struct linux_pidfd_open_args *p = params;
		iarg[a++] = p->pid; /* l_pid_t */
		iarg[a++] = p->flags; /* l_uint */
		*n_args = 2;
		break;
	...
	/* linux_openat2 */
	case 437: {
		struct linux_openat2_args *p = params;
		iarg[a++] = p->dfd; /* l_int */
		uarg[a++] = (intptr_t)p->filename; /* const char * */
		uarg[a++] = (intptr_t)p->how; /* struct l_open_how * */
		iarg[a++] = p->size; /* l_size_t */
		*n_args = 4;
		break;
	}
	/* linux_pidfd_getfd */
	case 438: {
		struct linux_pidfd_getfd_args *p = params;
		iarg[a++] = p->pidfd; /* l_int */
		iarg[a++] = p->fd; /* l_int */
		iarg[a++] = p->flags; /* l_uint */
		*n_args = 3;
	...
	case 440: {
		struct linux_process_madvise_args *p = params;
		iarg[a++] = p->pidfd; /* l_int */
		uarg[a++] = (intptr_t)p->vec; /* void * */
		iarg[a++] = p->vlen; /* l_size_t */
		iarg[a++] = p->behavior; /* l_int */
		iarg[a++] = p->flags; /* l_uint */
		*n_args = 5;
		break;
	}
	...
	/* linux_quotactl_fd64 */
	case 443: {
		struct linux_quotactl_fd64_args *p = params;
		iarg[a++] = p->fd; /* l_uint */
		iarg[a++] = p->cmd; /* l_uint */
		iarg[a++] = p->id; /* l_uint */
		uarg[a++] = (intptr_t)p->addr; /* void * */
	...
	/* linux_futex_waitv */
	case 449: {
		struct linux_futex_waitv_args *p = params;
		uarg[a++] = (intptr_t)p->waiters; /* struct l_futex_waitv * */
		iarg[a++] = p->nr_futexes; /* l_uint */
		iarg[a++] = p->flags; /* l_uint */
		uarg[a++] = (intptr_t)p->timeout; /* struct l_timespec * */
		iarg[a++] = p->clockid; /* l_int */
		*n_args = 5;
	...
	/* linux_futex_wake */
	case 454: {
		struct linux_futex_wake_args *p = params;
		uarg[a++] = (intptr_t)p->uaddr; /* void * */
		iarg[a++] = p->mask; /* l_ulong */
		iarg[a++] = p->nr; /* l_int */
		iarg[a++] = p->flags; /* l_uint */
		*n_args = 4;
		break;
	...
	/* linux_futex_wait */
	case 455: {
		struct linux_futex_wait_args *p = params;
		uarg[a++] = (intptr_t)p->uaddr; /* void * */
		iarg[a++] = p->val; /* l_ulong */
		iarg[a++] = p->mask; /* l_ulong */
		iarg[a++] = p->flags; /* l_uint */
		uarg[a++] = (intptr_t)p->timeout; /* struct l_timespec * */
		iarg[a++] = p->clockid; /* l_int */
	...
	/* linux_futex_requeue */
	case 456: {
		struct linux_futex_requeue_args *p = params;
		uarg[a++] = (intptr_t)p->waiters; /* struct l_futex_waitv * */
		iarg[a++] = p->flags; /* l_uint */
		iarg[a++] = p->nr_wake; /* l_int */
		iarg[a++] = p->nr_requeue; /* l_int */
		*n_args = 4;
		break;
	}
	/* linux_statmount */
	...
	/* linux_mseal */
	case 462: {
		struct linux_mseal_args *p = params;
		iarg[a++] = p->addr; /* l_ulong */
		iarg[a++] = p->len; /* l_size_t */
		iarg[a++] = p->flags; /* l_ulong */
		*n_args = 3;
	...
	/* linux_setxattrat */
	case 463: {
		struct linux_setxattrat_args *p = params;
		iarg[a++] = p->dfd; /* l_int */
		uarg[a++] = (intptr_t)p->path; /* const char * */
		iarg[a++] = p->at_flags; /* l_uint */
		uarg[a++] = (intptr_t)p->name; /* const char * */
		uarg[a++] = (intptr_t)p->args; /* struct l_xattr_args * */
		iarg[a++] = p->size; /* l_size_t */
		*n_args = 6;
		break;
	}
	/* linux_getxattrat */
	case 464: {
		struct linux_getxattrat_args *p = params;
		iarg[a++] = p->dfd; /* l_int */
		uarg[a++] = (intptr_t)p->path; /* const char * */
		iarg[a++] = p->at_flags; /* l_uint */
		uarg[a++] = (intptr_t)p->name; /* const char * */
	...
	/* linux_listxattrat */
	case 465: {
		struct linux_listxattrat_args *p = params;
		iarg[a++] = p->dfd; /* l_int */
		uarg[a++] = (intptr_t)p->path; /* const char * */
		iarg[a++] = p->at_flags; /* l_uint */
		uarg[a++] = (intptr_t)p->list; /* char * */
	...
	/* linux_removexattrat */
	case 466: {
		struct linux_removexattrat_args *p = params;
		iarg[a++] = p->dfd; /* l_int */
		uarg[a++] = (intptr_t)p->path; /* const char * */
		iarg[a++] = p->at_flags; /* l_uint */
		uarg[a++] = (intptr_t)p->name; /* const char * */
	...
	/* linux_file_getattr */
	case 468: {
		struct linux_file_getattr_args *p = params;
		iarg[a++] = p->dfd; /* l_int */
		uarg[a++] = (intptr_t)p->filename; /* const char * */
		uarg[a++] = (intptr_t)p->fattr; /* struct l_file_attr * */
		iarg[a++] = p->size; /* l_size_t */
		iarg[a++] = p->at_flags; /* l_uint */
		*n_args = 5;
		break;
	...
	/* linux_file_setattr */
	case 469: {
		struct linux_file_setattr_args *p = params;
		iarg[a++] = p->dfd; /* l_int */
		uarg[a++] = (intptr_t)p->filename; /* const char * */
		uarg[a++] = (intptr_t)p->fattr; /* struct l_file_attr * */
		iarg[a++] = p->size; /* l_size_t */
	...
	/* linux_fchroot */
	case 472: {
		struct linux_fchroot_args *p = params;
		iarg[a++] = p->fd; /* l_int */
		iarg[a++] = p->flags; /* l_uint */
		*n_args = 2;
		break;
...
systrace_entry_setargdesc(int sysnum, int ndx, char *desc, size_t descsz)
{
	...
	case 333:
		...
		case 4:
			...
			break;
		case 5:
			p = "userland struct l_aio_sigset *";
			break;
		default:
			break;
		};
	...
	case 449:
		switch (ndx) {
		case 0:
			p = "userland struct l_futex_waitv *";
			break;
		case 1:
			p = "l_uint";
			break;
	...
	case 463:
		...
		case 3:
			...
			break;
		case 4:
			p = "userland struct l_xattr_args *";
			break;
		case 5:
			p = "l_size_t";
			break;
